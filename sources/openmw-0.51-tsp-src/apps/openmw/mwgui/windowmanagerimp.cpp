...
namespace MWGui
{
    ...
    void WindowManager::updateVisible()
    {
        ...
        // TSP_UI_DIALOGUE_FIX_051_V35
        const bool tspDialogueMode
            = getMode() == GM_Dialogue;
    ...
    {
        // TSP_RESOLUTION_RESIZE_EVENT_051_V21
        // SDL/EGL reports the physical drawable here. Do not copy that
        // 1280x720 size back into the internal-render resolution setting.
        const int tspInternalWidth = Settings::video().mResolutionX;
        const int tspInternalHeight = Settings::video().mResolutionY;
        Log(Debug::Info)
            << "TSP_RESOLUTION_RESIZE_EVENT_051_V21"
            << " physical=" << x << "x" << y
            << " internal_preserved=" << tspInternalWidth << "x" << tspInternalHeight;
        Settings::CategorySettingVector filter = {
