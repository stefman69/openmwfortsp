...
OUTPUT_BINARY="${4:-/root/openmw-0.51-tsp-v15-depthbands-syncguard-backupbase}"
JOBS="${OPENMW_JOBS:-1}"
...
# Explicitly rebuild from the known-good source snapshot created immediately
# before the failed 05:25:54 V15 attempt. Override only if deliberately needed:
#   OPENMW_BASELINE_BACKUP=/path/to/another/backup
BASELINE_BACKUP="${OPENMW_BASELINE_BACKUP:-$SOURCE_DIR/.tsp-051-source-backups/v15-depthbands-syncguard-20260810-052554}"

# Preserve whatever is live right now before replacing it with the baseline.
PRE_RESTORE_BACKUP="$SOURCE_DIR/.tsp-051-source-backups/v15-pre-restore-current-$STAMP"

# This is the normal rollback point AFTER the requested baseline has been restored.
BACKUP_DIR="$SOURCE_DIR/.tsp-051-source-backups/v15-depthbands-syncguard-backupbase-$STAMP"
...
echo
echo "===== RESTORING REQUESTED SOURCE BASELINE ====="
echo "Baseline:"
...
do
    if [ ! -f "$BASELINE_BACKUP/$rel" ]; then
        ...
        echo "The live source has NOT been changed."
        exit 1
    fi
done
...
echo "PASS: restored the exact 20260810-052554 pre-attempt source baseline."
echo "Previous live source preserved at:"
...
# ---------------------------------------------------------------------
# 0. Normalize any alternate "fresh reload by default" V15 back to the
#    requested corrected-V13 behavior: warm/in-process is the default,
#    while [TSP] safe reload = 1 remains the explicit emergency fallback.
# ---------------------------------------------------------------------
if "TSP_FRESH_DEFAULT_051_V15" in state:
    state = re.sub(
        r'(?P<indent>^[ \t]*)// TSP_FRESH_DEFAULT_051_V15\s*\n'
        r'(?P=indent)bool\s+enabled\s*=\s*true\s*;',
        lambda m: m.group("indent") + 'bool enabled = false;',
        state,
        count=1,
        flags=re.MULTILINE,
    )
    ...
    state = re.sub(
        r'(?P<indent>^[ \t]*)Log\(Debug::Info\) << "TSP_FRESH_DEFAULT_051_V15"\n'
        r'(?P=indent)[ \t]*<< " effective_fresh=" << \(tspSafeReload\.enabled \? 1 : 0\)\n'
        r'(?P=indent)[ \t]*<< " explicit=" << \(tspSafeReload\.found \? 1 : 0\)\n'
        r'(?P=indent)[ \t]*<< " source=" << tspSafeReload\.source;\n',
        '',
        state,
        count=1,
        flags=re.MULTILINE,
    ...
    # Remove any remaining marker comment without touching V13's actual
    # safe-reload implementation.
    state = state.replace("// TSP_FRESH_DEFAULT_051_V15\n", "")

if re.search(r'bool\s+enabled\s*=\s*true\s*;', state) and "struct TspSafeReloadSetting" in state:
    # Only fail if the safe-reload struct itself still defaults true.
    struct_match = re.search(
        r'struct\s+TspSafeReloadSetting\s*\{(?P<body>.*?)\};',
        state,
        flags=re.DOTALL,
    )
    if struct_match and re.search(r'bool\s+enabled\s*=\s*true\s*;', struct_match.group("body")):
        raise RuntimeError("statemanagerimp.cpp: safe reload still defaults to fresh after normalization")
...
'''
    # Do not depend on IdentifyOpenGLOperation: this TSP tree has been revised
    # enough that the class is no longer a reliable insertion anchor.
    # Insert the helper immediately before the first namespace declaration,
    # after all includes have already been parsed.
    namespace_anchor = re.search(r'^[ \t]*namespace\b', engine, flags=re.MULTILINE)
    if not namespace_anchor:
        raise RuntimeError("engine.cpp: first namespace anchor missing")
    engine = engine[:namespace_anchor.start()] + helper + engine[namespace_anchor.start():]
...
if V15_CRASH not in state:
    ...
    # Do not require statemanagerimp.cpp to use a literal
    # "namespace MWState {" wrapper. Some OpenMW 0.51 source layouts define
    # StateManager methods with qualified names instead.
    #
    # The V15 tracer lives in its own anonymous namespace, so it can safely
    # be inserted immediately before the first StateManager implementation.
    state_manager_anchor = re.search(
        r'^[^\n]*StateManager::',
        state,
        flags=re.MULTILINE,
    )
    if not state_manager_anchor:
        raise RuntimeError(
            "statemanagerimp.cpp: first StateManager implementation anchor missing"
        )

    crash_insert_at = state_manager_anchor.start()
...
if "TSP_FRESH_DEFAULT_051_V15" in state:
    raise RuntimeError("statemanagerimp.cpp: alternate fresh-by-default V15 marker still present")

safe_struct = re.search(
    r'struct\s+TspSafeReloadSetting\s*\{(?P<body>.*?)\};',
    state,
    flags=re.DOTALL,
)
if safe_struct and re.search(r'bool\s+enabled\s*=\s*true\s*;', safe_struct.group("body")):
    raise RuntimeError("statemanagerimp.cpp: safe reload unexpectedly defaults to fresh")
...
echo "Baseline source: $BASELINE_BACKUP"
echo "Old live source: $PRE_RESTORE_BACKUP"
echo "Patch backup:    $BACKUP_DIR"
echo
echo "Defaults:"
echo "  safe reload absent/0 -> warm reload / NO ESM reparse"
