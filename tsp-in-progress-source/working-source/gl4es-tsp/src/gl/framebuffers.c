 ...
 * New diagnostic area: exact native GLES texture object + backing-storage
 * accounting, plus map RTT readback.  Everything in this block is dormant
 * unless LIBGL_TSP_TEXTURE_DIAG=1.
 *
 * Unlike TSP_TEXLIFE_V2 (which follows gl4es wrapper objects), this records
 * the native GLES names and every glTexImage2D/glCompressedTexImage2D backing
 * allocation that actually reaches the PowerVR driver.
 */
typedef struct {
    GLuint id;
    unsigned long long level_bytes[16];
    unsigned long long total_bytes;
    int used;
} tsp_native_tex_v2_t;

static tsp_native_tex_v2_t* tsp_tex_v2 = NULL;
static size_t tsp_tex_v2_cap = 0;
static unsigned long tsp_tex_v2_gen = 0;
static unsigned long tsp_tex_v2_del = 0;
static unsigned long tsp_tex_v2_storage = 0;
static unsigned long long tsp_tex_v2_live_bytes = 0;
static unsigned long tsp_tex_v2_seq = 0;

static int tsp_texture_diag_on(void)
{
    static int checked = 0;
    static int on = 0;
    if (!checked) {
        const char* e = getenv("LIBGL_TSP_TEXTURE_DIAG");
        on = (e && e[0] == '1');
        checked = 1;
    }
    return on;
...
static FILE* tsp_texture_diag_file(void)
{
    static int checked = 0;
    static FILE* file = NULL;
    if (!tsp_texture_diag_on())
        return NULL;
    if (!checked) {
        const char* p = getenv("LIBGL_TSP_TEXTURE_DIAG_PATH");
        checked = 1;
        if (p && p[0]) {
            file = fopen(p, "a");
            if (file) {
                fprintf(file, "# TSP_NATIVE_TEXTURE_DIAG_V2 seq event fields\n");
                fflush(file);
            }
        }
    }
...
static void tsp_texture_diag_log(const char* fmt, ...)
{
    FILE* f = tsp_texture_diag_file();
    if (!f)
        return;
    unsigned long seq = __sync_add_and_fetch(&tsp_tex_v2_seq, 1);
    fprintf(f, "seq=%lu ", seq);
    va_list ap;
    va_start(ap, fmt);
    vfprintf(f, fmt, ap);
...
static tsp_native_tex_v2_t* tsp_tex_v2_slot(GLuint id, int create)
{
    if (!id)
        return NULL;

    if ((size_t)id >= tsp_tex_v2_cap) {
        if (!create)
            return NULL;

        size_t cap = tsp_tex_v2_cap ? tsp_tex_v2_cap : 1024;
        while (cap <= (size_t)id)
            cap *= 2;

        tsp_native_tex_v2_t* n = realloc(
            tsp_tex_v2,
            cap * sizeof(tsp_native_tex_v2_t));

        if (!n) {
            tsp_texture_diag_log(
                "ERROR table_alloc id=%u requested_cap=%zu",
                id,
                cap);
            return NULL;
        }
        ...
        memset(
            n + tsp_tex_v2_cap,
            0,
            (cap - tsp_tex_v2_cap) * sizeof(tsp_native_tex_v2_t));

        tsp_tex_v2 = n;
        tsp_tex_v2_cap = cap;
    }

    tsp_native_tex_v2_t* r = &tsp_tex_v2[id];
...
static unsigned int tsp_texture_channels(GLenum format)
{
    switch (format) {
        case GL_ALPHA:
        case GL_LUMINANCE:
...
static unsigned long long tsp_texture_estimate(
    GLsizei width,
    GLsizei height,
    GLenum format,
    GLenum type)
{
    ...
    switch (type) {
        ...
        case GL_HALF_FLOAT:
            bpp = 2ULL * tsp_texture_channels(format);
            break;
        case GL_FLOAT:
            bpp = 4ULL * tsp_texture_channels(format);
            break;
        ...
        case GL_BYTE:
        default:
            bpp = tsp_texture_channels(format);
            break;
    }
...
void tsp_native_texture_generated_v2(GLuint id, const char* owner)
{
    if (!tsp_texture_diag_on() || !id)
        return;

    tsp_native_tex_v2_t* r = tsp_tex_v2_slot(id, 1);
    int reused = (r && r->total_bytes != 0);

    ++tsp_tex_v2_gen;

    tsp_texture_diag_log(
        "GEN_TEX id=%u owner=%s gen=%lu del=%lu live=%ld tracked_bytes=%llu reused=%d",
        id,
        owner ? owner : "?",
        tsp_tex_v2_gen,
        tsp_tex_v2_del,
        (long)tsp_tex_v2_gen - (long)tsp_tex_v2_del,
        tsp_tex_v2_live_bytes,
        reused);
}

void tsp_native_texture_deleted_v2(GLuint id, const char* owner)
{
    if (!tsp_texture_diag_on() || !id)
        return;

    tsp_native_tex_v2_t* r = tsp_tex_v2_slot(id, 0);
    unsigned long long released = 0;
    int tracked = 0;

    if (r) {
        released = r->total_bytes;
        tracked = 1;

        if (tsp_tex_v2_live_bytes >= released)
            tsp_tex_v2_live_bytes -= released;
        else
            tsp_tex_v2_live_bytes = 0;
    ...
    tsp_texture_diag_log(
        "DEL_TEX id=%u owner=%s tracked=%d released=%llu gen=%lu del=%lu live=%ld live_bytes=%llu",
        id,
        owner ? owner : "?",
        tracked,
        released,
...
void tsp_native_texture_storage_v2(
    GLuint id,
    GLint level,
    GLsizei width,
    GLsizei height,
    ...
    const char* owner)
{
    ...
    unsigned long long bytes = explicit_bytes
        ? explicit_bytes
        : tsp_texture_estimate(width, height, format, type);

    tsp_native_tex_v2_t* r = tsp_tex_v2_slot(id, id != 0);
    unsigned long long old = 0;

    if (r && level >= 0 && level < 16) {
        old = r->level_bytes[level];

        if (r->total_bytes >= old)
            r->total_bytes -= old;
        else
            r->total_bytes = 0;

        if (tsp_tex_v2_live_bytes >= old)
            tsp_tex_v2_live_bytes -= old;
        else
            tsp_tex_v2_live_bytes = 0;

        r->level_bytes[level] = bytes;
        r->total_bytes += bytes;
        tsp_tex_v2_live_bytes += bytes;
    }

    ++tsp_tex_v2_storage;

    tsp_texture_diag_log(
        "TEX_STORAGE id=%u owner=%s level=%d size=%dx%d ifmt=0x%x fmt=0x%x type=0x%x bytes=%llu old=%llu realloc=%d tex_bytes=%llu live_bytes=%llu calls=%lu",
        id,
        owner ? owner : "?",
        level,
        width,
...
/* Wrap the exact GLES allocation boundary.  The extra glGetIntegerv exists
 * only while diagnostics are armed; with them off this is a straight-through
 * call to the driver.
 */
void tsp_native_gles_teximage2d_v2(
    GLenum target,
    GLint level,
    ...
    const GLvoid* data)
{
    ...
    if (tsp_texture_diag_on() && target == GL_TEXTURE_2D) {
        LOAD_GLES(glGetIntegerv);
        if (gles_glGetIntegerv) {
            GLint bound = 0;
            gles_glGetIntegerv(GL_TEXTURE_BINDING_2D, &bound);
            if (bound > 0)
                id = (GLuint)bound;
        }
    }
...
void tsp_native_gles_compressed_teximage2d_v2(
    GLenum target,
    GLint level,
    GLenum internalformat,
    GLsizei width,
    ...
    const GLvoid* data)
{
    ...
    if (tsp_texture_diag_on()) {
        tsp_native_texture_storage_v2(
            ...
            internalformat,
            GL_UNSIGNED_BYTE,
            (unsigned long long)((imageSize > 0) ? imageSize : 0),
            "glCompressedTexImage2D");
    }
}

/* Keep V1 FBO/RB diagnostics useful, but feed the new native-texture tracker
 * from the same exact object boundary.
 */
void tsp_native_life_texture_generated(GLuint id)
{
    if (id && tsp_native_life_on()) {
        ++tsp_native_tex_gen;
        tsp_native_log(
            "GEN_TEX gl=%u gen=%lu del=%lu live=%ld",
            id,
            tsp_native_tex_gen,
            tsp_native_tex_del,
            (long)tsp_native_tex_gen - (long)tsp_native_tex_del);
    }

    tsp_native_texture_generated_v2(id, "native-gen");
}

void tsp_native_life_texture_deleted(GLuint id)
{
    if (id && tsp_native_life_on()) {
        ++tsp_native_tex_del;
        tsp_native_log(
            "DEL_TEX gl=%u gen=%lu del=%lu live=%ld",
            id,
            tsp_native_tex_gen,
            tsp_native_tex_del,
            (long)tsp_native_tex_gen - (long)tsp_native_tex_del);
    ...
    tsp_native_texture_deleted_v2(id, "native-delete");
}
...
/* TSP_BINDSYNC_20260816 */
static int tsp_bindfb_sync(void);
static int tsp_bindfb_sync(void) {
    /* The GL_DRAW_FRAMEBUFFER branch updated fbo_draw but left fbo_read
       pointing at the previously bound framebuffer, so ReadDraw_Push could
       rebind mid-sequence. Measured: 211 -> 440 complete framebuffers.
       LIBGL_TSP_BINDSYNC=0 restores original behaviour. */
    static int chk = 0, on = 1;
    if (!chk) { const char* e = getenv("LIBGL_TSP_BINDSYNC"); chk = 1;
                if (e && e[0] == '0') on = 0; }
    return on;
}

/* TSP_FBNORM_20260816 */
static GLenum tsp_fbnorm(GLenum target);
static GLenum tsp_fbnorm(GLenum target) {
    /* GLES2 has no read/draw framebuffer split. Normalise so bind, attach and
       status-check all refer to the same object.
       LIBGL_TSP_FBNORM=0 restores original behaviour. */
    static int chk = 0, on = 1;
    if (!chk) { const char* e = getenv("LIBGL_TSP_FBNORM"); chk = 1;
                if (e && e[0] == '0') on = 0; }
    if (!on) return target;
...
/* TSP_PACKED_DS_20260823 */
static int tsp_sep_depth(void);
static int tsp_sep_depth(void) {
    static int chk = 0, sep = 0;
    if (!chk) { const char* e = getenv("LIBGL_TSP_SEPDEPTH"); chk = 1;
                if (e && e[0] == '1') sep = 1; }
    return sep;
}

/* TSP_RTT_DEPTH24_20260815 */
static GLenum tsp_rtt_depth_format(void);
static GLenum tsp_rtt_depth_format(void) {
    /* gl4es hardcoded RTT depth renderbuffers to GL_DEPTH_COMPONENT16.
       This GPU reports oes_depth24=0 but packed_depth_stencil=1, so prefer
       DEPTH24_STENCIL8. LIBGL_TSP_RTTDEPTH=0 restores 16-bit. */
    static int chk = 0, on = 1;
    if (!chk) { const char* e = getenv("LIBGL_TSP_RTTDEPTH"); chk = 1;
                if (e && e[0] == '0') on = 0; }
    if (!on) return GL_DEPTH_COMPONENT16;
    /* TSP_PACKED_DS_20260823: prefer PACKED so depth and stencil can share one
       renderbuffer. Mali will not pair a separate depth buffer with a separate
       GL_STENCIL_INDEX8 buffer - it reports COMPLETE and renders undefined.
       LIBGL_TSP_SEPDEPTH=1 restores the old separate-buffer behaviour. */
    if (!tsp_sep_depth()) return GL_DEPTH24_STENCIL8;
    if (hardext.depth24) return GL_DEPTH_COMPONENT24;
    if (hardext.depthstencil) return GL_DEPTH24_STENCIL8;
    return GL_DEPTH24_STENCIL8;   /* driver advertises it even if probe missed */
}

/* TSP_RTT_STENCIL_20260823 */
static void tsp_attach_depth_rb(GLenum ntarget, GLuint rb);
static void tsp_attach_depth_rb(GLenum ntarget, GLuint rb) {
    /* A GL_DEPTH24_STENCIL8 renderbuffer is a PACKED depth+stencil buffer, and
       OES_packed_depth_stencil requires it to be attached to BOTH
       GL_DEPTH_ATTACHMENT and GL_STENCIL_ATTACHMENT. gl4es attached it to the
       depth point only, leaving the framebuffer in an undefined state that Mali
       still reports as COMPLETE - so nothing in OSG or gl4es ever warned.
       Surfaces that redraw every frame never show it. The local map and the
       inventory character preview render exactly once and keep the result, which
       is why only those two were broken.
       LIBGL_TSP_NOSTENCIL=1 restores the original single attachment. */
    static int chk = 0, on = 1;
    if (!chk) { const char* e = getenv("LIBGL_TSP_NOSTENCIL"); chk = 1;
                if (e && e[0] == '1') on = 0; }
    gl4es_glFramebufferRenderbuffer(ntarget, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, rb);
    if (on && rb) {
        GLint fmt = 0;
        GLint prev = 0;
        gl4es_glGetIntegerv(GL_RENDERBUFFER_BINDING, &prev);
        gl4es_glBindRenderbuffer(GL_RENDERBUFFER, rb);
        gl4es_glGetRenderbufferParameteriv(GL_RENDERBUFFER, GL_RENDERBUFFER_INTERNAL_FORMAT, &fmt);
        gl4es_glBindRenderbuffer(GL_RENDERBUFFER, (GLuint)prev);
        if (fmt == GL_DEPTH24_STENCIL8)
            gl4es_glFramebufferRenderbuffer(ntarget, GL_STENCIL_ATTACHMENT, GL_RENDERBUFFER, rb);
    }
}

const char* volatile tspFboAlwaysReattachTag = "TSP_FBO_ALWAYS_REATTACH_20260824";
const char* volatile tspFboGlnameFixTag = "TSP_FBO_GLNAME_FIX_20260824";
/* TSP_FBO_DIAG_20260815 */
static int tsp_fbo_on(void){
    static int chk=0,on=0;
    if(!chk){const char*e=getenv("LIBGL_TSP_FBODUMP");chk=1;on=(e&&e[0]=='1');}
    return on;
}
static void tsp_fbo_log(const char* fmt, ...){
    va_list ap; FILE* f;
    static const char* path=0; static int chk=0;
    if(!tsp_fbo_on()) return;
    if(!chk){chk=1; path=getenv("LIBGL_TSP_FBOPATH"); if(!path||!path[0]) path="/mnt/SDCARD/tsp_fbo.txt";}
...
/* TSP_FBO_TRACK_20260823 - measure only. Records which colour texture each
   framebuffer currently holds, so a bind can be attributed to a texture. */
#define TSP_FBTRACK_MAX 128
static GLuint tsp_fbcolor[TSP_FBTRACK_MAX];
static void tsp_fbtrack_set(GLuint fb, GLuint tex) {
    if (fb < TSP_FBTRACK_MAX) tsp_fbcolor[fb] = tex;
}
static GLuint tsp_fbtrack_get(GLuint fb) {
    return (fb < TSP_FBTRACK_MAX) ? tsp_fbcolor[fb] : 0;
}

/* TSP_FBO_SNAPSHOT_20260823 - MEASURE ONLY.
   At unbind time, dumps the contents of the two render targets we care about
   (256x256 local map cells, 512x1024 character preview) to PPM files, so we can
   see whether the RTT content is correct at the moment it finishes rendering.
   Completely inert unless LIBGL_TSP_FBOSNAP names a directory. */
/* TSP_DRAWFBO_V7 */
#ifndef RTLD_NOLOAD
 ...
 * Dump a few finished map RTTs straight from the native PowerVR framebuffer
 * before gl4es unbinds it.  This answers a new question that attachment/status
 * logging cannot: is the map already corrupt in GPU render-target memory?
 *
 * Limited to four 256x256 local-map dumps and three 954x864 world-map dumps.
 * Completely inert unless LIBGL_TSP_MAP_READBACK_DIR names a directory.
 */
const char* volatile tspMapRttReadbackV1Tag = "TSP_MAP_RTT_READBACK_V1";
static int tsp_map_readback_on(void)
{
    static int checked = 0;
    static int on = 0;
    if (!checked) {
        const char* p = getenv("LIBGL_TSP_MAP_READBACK_DIR");
        on = (p && p[0]);
        checked = 1;
    }
    return on;
...
static unsigned long long tsp_fnv1a64(
...
static void tsp_map_readback_dump(void)
{
    static unsigned int local_n = 0;
    static unsigned int world_n = 0;

    if (!tsp_map_readback_on())
        return;

    if (!glstate || !glstate->fbo.fbo_draw)
        return;
    ...
    const char* dir = getenv("LIBGL_TSP_MAP_READBACK_DIR");
    if (!dir || !dir[0])
        return;

    size_t rgba_bytes = (size_t)w * (size_t)h * 4U;
    unsigned char* rgba = malloc(rgba_bytes);

    if (!rgba) {
        tsp_texture_diag_log(
            "MAP_READBACK_ERROR kind=%s fb=%u reason=malloc bytes=%zu",
            kind,
            fb->id,
            rgba_bytes);
        return;
    ...
    unsigned long long hash = tsp_fnv1a64(rgba, rgba_bytes);
    GLuint tex = tsp_fbtrack_get(fb->id);
    ...
    if (!out) {
        tsp_texture_diag_log(
            "MAP_READBACK_ERROR kind=%s fb=%u tex=%u reason=fopen path=%s hash=%016llx",
            kind,
            fb->id,
            tex,
            path,
    ...
    unsigned char* rgb = malloc((size_t)w * 3U);

    if (!rgb) {
        fclose(out);
        free(rgba);
        tsp_texture_diag_log(
            "MAP_READBACK_ERROR kind=%s fb=%u tex=%u reason=rowmalloc",
            kind,
            fb->id,
            tex);
        return;
    ...
    /* GL's framebuffer origin is bottom-left; write top row first for PPM. */
    for (int y = h - 1; y >= 0; --y) {
        const unsigned char* src
            = rgba + ((size_t)y * (size_t)w * 4U);
    ...
    tsp_texture_diag_log(
        "MAP_READBACK kind=%s index=%u fb=%u tex=%u size=%dx%d hash=%016llx center=%u,%u,%u,%u path=%s",
        kind,
        index,
        fb->id,
        tex,
...
GLenum APIENTRY_GL4ES gl4es_glCheckFramebufferStatus(GLenum target) {
    target = tsp_fbnorm(target);
    tsp_fbo_log("CHECK target=0x%x", target);
    GLenum result;
...
void APIENTRY_GL4ES gl4es_glBindFramebuffer(GLenum target, GLuint framebuffer) {
    tsp_fbo_mark((unsigned int)framebuffer);
    target = tsp_fbnorm(target);
    tsp_fbo_log("BIND target=0x%x fb=%u color=%u", target, framebuffer, tsp_fbtrack_get(framebuffer));
    if (framebuffer == 0) tsp_fbo_snap();
...
void APIENTRY_GL4ES gl4es_glFramebufferTexture2D(GLenum target, GLenum attachment, GLenum textarget, GLuint texture, GLint level) {
    target = tsp_fbnorm(target);
    tsp_fbo_log("ATTACH_TEX fb=%u target=0x%x att=0x%x textarget=0x%x tex=%u level=%d",
        glstate->fbo.current_fb ? glstate->fbo.current_fb->id : 0,
        target, attachment, textarget, texture, level);
    if (attachment == GL_COLOR_ATTACHMENT0 && glstate->fbo.current_fb)
        tsp_fbtrack_set(glstate->fbo.current_fb->id, texture);
    ...
    if (texture) {
        ...
        if (!tex) {
            ...
            if (globals4es.fbtexscale > 0.0f || tex->shrink || tex->useratio || (tex->adjust && (hardext.npot==1 || hardext.npot==2) && !globals4es.potframebuffer)) {
                ...
                tsp_native_gles_teximage2d_v2(GL_TEXTURE_2D, 0, tex->format, tex->nwidth, tex->nheight, 0, tex->format, tex->type, NULL);
 ...
 /*           if ((tex->width<32) || (tex->height<32)) {
            ...
            fb->width  = twidth;
            fb->height = theight;
            tsp_fbo_log("ATTACH_SIZE tex=%u %dx%d", texture, twidth, theight);
    ...
    if (0) /* TSP_FBO_ALWAYS_REATTACH_20260824 - see tag below */
    {
        // no need to reattach
        noerrorShim();
        return;
    ...
    if(attachment==GL_DEPTH_ATTACHMENT /*&& hardext.depthtex==0*/) {
        ...
        if(hardext.depthtex && (tex || !texture)) {
            ...
            if(tex && !tex->renderdepth) {
                ...
                gl4es_glRenderbufferStorage(GL_RENDERBUFFER, tsp_rtt_depth_format(), twidth, theight);
                gl4es_glBindRenderbuffer(GL_RENDERBUFFER, 0);
            }
            tsp_attach_depth_rb(ntarget, tex?tex->renderdepth:0);
        }
        errorGL();
        ReadDraw_Pop(target);
        return;
    ...
    if(attachment==GL_DEPTH_STENCIL_ATTACHMENT /*&& hardext.depthtex==0*/) {
        ...
        if(hardext.depthstencil /*&& !(hardext.depthtex && tex)*/) {
            ...
            if (!tsp_sep_depth()) {
                /* TSP_PACKED_DS_20260823 - one packed buffer, both attachment points.
                   Overrides whatever the branch above attached to depth. */
                if(tex && !tex->renderdepth) {
                    gl4es_glGenRenderbuffers(1, &tex->renderdepth);
                    gl4es_glBindRenderbuffer(GL_RENDERBUFFER, tex->renderdepth);
                    gl4es_glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, twidth, theight);
    ...
    if (level!=0) {
        ...
        if ((scrap_width!=twidth) || (scrap_height!=theight)) {
                ...
                tsp_native_gles_teximage2d_v2(GL_TEXTURE_2D, 0, GL_RGBA, scrap_width, scrap_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
...
void APIENTRY_GL4ES gl4es_glFramebufferRenderbuffer(GLenum target, GLenum attachment, GLenum renderbuffertarget, GLuint renderbuffer) {
    target = tsp_fbnorm(target);
    tsp_fbo_log("ATTACH_RB target=0x%x att=0x%x rb=%u", target, attachment, renderbuffer);
...
void APIENTRY_GL4ES gl4es_glRenderbufferStorage(GLenum target, GLenum internalformat, GLsizei width, GLsizei height) {
    tsp_fbo_log("RB_STORAGE target=0x%x fmt=0x%x %dx%d", target, internalformat, width, height);
    ...
    if ((internalformat == GL_DEPTH_COMPONENT || internalformat == GL_DEPTH_COMPONENT16) && hardext.depth24)
        internalformat = GL_DEPTH_COMPONENT24;
    // in that case, create first a STENCIL one then a DEPTH one....
    if (getenv("LIBGL_TSP_NODOWNGRADE")==NULL && (internalformat == GL_DEPTH24_STENCIL8 && (hardext.depthstencil==0 || ((hardext.vendor&VEND_IMGTEC)==VEND_IMGTEC)))) {
    ...
    if(rend->secondarybuffer) {
        if(use_secondarybuffer) {
            ...
            tsp_native_rb_storage(rend->secondarybuffer, GL_STENCIL_INDEX8, width, height, "secondarybuffer-storage");
...
void createMainFBO(int width, int height) {
    // TSP_GL4ES_NATIVE_MAINFBO_051_V31
    const int tsp_caller_width = width;
    const int tsp_caller_height = height;

    const char* tsp_fbo_env = getenv("LIBGL_FBO");
    int tsp_internal_width = 0;
    int tsp_internal_height = 0;
    ...
    if (tsp_fbo_env != NULL
        && sscanf(
            ...
            "%dx%d",
            &tsp_internal_width,
            &tsp_internal_height) == 2
        && tsp_internal_width > 0
        && tsp_internal_height > 0)
    {
        width = tsp_internal_width;
        height = tsp_internal_height;
    }

    static int tsp_last_width = -1;
    static int tsp_last_height = -1;

    if (width != tsp_last_width || height != tsp_last_height)
    {
        fprintf(
            stderr,
            "TSP_GL4ES_NATIVE_MAINFBO_051_V31 "
            "caller=%dx%d internal=%dx%d\n",
            tsp_caller_width,
            tsp_caller_height,
            width,
            height);
        ...
        tsp_last_width = width;
        tsp_last_height = height;
    }
    ...
    if(createIt)
        ...
        tsp_native_life_texture_generated(glstate->fbo.mainfbo_tex);
    ...
    tsp_native_gles_teximage2d_v2(GL_TEXTURE_2D, 0, globals4es.fbo_noalpha?GL_RGB:GL_RGBA, width, height,
...
void blitMainFBO(int x, int y, int width, int height) {
    ...
    // TSP_GL4ES_NATIVE_PRESENT_051_V31
    const char* tsp_output_env = getenv("LIBGL_TSP_OUTPUT");

    int tsp_output_width = 0;
    int tsp_output_height = 0;

    const int tsp_force_native_output =
        tsp_output_env != NULL
        && sscanf(
            tsp_output_env,
            "%dx%d",
    ...
    if (tsp_force_native_output)
    {
        x = 0;
        y = 0;
        width = tsp_output_width;
        height = tsp_output_height;
    }
    ...
    const int tsp_viewport_width =
        tsp_force_native_output ? tsp_output_width : glstate->fbowidth;

    const int tsp_viewport_height =
        tsp_force_native_output ? tsp_output_height : glstate->fboheight;
    ...
    if (tsp_force_native_output)
    {
        rx = (float)tsp_output_width
            / (float)glstate->fbo.mainfbo_width;

        ry = (float)tsp_output_height
            / (float)glstate->fbo.mainfbo_height;
    ...
    static int tsp_present_logged = 0;

    if (!tsp_present_logged)
    {
        fprintf(
            stderr,
            "TSP_GL4ES_NATIVE_PRESENT_051_V31 "
    ...
    gl4es_blitTexture(
        ...
        glstate->fbo.mainfbo_width,
        glstate->fbo.mainfbo_height,
        glstate->fbo.mainfbo_nwidth,
        glstate->fbo.mainfbo_nheight,
        rx,
        ry,
        0,
        0,
...
void deleteMainFBO(void *state) {
    ...
    if (glstate->fbo.mainfbo_tex) {
        tsp_native_life_texture_deleted(glstate->fbo.mainfbo_tex);
