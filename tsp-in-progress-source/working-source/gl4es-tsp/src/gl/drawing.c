...
void APIENTRY_GL4ES gl4es_glDrawElements(GLenum mode, GLsizei count, GLenum type, const GLvoid *indices) {
    /* TSPCOUNT_gl4es_glDrawElements */ { extern unsigned long tsp_drawcount; extern void tsp_draw_probe(void); tsp_drawcount++; tsp_draw_probe(); }
...
void APIENTRY_GL4ES gl4es_glDrawArrays(GLenum mode, GLint first, GLsizei count) {
    /* TSPCOUNT_gl4es_glDrawArrays */ { extern unsigned long tsp_drawcount; extern void tsp_draw_probe(void); tsp_drawcount++; tsp_draw_probe(); }
        ...
        if (intercept) {
            ...
            if (mode==GL_QUADS) {
                ...
                if((indcnt < realcount) || (indfirst!=realfirst)) {
                    ...
                    indfirst = realfirst;
                    GLushort *p = indices;
                    /* TSP_QUADIDX_FIX: k was incremented twice and j never, so every quad reused
               indfirst..indfirst+3 and half the index buffer was left as
               uninitialised malloc memory -> garbage indices. */
