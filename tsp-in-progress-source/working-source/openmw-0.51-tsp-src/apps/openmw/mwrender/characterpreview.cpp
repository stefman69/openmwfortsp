...
namespace MWRender
{
    ...
    // TSP_RTT_HOLD_V51 - render for N frames instead of one, so the tile resolve has
    // landed before the texture is displayed. See rtt.cpp TSP_RTT_FINISH_V51.
    inline int tspRttHoldFrames()
    {
        static const int n = [] {
            const char* e = std::getenv("TSP_RTT_HOLD_FRAMES");
            int v = (e && e[0]) ? atoi(e) : 3;
            if (v < 1) v = 1;
            if (v > 30) v = 30;
            return v;
        }();
    ...
    public:
        ...
        void redrawNextFrame() { mRendered = false; mTspFramesLeft = tspRttHoldFrames(); }

        unsigned int getLastRenderedFrame() const { return mLastRenderedFrame; }

    private:
        bool mRendered;
        int mTspFramesLeft = tspRttHoldFrames();
