...
namespace MWRender
{
    ...
    {
        // TSP_RTT_HOLD_V51 - keep this one-shot RTT alive a few frames. UserValue rather
        // than a new member so localmap.hpp does not change and the rebuild stays small.
        int tspLeft = 0;
        if (!node->getUserValue("tspHoldFrames", tspLeft))
        {
            const char* e = std::getenv("TSP_RTT_HOLD_FRAMES");
            tspLeft = (e && e[0]) ? atoi(e) : 3;
            if (tspLeft < 1) tspLeft = 1;
            if (tspLeft > 30) tspLeft = 30;
        }
        if (tspLeft > 1)
        {
            node->setUserValue("tspHoldFrames", tspLeft - 1);
            node->mActive = true;
        }
        else
        {
