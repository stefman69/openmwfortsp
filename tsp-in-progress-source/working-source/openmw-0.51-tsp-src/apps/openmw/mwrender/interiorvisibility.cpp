...
namespace MWRender
{
    ...
    void InteriorVisibilityCullCallback::operator()(osg::Node* node, osgUtil::CullVisitor* cv)
    {
        ...
        // TSP_ROOM_RENDER_SAFETY_051_V30_R9 NEAR_GAMEPLAY_BYPASS
        // V27 attaches this callback only to non-static gameplay roots.
        // Scene room/floor residency remains authoritative for existence; this
        // prevents the older screen-depth curtain from hiding local gameplay
        // that is already resident, especially just beyond an opened doorway.
        // TSP_ROOM_DUAL_AUTHORITY_051_V30_R11 FIVE_TIER_XY_HEIGHT_RENDER
        const int tspAdaptiveRangeMode = getInteriorAdaptiveRangeMode();
        const double tspGameplayCullBypass = tspAdaptiveRangeMode >= 4 ? 450.0
            : (tspAdaptiveRangeMode == 3 ? 700.0
            : (tspAdaptiveRangeMode == 2 ? 950.0
