...
namespace MWWorld
{
    ...
    private:
        ...
        std::set<ESM::RefNum> mTspRoomSuppressedRefs;
        CellStore* mTspRoomLifecycleCell = nullptr;
        float mTspRoomLifecycleAccumulator = 0.f;
        std::uint64_t mTspRoomSuppressTotal = 0;
        std::uint64_t mTspRoomWakeTotal = 0;
        float mTspRoomLogAccumulator = 0.f;

        bool tspRoomObjectEligible(const Ptr& ptr) const;
        void tspRoomSuppressObject(const Ptr& ptr);
