...
namespace MWWorld
{
    ...
    void Scene::tspUpdateRoomObjectLifecycle(float duration)
    {
        ...
        int resident = 0;
        int inactive = 0;
        // TSP_ROOM_RENDER_SAFETY_051_V30_R9 SCENE_R7_RANGES
        // TSP_ROOM_RANGE_TUNE_051_V30_R8R2
        // TSP_ROOM_NEARFIELD_CORRECTNESS_051_V30_R7
        // Topology remains the primary culling authority, but nearby ordinary clutter
        // gets a direct player-relative correctness floor. Hysteresis keeps a live
        // object resident farther than the distance required to wake a parked object.
        const osg::Vec3f tspRoomNearFieldPlayer = MWBase::Environment::get().getWorld()
            ->getPlayerPtr().getRefData().getPosition().asVec3();
        int tspRoomNearWakeProtect = 0;
        int tspRoomNearHoldProtect = 0;
        ...
        mCurrentCell->forEach([&](const Ptr& ptr) {
            ...
            // Room/floor authority remains primary for every object. A light that would
            // otherwise be parked is protected only when it is physically near the player.
            // XY gets the requested 3x range; Z is independently capped below floor spacing.
            // TSP_ROOM_DUAL_AUTHORITY_051_V30_R11 FIVE_TIER_XY_HEIGHT_SCENE
            const int tspAdaptiveRangeMode = MWRender::getInteriorAdaptiveRangeMode();
            const int tspAdaptiveHeightMode = MWRender::getInteriorAdaptiveHeightMode();
            const float tspRoomLightNearXY = tspAdaptiveRangeMode >= 4 ? 750.f
                : (tspAdaptiveRangeMode == 3 ? 1200.f
                : (tspAdaptiveRangeMode == 2 ? 1650.f
...
if (!shouldLive && ptr.getType() != ESM::REC_LIGH)
{
    ...
    const bool tspWasParked
        = mTspRoomSuppressedRefs.find(refnum) != mTspRoomSuppressedRefs.end();
    const float tspNearXY = tspWasParked ? tspNearWakeXY : tspNearHoldXY;
    const float tspNearZ = tspWasParked ? tspNearWakeZ : tspNearHoldZ;
    const float tspDx = origin.x() - tspRoomNearFieldPlayer.x();
    const float tspDy = origin.y() - tspRoomNearFieldPlayer.y();
    const float tspDz = origin.z() - tspRoomNearFieldPlayer.z();
    if (tspDx * tspDx + tspDy * tspDy <= tspNearXY * tspNearXY
        && tspDz >= -tspNearZ && tspDz <= tspNearZ)
    {
        shouldLive = true;
        if (tspWasParked)
...
if (shouldLive)
        ...
        if (mTspRoomLogAccumulator >= 1.f)
        {
            mTspRoomLogAccumulator = 0.f;
            Log(Debug::Info)
                << "[TSP_ROOMOBJ_V30] adaptiveXYTier=" << MWRender::getInteriorAdaptiveRangeMode() << " adaptiveZTier=" << MWRender::getInteriorAdaptiveHeightMode() << " pvs=" << (pvsEnabled ? 1 : 0)
                << " eligible=" << eligible
                << " resident=" << resident
                << " inactive=" << inactive
                ...
                << " nearWake=" << tspRoomNearWakeProtect
                << " nearHold=" << tspRoomNearHoldProtect;
        }
    }
