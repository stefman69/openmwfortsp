...
namespace MWGui
{
    ...
    void MapWindow::write(ESM::ESMWriter& writer, Loading::Listener& progress)
    {
        ...
        // TSP_GMAP_DUMP_V1
        if (const char* tspGmapDump = std::getenv("TSP_GMAP_DUMP"))
        {
            std::ofstream tspGmapFile(tspGmapDump, std::ios::binary);
            tspGmapFile.write(reinterpret_cast<const char*>(map.mImageData.data()), map.mImageData.size());
            Log(Debug::Info) << "TSP_GMAP_MEM_V1 phase=dump path=" << tspGmapDump << " bytes=" << map.mImageData.size();
        }
    ...
    // TSP_GMAP_MEM_V1
    static long long tspMapWindowInuseKb()
    {
    ...
    void MapWindow::readRecord(ESM::ESMReader& reader, uint32_t type)
    {
        ...
        {
            // TSP_GMAP_MEM_V1
            const long long tspG0 = tspMapWindowInuseKb();
            ESM::GlobalMap map;
            map.load(reader);
            const long long tspG1 = tspMapWindowInuseKb();

            mGlobalMapRender->read(map);
            const long long tspG2 = tspMapWindowInuseKb();
            ...
            const long long tspG3 = tspMapWindowInuseKb();

            Log(Debug::Info) << "TSP_GMAP_MEM_V1 phase=readRecord esm_load_kb=" << (tspG1 - tspG0)
                             << " globalmap_read_kb=" << (tspG2 - tspG1)
                             << " markers_kb=" << (tspG3 - tspG2)
                             << " png_bytes=" << map.mImageData.size()
                             << " markers=" << map.mMarkers.size();
        }
    }
