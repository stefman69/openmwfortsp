...
namespace MWSound
{
    ...
    void SoundManager::update(float duration)
    {
        ...
        /* TSP_SOUNDPHASE_V1: the sound slot averages 0.07-0.19 ms and spikes to
           a repeatable ~33 ms in 6 of 13 ring captures, producing the worst frame
           in each, verdict "burned real CPU" - so it is decoding on this thread.
           Time all three phases so one run names the culprit. */
        const auto tspT0 = std::chrono::steady_clock::now();
        updateSounds(duration);
        const auto tspT1 = std::chrono::steady_clock::now();
        double tspRegion = 0.0, tspWater = 0.0;
        if (state != MWBase::StateManager::State_NoGame)
        {
            const auto tspT2 = std::chrono::steady_clock::now();
            updateRegionSound(duration);
            const auto tspT3 = std::chrono::steady_clock::now();
            updateWaterSound();
            const auto tspT4 = std::chrono::steady_clock::now();
            tspRegion = std::chrono::duration<double, std::milli>(tspT3 - tspT2).count();
            tspWater = std::chrono::duration<double, std::milli>(tspT4 - tspT3).count();
        }
        const double tspSounds = std::chrono::duration<double, std::milli>(tspT1 - tspT0).count();
        const double tspTotal = tspSounds + tspRegion + tspWater;
        static int tspSndLines = 0;
        if (tspTotal >= 15.0 && tspSndLines < 200)
        {
            ++tspSndLines;
            Log(Debug::Info) << "TSP_SOUNDPHASE_V1 total=" << tspTotal
                             << " updateSounds=" << tspSounds
                             << " regionSound=" << tspRegion
                             << " waterSound=" << tspWater
                             << " active=" << mActiveSounds.size()
                             << " music=" << (isMusicPlaying() ? 1 : 0);
        }
    }
