...
namespace Shader
{
    ...
    void ShaderVisitor::createProgram(const ShaderRequirements& reqs)
    {
        ...
        /* TSP_NO_ENVMAP: environment mapping is one of only six genuinely
           distinct objects shader variants on this device, and each variant
           costs Mali ~90ms to link plus ~240ms on its first draw. The shiny
           metal/glass reflection it produces is barely visible at handheld
           size, so trading it for ~380ms of load stall is worth testing.
           Set TSP_KEEP_ENVMAP=1 to restore it without rebuilding. */
        if (!std::getenv("TSP_KEEP_ENVMAP") && defineMap["envMap"] != "0")
        {
            defineMap["envMap"] = "0";
            defineMap["envMapUV"] = "0";
        }
