#include <cstdlib>    // TSP_LIGHTSIMPLE_V3 (getenv)
...
namespace SceneUtil
{
        ...
        const LightList& lightList, size_t frameNum, const osg::RefMatrix* viewMatrix)
    {
        if (getLightingMethod() == LightingMethod::PerObjectUniform)
        {
            /* TSP_LIGHTSIMPLE_V3 - one light stateset for the whole view.
               Upstream returns a freshly allocated StateSet here for EVERY lit
               drawable EVERY frame; this early return sits above mStateSetCache
               so that cache is unreachable on the only lighting path this build
               uses (settings.cfg says "legacy", which is not a valid value here,
               so line 636 falls back to PerObjectUniform).
               Measured hit rates on device, Balmora Mages Guild:
                 mode 1, truncate to nearest-1 per object -> 0.63. The list is
                   sorted nearest-to-THAT-object, so each object's first light is
                   usually a different light: ~one miss per drawable, plus a copy
                   and a hash per call. Measurably worse than upstream.
                 mode 2, no point lights at all             -> 0.98, 16-17 to
                   19-20 fps. The win came from every drawable sharing ONE key,
                   not from lighting being expensive.
               Mode 3 keeps that sharing and keeps the lights: pick one light
               list for the entire view, anchored on the light nearest the
               camera, and hand the same StateSet to every drawable. Falloff
               becomes global rather than per-object - the PS1 model.
               Anchor is chosen greedily within a generation: if a nearer light
               shows up later in the traversal we regenerate once and use that
               from then on, so the anchor converges on the nearest light in view
               after a handful of misses instead of depending on cull order.
               TSP_LIGHT_SIMPLE=0 upstream  1 per-object truncate (do not use)
                                2 ambient only (ceiling test)  3 global list
               TSP_LIGHT_SIMPLE_N = lights in the global list, default 2.
               Safe to share: fade is applied once per light per frame under the
               getLastAppliedFrame() guard and measured light-to-camera, not
               light-to-drawable; LightSourceViewBound holds no per-object data.
               generate() bakes mViewMatrix into light positions, so the view
               matrix is part of the generation key; it changes once per render
               stage (main, water reflection, water refraction, shadows).
               Function-local statics, not members: the header has many
               consumers and this keeps the rebuild to one translation unit.
               Unguarded, like the cache below, sound under
               OSG_THREADING=SingleThreaded (launcher line 61).
               Log(Debug::Warning), never std::cerr - OpenMW swaps cerr's stream
               buffer at startup and the line vanishes. Goes to openmw.log. */
            static int sTspMode = -1;
            static size_t sTspN = 2;
            ...
            static osg::ref_ptr<osg::StateSet> sTspGlobalSs;
            static float sTspAnchor = -1.0f;
            if (sTspMode < 0)
            {
                ...
                Log(Debug::Warning) << "TSP_LIGHTSIMPLE_V3 mode=" << sTspMode << " n=" << sTspN
                                    << " (0=upstream 1=per-object 2=ambient-only 3=global)";
            }
            if (sTspMode == 0)
            {
            ...
            {
                sTspCache.clear();
                sTspGlobalSs = nullptr;
                sTspAnchor = -1.0f;
                sTspFrame = frameNum;
                sTspView = *viewMatrix;
            }
            if (++sTspHit + sTspMiss > 0 && (sTspHit % 20000) == 0)
                Log(Debug::Warning) << "TSP_LIGHTSIMPLE_V3 hit=" << sTspHit << " miss=" << sTspMiss
                                    << " ratio=" << (double)sTspHit / (double)(sTspHit + sTspMiss);
            if (sTspMode >= 3)
            {
                float tspD = lightList.empty() ? -1.0f
                    : lightList[0]->mViewBound.center().length();
                bool tspBetter = !sTspGlobalSs
                    || (tspD >= 0.0f && (sTspAnchor < 0.0f || tspD < sTspAnchor));
                if (tspBetter)
                {
                    --sTspHit;
                    ++sTspMiss;
                    LightList tspG = lightList;
                    if (tspG.size() > sTspN)
                        tspG.resize(sTspN);
                    mStateSetGenerator->mViewMatrix = *viewMatrix;
                    sTspGlobalSs = mStateSetGenerator->generate(tspG, frameNum);
                    sTspAnchor = tspD;
                }
                return sTspGlobalSs;
            }
