...
namespace Gui
{
    ...
    void MWList::addItem(std::string_view name, int verticalPadding, int fontHeight)
    {
        mItems.emplace_back(name, verticalPadding, fontHeight);
    }
    ...
    void MWList::redraw(bool scrollbarShown)
    {
        ...
        for (const auto& item : mItems)
        {
            ...
            if (!item.mName.empty())
            {
                ...
                // TSP_MWLIST_PER_ITEM_FONT_051_20260813
                if (item.mFontHeight > 0)
                    button->setFontHeight(item.mFontHeight);
    ...
    void MWList::ensureItemVisible(MyGUI::Widget* item)
    {
        if (item == nullptr || mScrollView == nullptr)
            return;

        // TSP_JOURNAL_SCROLL_V3
        // Use actual rendered geometry rather than assuming a fixed
        // number of quest/topic rows fit on screen.
        const int viewHeight = mScrollView->getSize().height;
        const int currentViewTop = -mScrollView->getViewOffset().top;

        const int itemTop = item->getPosition().top;
        const int itemBottom = itemTop + item->getSize().height;

        int newViewTop = currentViewTop;

        if (itemTop < currentViewTop)
            newViewTop = itemTop;
        else if (itemBottom > currentViewTop + viewHeight)
            newViewTop = itemBottom - viewHeight;

        const int maxViewTop = std::max(0, mScrollView->getCanvasSize().height - viewHeight);

        if (newViewTop < 0)
            newViewTop = 0;
        if (newViewTop > maxViewTop)
            newViewTop = maxViewTop;

        mScrollView->setViewOffset(MyGUI::IntPoint(0, -newViewTop));
    }
}
