...
OUTPUT_BINARY="${3:-/root/openmw_cursor_graphics_compat_v10}"
...
echo "============================================================"
echo "OpenMW 0.48 TSP cursor + GL4ES compatibility patch V10"
...
CPP_BACKUP="$CPP.before-tsp-compat-v10.$STAMP"
HPP_BACKUP="$HPP.before-tsp-compat-v10.$STAMP"
WATER_BACKUP="$WATER_CPP.before-tsp-compat-v10.$STAMP"
GROUNDCOVER_BACKUP="$GROUNDCOVER_CPP.before-tsp-compat-v10.$STAMP"
OBJECTPAGING_BACKUP="$OBJECTPAGING_CPP.before-tsp-compat-v10.$STAMP"
...
new_update = r'''    // TSP_MYGUI_CURSOR_V10
...
if depth_callback_line in water:
    water = water.replace(
        depth_callback_line,
        "    // TSP_GL4ES_WATER_V10: depth clamp disabled for gl4es/GLES2.",
        1,
    )
elif "TSP_GL4ES_WATER_V10: depth clamp disabled" not in water:
...
if "TSP_GL4ES_GROUNDCOVER_V10: use normal transforms" not in groundcover:
    ...
    marker = "        // TSP_GL4ES_GROUNDCOVER_V10: use normal transforms, not instancing.\\n"
    brace = ctor.find("{")
    ctor = ctor[:brace + 1] + "\\n" + marker + ctor[brace + 1:]
...
grep -n "TSP_MYGUI_CURSOR_V10" "$CPP" "$HPP"
grep -n "SDL_SCANCODE_PAUSE" "$CPP"
grep -n "mTspCursorVisible" "$CPP" "$HPP"
grep -n "TSP_GL4ES_WATER_V10" "$WATER_CPP"
grep -n "TSP_GL4ES_GROUNDCOVER_V10" "$GROUNDCOVER_CPP"
