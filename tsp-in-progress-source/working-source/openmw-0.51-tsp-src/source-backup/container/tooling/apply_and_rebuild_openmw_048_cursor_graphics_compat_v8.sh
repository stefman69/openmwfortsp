...
OUTPUT_BINARY="${3:-/root/openmw_cursor_graphics_compat_v8}"
...
echo "============================================================"
echo "OpenMW 0.48 TSP cursor + GL4ES compatibility patch V8"
...
CPP_BACKUP="$CPP.before-tsp-compat-v8.$STAMP"
HPP_BACKUP="$HPP.before-tsp-compat-v8.$STAMP"
WATER_BACKUP="$WATER_CPP.before-tsp-compat-v8.$STAMP"
GROUNDCOVER_BACKUP="$GROUNDCOVER_CPP.before-tsp-compat-v8.$STAMP"
OBJECTPAGING_BACKUP="$OBJECTPAGING_CPP.before-tsp-compat-v8.$STAMP"
...
new_update = r'''    // TSP_MYGUI_CURSOR_V8
...
if depth_callback_line in water:
    water = water.replace(
        depth_callback_line,
        "    // TSP_GL4ES_WATER_V8: depth clamp disabled for gl4es/GLES2.",
        1,
    )
elif "TSP_GL4ES_WATER_V8: depth clamp disabled" not in water:
...
constructor_lines = '''         mStateset->setAttribute(new osg::VertexAttribDivisor(6, 1));
...
if constructor_lines in groundcover:
    groundcover = groundcover.replace(
        constructor_lines,
        "         // TSP_GL4ES_GROUNDCOVER_V8: use normal transforms, not instancing.",
        1,
    )
elif "TSP_GL4ES_GROUNDCOVER_V8: use normal transforms" not in groundcover:
    raise RuntimeError(
        "Could not locate the groundcover instancing constructor block."
    )

new_groundcover_chunk = r'''    osg::ref_ptr<osg::Node> Groundcover::createChunk(
        InstanceMap& instances, const osg::Vec2f& center)
    {
        // TSP_GL4ES_GROUNDCOVER_V8
...
if merge_setting in objectpaging:
    objectpaging = objectpaging.replace(
        merge_setting,
        "         // TSP_GL4ES_OBJECT_PAGING_V8\n"
        "         mMergeFactor = 0.f;",
        1,
    )
elif "TSP_GL4ES_OBJECT_PAGING_V8" not in objectpaging:
    raise RuntimeError(
...
grep -n "TSP_MYGUI_CURSOR_V8" "$CPP" "$HPP"
grep -n "SDL_SCANCODE_PAUSE" "$CPP"
grep -n "mTspCursorVisible" "$CPP" "$HPP"
grep -n "TSP_GL4ES_WATER_V8" "$WATER_CPP"
grep -n "TSP_GL4ES_GROUNDCOVER_V8" "$GROUNDCOVER_CPP"
grep -n "TSP_GL4ES_OBJECT_PAGING_V8" "$OBJECTPAGING_CPP"
