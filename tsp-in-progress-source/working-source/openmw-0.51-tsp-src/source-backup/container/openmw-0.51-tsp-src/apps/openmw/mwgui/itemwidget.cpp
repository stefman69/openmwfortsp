...
namespace MWGui
{
    ...
    void ItemWidget::setCount(int count)
    {
        if (!mText)
            return;
        // TSP_FONT_SIZES_051_V55 -- this never set a height at all, so stack counts
        // inherited the global font size (28 on this device).
        mText->setProperty(
            "FontHeight", std::to_string(static_cast<int>(Settings::gui().mTspItemCountFontSize)));
