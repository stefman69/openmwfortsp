    ...
    : Layout(parLayout)
{
    ...
    // TSP_FONT_SIZES_051_V55 -- the caption on every window that has one, in a
    // single place. Same getSkinWidgetsByName call the "Action" lookup
    // below already uses; "Caption" is the name the skin gives it
    // (openmw_windows.skin.xml:581 and :852). setProperty rather than a
    // cast to TextBox so no new include is needed -- MyGUI logs a warning
    // instead of throwing if a widget cannot take the property.
    for (MyGUI::Widget* tspCaption : window->getSkinWidgetsByName("Caption"))
        tspCaption->setProperty(
            "FontHeight", std::to_string(static_cast<int>(Settings::gui().mTspTitleFontSize)));
