...
# OpenMW 0.51.0 TrimUI Smart Pro input/cursor compatibility rebuild.
#
...
#
# This INPUT-ONLY version intentionally does NOT modify renderer/water source.

SOURCE_DIR="${1:-/root/openmw-0.51-tsp-src}"
BUILD_DIR="${2:-/root/openmw-0.51-tsp-build}"
PACKAGE_DIR="${3:-/root/openmw-0.51-tsp-package}"
OUTPUT_BINARY="${4:-/root/openmw-0.51-tsp-input-v1}"
...
SCRIPT_REVISION="TSP-051-INPUT-HYBRID-V1-2026-08-07"
STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP_DIR="$SOURCE_DIR/.tsp-051-source-backups/input-hybrid-$STAMP"
...
restore_on_patch_error() {
    ...
    if [ "$result" -ne 0 ] && [ -d "$BACKUP_DIR" ]; then
        ...
        echo "Input source restoration completed."
...
echo "============================================================"
echo "OpenMW 0.51 TSP input + hybrid mouse/text rebuild"
...
echo "Created input source backup: $BACKUP_DIR"
...
if [ "$PATCH_ONLY" = "1" ]; then
    ...
    echo "============================================================"
    echo "OpenMW input source was patched and helper source was generated."
...
[ ! -e "$PACKAGE_BINARY" ] || cp -f "$PACKAGE_BINARY" "$PACKAGE_BINARY.before-input-v1-$STAMP"
...
echo
echo "============================================================"
echo "OpenMW 0.51 INPUT rebuild completed"
echo "============================================================"
