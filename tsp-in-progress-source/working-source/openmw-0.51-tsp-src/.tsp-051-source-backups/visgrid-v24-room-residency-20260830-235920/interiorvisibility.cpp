...
namespace MWRender
{
    namespace
    {
        ...
        bool pvsOriginOverlapsSector(const osg::Vec3f& origin, float radius, int sector)
        {
            ...
            const float xy = std::max(0.f, radius)
    ...
    void InteriorVisibilityCullCallback::operator()(osg::Node* node, osgUtil::CullVisitor* cv)
    {
        ...
        // TSP_INTERIOR_VISGRID_051_V5_TOPO_PVS
        // Only conservative, stationary ESM::Static roots are eligible.
        ...
        if (pvsEnabled && mPvsEligible && std::isfinite(mPvsRadius) && mPvsRadius > 0.f)
        {
            ...
            if (sectorCount > 0 && sectorCount <= sPvsMaxSectors && visibleMask != 0)
            {
                ...
                {
                    if (!pvsOriginOverlapsSector(mWorldOrigin, mPvsRadius, sector))
                        continue;
