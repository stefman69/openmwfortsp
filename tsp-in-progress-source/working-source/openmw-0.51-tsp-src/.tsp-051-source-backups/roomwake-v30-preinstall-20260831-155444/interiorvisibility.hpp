...
namespace MWRender
{
    ...
    // Main-thread room residency query for scene materialization. Unmapped objects
    // and disabled/invalid PVS fail open (resident=true).
    bool isInteriorTopologyPvsEnabled();
    bool isInteriorTopologyObjectResident(
        const osg::Vec3f& origin, float radius, bool structural = false);

    // TSP_ROOM_OBJECT_SEPARATE_CLUTTER_051_V28
    // Independent from structural/fog topology PVS. Lua publishes only the
    // clutter rooms that should be physically materialized in the scene.
