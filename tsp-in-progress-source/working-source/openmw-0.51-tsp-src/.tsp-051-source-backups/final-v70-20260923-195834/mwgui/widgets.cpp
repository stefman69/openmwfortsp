...
namespace MWGui::Widgets
{
    ...
    void MWSpell::setNameFontHeight(int height)
    {
        if (mSpellNameWidget != nullptr)
    ...
    void MWSpellEffect::initialiseOverride()
    {
        ...
        // TSP_EFFECT_ROW_READABILITY_051_V35_3
        //
        // Global UI font remains 28px, but magic-effect rows in
        // tooltips/spell details are compact fixed-height widgets.
        // Give them a slightly smaller dedicated font and enough
        // physical height so descenders/second lines are not clipped.
        constexpr int tspEffectFontHeight = 24;
        constexpr int tspEffectRowHeight = 30;

        setSize(getWidth(), std::max(getHeight(), tspEffectRowHeight));

        if (mTextWidget != nullptr)
        {
            mTextWidget->setFontHeight(tspEffectFontHeight);
            mTextWidget->setSize(
                mTextWidget->getWidth(),
                std::max(
                    mTextWidget->getHeight(),
                    tspEffectRowHeight));
        }
    }
