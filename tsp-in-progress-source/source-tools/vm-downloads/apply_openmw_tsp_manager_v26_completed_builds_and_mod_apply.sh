...
WORK="$HOST_DIR/openmw-tsp-manager-v2.6"
...
BUILD_LOG="$WORK/openmw-manager-v26-build-$STAMP.log"
STATE="$HOST_DIR/openmw-manager-v26-install.state"
...
static void header(Framebuffer&f){f.rect(0,0,f.width(),68,{9,10,12});label(f,"OPENMW 0.51",28,18,3,ACC);label(f,"TRIMUI SMART PRO MANAGER",270,25,2,FG);label(f,"INTEGRATED V2.6",1010,25,2,DIM);f.rect(0,68,f.width(),2,ACC);}
...
static void modsPage(Framebuffer&f,App&a)
{
    ...
    label(f,"X RESCAN + AUDIT   START APPLY   CONFIG IS BACKED UP TRANSACTIONALLY",376,622,1,DIM);
}
...
host_selftest() {
    ...
    grep -Fq 'INTEGRATED V2.6' "$CPP" || fail 16 "V2.6 UI marker missing"
...
collect_action() {
    ensure_ssh
    local out="$HOST_DIR/openmw-manager-v26-diagnostics-$STAMP.tar.gz"
...
exec > >(tee "$WORK/openmw-manager-v26-install-$STAMP.log") 2>&1

cat <<'BANNER'
============================================================
OPENMW 0.51 TSP — MANAGER V2.6
COMPLETED BUILDS PASS + NO DB BACKUPS + REAL MOD APPLY STATE
============================================================
...
DEVICE_BACKUP="$ROOT/launcher/install-backups/manager-v26-before-$STAMP"
...
sync
echo "PASS Manager V2.6 installed and self-tested on device"
