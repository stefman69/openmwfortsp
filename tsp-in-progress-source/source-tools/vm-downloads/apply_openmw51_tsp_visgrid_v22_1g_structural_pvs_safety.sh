...
EXPECTED_BASE_SHA="8b2db68de598febff9559cb248e509a99d8c3abdda29c54b42447f6970b0f7fc"
EXPECTED_NEW_SHA="cd422b72de855afd1853267e321ee86da426bee7c6c78293cee958f1c7645ebb"
...
OUT="$HOME/Downloads/openmw51-visgrid-v22-1g-$STAMP"
PKG="$OUT/pkg"
...
sha64() { [[ "$1" =~ ^[0-9a-f]{64}$ ]]; }
sha64 "$EXPECTED_BASE_SHA" || fail "internal base SHA is malformed"
sha64 "$EXPECTED_NEW_SHA" || fail "internal output SHA is malformed"
sha64 "$EXPECTED_GAME_SHA" || fail "internal game SHA is malformed"
sha64 "$EXPECTED_TOPO_SHA" || fail "internal topology SHA is malformed"
sha64 "$EXPECTED_DOOR_SHA" || fail "internal doorgraph SHA is malformed"
...
on_error() {
    ...
    echo
    echo "=================================================================="
    echo "V22.1G CONTROLLER STOPPED (rc=$rc)"
    echo "=================================================================="
    if [[ "$ROLLBACK_READY" == "1" && -n "$REMOTE_BACKUP" ]]; then
        echo "Attempting automatic sensor rollback..."
        if ssh -o BatchMode=yes -o ConnectTimeout=8 "$DEV" "
            set -e
        ...
        "; then
            echo "PASS: exact V22.1F sensor restored."
        else
            echo "WARNING: automatic rollback could not be verified."
        fi
    else
...
echo "=================================================================="
echo "OPENMW 0.51 — VISGRID V22.1G STRUCTURAL PVS SAFETY"
echo "=================================================================="
echo "Exact base required: V22.1F $EXPECTED_BASE_SHA"
echo "Exact output:        V22.1G $EXPECTED_NEW_SHA"
echo
echo "Purpose:"
echo "  - fix the deeper structural blue-hole failure, not merely repair it with rays"
echo "  - restore V17's whole-current-floor PVS safety invariant"
echo "  - keep a ray-independent 3-D structural shell around the player across floor labels"
echo "  - when local navmesh segmentation is too dense to safely own render meshes,"
echo "    fail ONLY topology PVS open; ordinary VISGRID depth culling remains active"
echo "  - widen static-root/sector overlap padding so walls/arches/rails that straddle"
echo "    sector boundaries inherit the active structural side"
echo "  - preserve V22.1F revisit trail, ray-PVS authority, V22.1E door halo,"
echo "    intercell entry handling, topology, door graph, binary, and navmesh DB"
echo
echo "This modifies visgrid.lua only."
echo "Protected game/topology/doorgraph/DB state is snapshotted before install"
echo "and required to be byte/state-identical afterward."
echo "No OpenMW process-name gate is used."
echo "=================================================================="

echo
echo "===== 1/6 VERIFY EXACT LIVE V22.1F + SNAPSHOT PROTECTED STATE ====="
ssh -o BatchMode=yes -o ConnectTimeout=8 "$DEV" "
    ...
    grep -Fq 'TSP_VISGRID_LUA_V22_1F_REVISIT_TRAIL' '$LIVE_LUA'
    grep -Fq 'TSP_VISGRID_LUA_V22_1F_RAY_PVS_AUTHORITY' '$LIVE_LUA'
    grep -Fq 'TSP_VISGRID_GLOBAL_TOPOLOGY_V1' '$TOPOLOGY'
    grep -Fq 'TSP_VISGRID_REAL_DOOR_GRAPH_V1' '$DOORGRAPH'
"

BASE_SHA="$(ssh "$DEV" "sha256sum '$LIVE_LUA'" | awk 'NF {print $1; exit}' | tr -d '
')"
if [[ "$BASE_SHA" == "$EXPECTED_NEW_SHA" ]]; then
    ssh -o BatchMode=yes -o ConnectTimeout=8 "$DEV"         "grep -Fq 'TSP_VISGRID_LUA_V22_1G_STRUCTURAL_PVS_SAFETY' '$LIVE_LUA'"
    echo "PASS: exact V22.1G is already installed."
    ROLLBACK_READY=0
else
    assert_eq "exact V22.1F sensor SHA" "$BASE_SHA" "$EXPECTED_BASE_SHA"
fi

GAME_SHA_BEFORE="$(ssh "$DEV" "sha256sum '$GAME'" | awk 'NF {print $1; exit}' | tr -d '
')"
TOPO_SHA_BEFORE="$(ssh "$DEV" "sha256sum '$TOPOLOGY'" | awk 'NF {print $1; exit}' | tr -d '
')"
DOOR_SHA_BEFORE="$(ssh "$DEV" "sha256sum '$DOORGRAPH'" | awk 'NF {print $1; exit}' | tr -d '
')"
DB_STAT_BEFORE="$(ssh "$DEV" "stat -c '%s %Y' '$DB'" | awk 'NF>=2 {print $1, $2; exit}' | tr -d '
')"
TOPO_COUNT_BEFORE="$(ssh "$DEV" "find '$TOPOLOGY_CELLS' -maxdepth 1 -type f -name 'c_*.lua' | wc -l" | awk 'NF {print $1; exit}' | tr -d '
')"
DOOR_COUNT_BEFORE="$(ssh "$DEV" "find '$DOORGRAPH_CELLS' -maxdepth 1 -type f -name 'c_*.lua' | wc -l" | awk 'NF {print $1; exit}' | tr -d '
')"
...
' "$DB_STAT_BEFORE"
printf 'topology shards:  %s
' "$TOPO_COUNT_BEFORE"
printf 'doorgraph shards: %s
' "$DOOR_COUNT_BEFORE"
...
assert_eq "canonical navmesh DB size/mtime" "$DB_STAT_BEFORE" "$EXPECTED_DB_STAT"
...
if [[ "$BASE_SHA" == "$EXPECTED_NEW_SHA" ]]; then
    echo "Skipping reinstall; proceeding to helper generation."
else
    echo
    echo "===== 2/6 MATERIALIZE + VALIDATE EXACT V22.1G ====="
    cat > "$PKG/visgrid-v22.1g.lua" <<'EOF_V221G_SENSOR'
...
-- TSP_VISGRID_LUA_V22_1F_REVISIT_TRAIL  (recently-traversed path memory + directional depth preload)
-- TSP_VISGRID_LUA_V22_1F_RAY_PVS_AUTHORITY  (positive-ray sectors bypass bounded resident rejection)
-- TSP_VISGRID_LUA_V22_1G_STRUCTURAL_PVS_SAFETY  (whole-floor safety + cross-floor shell + ambiguity fail-open)
...
local mapState = {
    ...
    -- V22.1F recently-traversed path memory. Entry-near preload is deliberately
    -- short-lived, but a place the player actually walked through is stronger
    -- evidence than a speculative arrival bubble. Keep a bounded trail of eye
    -- positions and their topology sectors. When the camera faces that trail
    -- again, pre-open only the matching angular bins and keep those proven
    -- sectors structural. This makes a quick turn-around/revisit fail open
    -- without globally widening tight rooms.
    revisitTrail = {}, revisitTrailDepthBins = {}, revisitTrailSectors = {},
    revisitTrailCount = 0, revisitTrailAdds = 0, revisitTrailDepthTouches = 0,
    revisitTrailPvsTouches = 0, revisitTrailSectorCount = 0,
    revisitTrailLastX = nil, revisitTrailLastY = nil, revisitTrailLastZ = nil,
    revisitTrailLastTime = 0.0, revisitTrailSignature = '',

    -- V22.1F correctness authority for complex interiors. The V22 resident
    -- set is intentionally tiny, but a sector that a CURRENT physics ray
    -- proves lies on the visible line may never disappear merely because
    -- all 3-5 resident slots are still inside their minimum hold. Keep a
    -- short LRU/TTL set of ray-proven sectors outside that bounded cache.
    rayPvsUntil = {}, rayPvsCount = 0, rayPvsTouches = 0,
    rayPvsAdds = 0, rayPvsExpires = 0, rayPvsBypasses = 0,
    rayPvsDrops = 0, rayPvsChanges = 0, rayPvsSignature = '',
    rayPvsDebugNext = 0.0,

    -- V22.1G structural correctness layer. Navmesh sectors are navigation
    -- partitions, not render-room ownership. Keep the entire current topology
    -- floor structural (the original V17 invariant), plus a physical 3-D shell
    -- around the player regardless of floor label. If that local shell becomes
    -- so dense that sector ownership is ambiguous, fail only topology PVS open;
    -- the ordinary VISGRID depth curtain remains active.
    pvsStructNearCount = 0, pvsStructFloorCount = 0,
    pvsStructSignature = '', pvsStructUnsafe = false,
    pvsStructUnsafeReason = '', pvsStructFailOpenActivations = 0,
    pvsStructFailOpenUpdates = 0, pvsStructWholeFloorCount = 0,
    ...
    -- V22.1F revisit memory. Sample the path sparsely enough to be cheap but
    -- densely enough to follow stairs/corners. The trail is cell-local and
    -- expires quickly, so this cannot turn into a whole-dungeon keepalive.
    v221fTrailHold = 20.0,
    v221fTrailSampleDist = 120.0,
    v221fTrailSampleTime = 0.55,
    v221fTrailMax = 48,
    v221fTrailMinRange = 100.0,
    v221fTrailMaxRange = 3200.0,
    v221fTrailDepthPad = 900.0,

    -- Positive-ray structural authority. This is deliberately separate from
    -- v22ResidentTarget/v22ResidentHardMax: those are a performance cache,
    -- never permission to reject geometry the live collision view proves visible.
    v221fRayPvsHold = 2.00,
    v221fRayPvsMax = 12,
    v221fRayPvsMinDepth = 320.0,
    -- V22.1E only applied its learn-first gate to interior->interior changes.

    -- V22.1G structural safety. These are not ray/preload ranges: they define
    -- the minimum local static shell topology PVS is never allowed to remove.
    -- Large/open and stair sectors are intentionally wider because visually
    -- continuous atria are frequently split across several navmesh floors.
    v221gPvsNearRoomRange = 1250.0,
    v221gPvsNearOpenRange = 3400.0,
    v221gPvsNearConnectorRange = 2600.0,
    v221gPvsUnsafeRoomNear = 18,
    v221gPvsUnsafeOpenNear = 26,
    v221gPvsUnsafeConnectorNear = 24,
    v221gPvsUnsafeRatio = 0.86,

    -- Render roots near navmesh boundaries need generous overlap. The C++ PVS
    -- still culls only immutable ESM::Static roots; this padding merely makes
    -- boundary-spanning walls/arches/rails overlap the structural side too.
    v17PvsXyPad = 360.0,
    v17PvsZPad = 260.0,
}
do
...
local function resetRuntimeState()
    ...
    mapState.revisitTrail = {}
    mapState.revisitTrailDepthBins = {}
    mapState.revisitTrailSectors = {}
    mapState.revisitTrailCount = 0
    mapState.revisitTrailAdds = 0
    mapState.revisitTrailDepthTouches = 0
    mapState.revisitTrailPvsTouches = 0
    mapState.revisitTrailSectorCount = 0
    mapState.revisitTrailLastX = nil
    mapState.revisitTrailLastY = nil
    mapState.revisitTrailLastZ = nil
    mapState.revisitTrailLastTime = 0.0
    mapState.revisitTrailSignature = ''
    mapState.rayPvsUntil = {}
    mapState.rayPvsCount = 0
    mapState.rayPvsTouches = 0
    mapState.rayPvsAdds = 0
    mapState.rayPvsExpires = 0
    mapState.rayPvsBypasses = 0
    mapState.rayPvsDrops = 0
    mapState.rayPvsChanges = 0
    mapState.rayPvsSignature = ''
    mapState.rayPvsDebugNext = 0.0
    mapState.pvsStructNearCount = 0
    mapState.pvsStructFloorCount = 0
    mapState.pvsStructWholeFloorCount = 0
    mapState.pvsStructSignature = ''
    mapState.pvsStructUnsafe = false
    mapState.pvsStructUnsafeReason = ''
    mapState.pvsStructFailOpenActivations = 0
    mapState.pvsStructFailOpenUpdates = 0
        ...
        for col = 1, COLS do
            ...
            if not okDir or dirOrErr == nil then
                ...
                if len <= 0.0001 then
                    ...
                    -- V22.1F: physical traversal outranks later shallow ray
                    -- convergence. Only bins pointing back along the recently
                    -- walked path receive this floor; unrelated walls remain tight.
                    local revisitDepth = mapState.revisitDepthForBin(idx)
                    if revisitDepth ~= nil and revisitDepth > depth then
                        depth = revisitDepth
                    end
                end
            end
...
mapState.seedIntercellEntryNeighborhood = function(ex, ey, ez)
    ...
    mapState.intercellEntryNearCount = 0
    mapState.intercellEntryNearSignature = ''
    if mapState.topoCell == nil then return end
    ...
    print(string.format(
        ...
        tostring(mapState.intercellDest or lastCellName or '?'), #ids,
        mapState.intercellEntryNearSignature, mapState.v221eEntryNearRange,
        mapState.v221eEntryNearHold))
end

-- V22.1F: remember where the player actually walked. Unlike the V22 resident
-- room set, this is not inferred from rays and cannot be evicted just because
-- three newer candidates appeared. It is a short, bounded, cell-local trail.
mapState.updateRevisitTrail = function(ex, ey, ez, moveDist)
    local now = interiorElapsed
    local trail = mapState.revisitTrail or {}
    local kept = {}
    for i = 1, #trail do
        local p = trail[i]
        if p ~= nil and (p.untilTime or 0.0) > now then
            kept[#kept + 1] = p
        end
    end
    trail = kept
    ...
    local add = false
    if mapState.revisitTrailLastX == nil then
        add = true
    else
        local dx = ex - mapState.revisitTrailLastX
        local dy = ey - mapState.revisitTrailLastY
        local dz = ez - mapState.revisitTrailLastZ
        local d = sqrt(dx*dx + dy*dy + dz*dz)
        local newest = trail[#trail]
        if d >= mapState.v221fTrailSampleDist
            or ((moveDist or 0.0) > 1.0
                and now - (mapState.revisitTrailLastTime or 0.0)
                    >= mapState.v221fTrailSampleTime)
            or (newest ~= nil and sid > 0 and newest.sid ~= sid) then
            add = true
        end
    end
    ...
    if add then
        trail[#trail + 1] = {
            x = ex, y = ey, z = ez, sid = sid,
            untilTime = now + mapState.v221fTrailHold,
        }
        while #trail > mapState.v221fTrailMax do table.remove(trail, 1) end
        mapState.revisitTrailLastX = ex
        mapState.revisitTrailLastY = ey
        mapState.revisitTrailLastZ = ez
        mapState.revisitTrailLastTime = now
        mapState.revisitTrailAdds = (mapState.revisitTrailAdds or 0) + 1
    end
    ...
    for i = 1, #trail do
        local p = trail[i]
        local psid = tonumber(p.sid or 0) or 0
        if psid > 0 then sectors[psid] = true end
        local dx, dy, dz = p.x-ex, p.y-ey, p.z-ez
        local dist = sqrt(dx*dx + dy*dy + dz*dz)
        if dist >= mapState.v221fTrailMinRange
            and dist <= mapState.v221fTrailMaxRange then
            local idx = binOfDir(dx, dy, dz, dist)
            local by = (idx - 1) % YAW_BINS
            local bp = floor((idx - 1) / YAW_BINS)
            local ys = dist < 900.0 and 3 or (dist < 1800.0 and 2 or 1)
            local ps = dist < 1200.0 and 2 or 1
            local floorDepth = min(OPEN_DEPTH, dist + mapState.v221fTrailDepthPad)
            for dp = -ps, ps do
                local pp = bp + dp
                if pp >= 0 and pp < PITCH_BINS then
                    for dyw = -ys, ys do
                        local j = pp * YAW_BINS + ((by + dyw) % YAW_BINS) + 1
                        if bins[j] == nil then binCount = binCount + 1 end
                        if bins[j] == nil or bins[j] < floorDepth then
                            bins[j] = floorDepth
                        end
                    end
                end
    ...
    mapState.revisitTrail = trail
    mapState.revisitTrailCount = #trail
    mapState.revisitTrailDepthBins = bins
    mapState.revisitTrailBinCount = binCount
    mapState.revisitTrailSectors = sectors

    local ids = {}
    for psid in pairs(sectors) do ids[#ids + 1] = psid end
    table.sort(ids)
    mapState.revisitTrailSectorCount = #ids
    local signature = table.concat(ids, ',')
    if signature ~= (mapState.revisitTrailSignature or '') then
        mapState.revisitTrailSignature = signature
        print(string.format(
            '[TSP_VISGRID_V22.1F] REVISIT-TRAIL points=%d sectors=%d ids=%s bins=%d hold=%.1f',
            #trail, #ids, signature, binCount, mapState.v221fTrailHold))
        if mapState.updateTopologyPvs ~= nil then mapState.updateTopologyPvs(false) end
    end
end

mapState.revisitDepthForBin = function(idx)
    if idx == nil then return nil end
    local d = (mapState.revisitTrailDepthBins or {})[idx]
    if d ~= nil then
        mapState.revisitTrailDepthTouches
            = (mapState.revisitTrailDepthTouches or 0) + 1
    end
    return d
end
...
-- V22.1F: short-lived positive-ray sectors are a correctness layer, not a
-- performance working-set slot. In Ald-ruhn the bounded V22 cache rejected
-- thousands of ray promotions while PVS fell to only 5/31 sectors; this latch
-- preserves live visible corridors without making the whole cell resident.
mapState.rayPvsLatch = function(sid)
    sid = tonumber(sid or 0) or 0
    if sid <= 0 or sid == (mapState.topoSectorId or 0) then return false end
    local tc = mapState.topoCell
    if tc == nil or tc.sectors == nil or tc.sectors[sid] == nil then return false end

    local now = interiorElapsed
    local holds = mapState.rayPvsUntil or {}
    mapState.rayPvsUntil = holds

    -- Cheap bounded purge (normally <=10 entries).
    local live = 0
    for oid, expiry in pairs(holds) do
        if expiry == nil or expiry <= now then
            holds[oid] = nil
            mapState.rayPvsExpires = (mapState.rayPvsExpires or 0) + 1
        else
            live = live + 1
        end
    end
    mapState.rayPvsCount = live

    local existed = holds[sid] ~= nil
    if not existed and live >= mapState.v221fRayPvsMax then
        -- Drop the least-recently-refreshed ray latch only. Never touch the
        -- current room, authored door halo, revisit trail, or resident cache.
        local victim, victimExpiry = nil, 1.0e30
        for oid, expiry in pairs(holds) do
            if expiry < victimExpiry then
                victim, victimExpiry = oid, expiry
            end
        end
        if victim ~= nil then
            holds[victim] = nil
            live = max(0, live - 1)
            mapState.rayPvsDrops = (mapState.rayPvsDrops or 0) + 1
        end
    end

    if not existed then
        live = live + 1
        mapState.rayPvsAdds = (mapState.rayPvsAdds or 0) + 1
    end
    holds[sid] = now + mapState.v221fRayPvsHold
    mapState.rayPvsCount = live
    mapState.rayPvsTouches = (mapState.rayPvsTouches or 0) + 1
    return not existed
end
...
mapState.residentRayDiscover = function(idx, ex, ey, ez, observedDepth, wasMiss, rayLen)
    ...
    if reach < mapState.v221fRayPvsMinDepth then return false end
    ...
    for i = 1, #samples do
        ...
        if d >= mapState.v221fRayPvsMinDepth * 0.70 then
            local sx, sy, sz = ex + dx*d, ey + dy*d, ez + dz*d
            local sid = mapState.topology.findSector(lastCellName, sx, sy, sz)
            sid = tonumber(sid or 0) or 0
            if sid > 0 and sid ~= current and not seen[sid] then
                seen[sid] = true
                if mapState.rayPvsLatch(sid) then changed = true end

                -- Keep the older V22 long-lived room model too when it has room,
                -- but a full resident cache no longer vetoes the visible sector.
                if reach >= mapState.v22RayMinDepth then
                    if mapState.residentPromote(
                        ...
                        mapState.rayPvsBypasses = (mapState.rayPvsBypasses or 0) + 1
                    end
                end
            end
        end
...
-- V22.1G: topology PVS is a coarse structural pre-culler, so its safety
-- contract must be stronger than the live ray curtain. Navmesh sectors do not
-- own render meshes. Walls, arches, stairs and balcony rails can straddle sector
-- and floor boundaries even when their visible geometry is right beside the
-- player. This function builds a ray-independent local structural shell.
mapState.addStructuralPvsSafety = function(active)
    local tc = mapState.topoCell
    if tc == nil or tc.sectors == nil then
        mapState.pvsStructNearCount = 0
        mapState.pvsStructFloorCount = 0
        mapState.pvsStructWholeFloorCount = 0
        mapState.pvsStructUnsafe = false
    ...
    local floorNow = tonumber(mapState.topoFloor or 0) or 0
    local wholeFloor = 0
    if floorNow > 0 then
        -- Restore V17's proven structural invariant: every sector on the current
        -- topology floor stays eligible. V22's tiny resident cache remains useful
        -- for prediction, but it is no longer permission to delete same-floor
        -- static structure.
        for sid, sec in pairs(tc.sectors) do
            if type(sid) == 'number' and sid > 0
                and tonumber(sec.floor or 0) == floorNow then
                active[sid] = true
                wholeFloor = wholeFloor + 1
            end
        end
    end
    mapState.pvsStructWholeFloorCount = wholeFloor

    local x, y, z = mapState.topoX, mapState.topoY, mapState.topoZ
    ...
    local kind = tostring(mapState.topoKind or 'room')
    local range = mapState.v221gPvsNearRoomRange
    local unsafeNear = mapState.v221gPvsUnsafeRoomNear
    if kind == 'large_open' then
        range = mapState.v221gPvsNearOpenRange
        unsafeNear = mapState.v221gPvsUnsafeOpenNear
    elseif kind == 'vertical_connector' then
        range = mapState.v221gPvsNearConnectorRange
        unsafeNear = mapState.v221gPvsUnsafeConnectorNear
    end
    ...
    for sid, sec in pairs(tc.sectors) do
        local b = sec ~= nil and sec.bbox or nil
        if type(sid) == 'number' and sid > 0 and b ~= nil and #b >= 6 then
            local dx = 0.0
            if x < b[1] then dx = b[1] - x elseif x > b[4] then dx = x - b[4] end
            local dy = 0.0
            if y < b[2] then dy = b[2] - y elseif y > b[5] then dy = y - b[5] end
            local dz = 0.0
            if z < b[3] then dz = b[3] - z elseif z > b[6] then dz = z - b[6] end
            if dx*dx + dy*dy + dz*dz <= r2 then
                ids[#ids + 1] = sid
                local sf = tonumber(sec.floor or 0) or 0
                if sf > 0 then floors[sf] = true end
                -- No cap here. Truncating the nearest-sector list is exactly how
                -- a nearby wall can become structurally ownerless. If this set is
                -- too large to be a useful PVS, we fail the PVS open below.
    ...
    mapState.pvsStructNearCount = #ids
    mapState.pvsStructFloorCount = floorCount

    local total = mapState.pvsSectorCount or 0
    local ratioLimit = total > 0 and math.ceil(total * mapState.v221gPvsUnsafeRatio) or 999
    -- A large/open room or stair connector whose immediate shell crosses
    -- three or more authored floor labels is exactly the case where navmesh floor
    -- ownership is least trustworthy for render meshes. Do not gamble there:
    -- fail only topology PVS open and leave ordinary VISGRID active.
    local multiFloorAmbiguous = floorCount >= 3
        and (kind == 'large_open' or kind == 'vertical_connector')
    local unsafe = #ids > unsafeNear
        or (total >= 8 and #ids >= ratioLimit)
        or multiFloorAmbiguous
    local reason = ''
    if multiFloorAmbiguous then
        reason = 'multifloor-visual-shell'
    elseif #ids > unsafeNear then
        reason = 'dense-local-shell'
    elseif total >= 8 and #ids >= ratioLimit then
        reason = 'pvs-no-useful-separation'
    end

    local wasUnsafe = mapState.pvsStructUnsafe == true
    mapState.pvsStructUnsafe = unsafe
    mapState.pvsStructUnsafeReason = reason
    if unsafe and not wasUnsafe then
        mapState.pvsStructFailOpenActivations
            = (mapState.pvsStructFailOpenActivations or 0) + 1
    end

    local sig = string.format('%s|%d|%d|%d|%s', kind, wholeFloor, #ids,
        floorCount, table.concat(ids, ','))
    if sig ~= (mapState.pvsStructSignature or '') or unsafe ~= wasUnsafe then
        mapState.pvsStructSignature = sig
        print(string.format(
            '[TSP_VISGRID_V22.1G] STRUCT-SHELL kind=%s floor=%d whole=%d near=%d floors=%d range=%.0f unsafe=%d reason=%s ids=%s',
            kind, floorNow, wholeFloor, #ids, floorCount, range,
            unsafe and 1 or 0, reason ~= '' and reason or '-', table.concat(ids, ',')))
    end
    return unsafe
end

mapState.updateTopologyPvs = function(force)
    -- V22.1G structural safety must not depend on a current sector, resident-cache
    -- slot, or ray hit. If topology position exists, the physical shell itself can
    -- provide the safe structural set; otherwise the zero-ID path below fails open.
    ...
    -- V22.1F: current positive-ray sectors bypass the bounded resident cache.
    -- Expiry is short and refreshed only by live line-of-sight evidence.
    local rayIds = {}
    for sid, expiry in pairs(mapState.rayPvsUntil or {}) do
        if expiry ~= nil and expiry > interiorElapsed then
            mapState.pvsAddLatchedSector(active, sid)
            rayIds[#rayIds + 1] = sid
        else
            mapState.rayPvsUntil[sid] = nil
            mapState.rayPvsExpires = (mapState.rayPvsExpires or 0) + 1
        end
    end
    table.sort(rayIds)
    mapState.rayPvsCount = #rayIds
    local raySig = table.concat(rayIds, ',')
    if raySig ~= (mapState.rayPvsSignature or '') then
        mapState.rayPvsSignature = raySig
        mapState.rayPvsChanges = (mapState.rayPvsChanges or 0) + 1
        if interiorElapsed >= (mapState.rayPvsDebugNext or 0.0) then
            mapState.rayPvsDebugNext = interiorElapsed + 0.40
            print(string.format(
                '[TSP_VISGRID_V22.1F] RAY-PVS active=%d ids=%s touches=%d adds=%d bypass=%d drops=%d',
                #rayIds, raySig, mapState.rayPvsTouches or 0,
                mapState.rayPvsAdds or 0, mapState.rayPvsBypasses or 0,
                mapState.rayPvsDrops or 0))
        end
    end

    -- V22.1F: sectors the player physically occupied in the recent trail are
    -- stronger than speculative PVS. Keep them structural until their trail
    -- points expire so turning back cannot reveal a red/fogged room shell.
    local trailPvs = 0
    for sid in pairs(mapState.revisitTrailSectors or {}) do
        if sid > 0 then
            mapState.pvsAddLatchedSector(active, sid)
            trailPvs = trailPvs + 1
        end
    end
    mapState.revisitTrailPvsTouches
        = (mapState.revisitTrailPvsTouches or 0) + trailPvs

    -- V22.1G is deliberately independent of ray evidence. It is the structural
    ...
    -- V22.1G correctness rule: if the physically local structural shell itself is
    -- too dense for navmesh-sector ownership to be meaningful, topology PVS has no
    -- safe answer. Clear only the structural PVS. The ordinary VISGRID curtain and
    -- all ray learning remain active, so this trades some performance for guaranteed
    -- local structural correctness instead of waiting for rays to repair a hole.
    if structUnsafe then
        pcall(camera.clearInteriorTopologyPvs)
        mapState.pvsSignature = nil
        mapState.pvsActiveCount = 0
        mapState.pvsStructFailOpenUpdates
            = (mapState.pvsStructFailOpenUpdates or 0) + 1
        return
    end
    ...
    if force or signature ~= mapState.pvsSignature then
        ...
        if ok then
            ...
            print(string.format(
                '[TSP_VISGRID_V20] PVS floor=%d sector=%d active=%d/%d recent=%d latch=%d portal=%d ray=%d trail=%d rrj=%d',
                floorNow, current, #ids, mapState.pvsSectorCount,
                mapState.pvsRecentActive or 0, mapState.portalLatchCount or 0,
                mapState.pvsPortalSector or 0, mapState.rayPvsCount or 0,
                mapState.revisitTrailSectorCount or 0, mapState.residentRejects or 0))
        else
...
local function printStatus(budget)
    ...
    print(string.format(
        ...
        .. string.format(
            ' v221f=1 vt=%d vts=%d vtb=%d vta=%d vtd=%d vtp=%d rv=%d rvt=%d rva=%d rvx=%d rvb=%d rvd=%d',
            mapState.revisitTrailCount or 0,
            mapState.revisitTrailSectorCount or 0,
            mapState.revisitTrailBinCount or 0,
            mapState.revisitTrailAdds or 0,
            mapState.revisitTrailDepthTouches or 0,
            mapState.revisitTrailPvsTouches or 0,
            mapState.rayPvsCount or 0, mapState.rayPvsTouches or 0,
            mapState.rayPvsAdds or 0, mapState.rayPvsExpires or 0,
            mapState.rayPvsBypasses or 0, mapState.rayPvsDrops or 0)
        .. string.format(
            ' v221g=1 psn=%d psf=%d psw=%d psu=%d pso=%d psx=%d',
            mapState.pvsStructNearCount or 0,
            mapState.pvsStructFloorCount or 0,
            mapState.pvsStructWholeFloorCount or 0,
            mapState.pvsStructUnsafe and 1 or 0,
            mapState.pvsStructFailOpenActivations or 0,
            mapState.pvsStructFailOpenUpdates or 0))
...
local function enterInterior(cellName, ex, ey, ez)
    ...
    -- Preserve V22.1E behavior exactly: entry-neighborhood seeding and the
    -- learn-first gate belong only to a confirmed interior->interior transition.
...
local function onFrameBody(dt)
    ...
    -- V22.1F: capture the physical route BEFORE publish. If the player turns
    -- straight back into space they occupied seconds ago, this frame already
    -- has a directional preload instead of waiting for rays to rediscover it.
    mapState.updateRevisitTrail(ex, ey, ez, moveDist)
...
print('[TSP_VISGRID_V22.1F] revisit preload + positive-ray PVS authority loaded (ray-visible sectors cannot be resident-rejected)')
print('[TSP_VISGRID_V22.1G] structural PVS safety loaded (whole current floor + cross-floor local shell + ambiguity fail-open)')
...
EOF_V221G_SENSOR
    ...
    NEW_SHA="$(sha256sum "$PKG/visgrid-v22.1g.lua" | awk '{print $1}')"
    assert_eq "materialized V22.1G SHA" "$NEW_SHA" "$EXPECTED_NEW_SHA"
    grep -Fq 'TSP_VISGRID_LUA_V22_1F_REVISIT_TRAIL' "$PKG/visgrid-v22.1g.lua"
    grep -Fq 'TSP_VISGRID_LUA_V22_1F_RAY_PVS_AUTHORITY' "$PKG/visgrid-v22.1g.lua"
    grep -Fq 'TSP_VISGRID_LUA_V22_1G_STRUCTURAL_PVS_SAFETY' "$PKG/visgrid-v22.1g.lua"
    grep -Fq 'STRUCT-SHELL' "$PKG/visgrid-v22.1g.lua"
    grep -Fq 'RAY-PVS active=' "$PKG/visgrid-v22.1g.lua"
    grep -Fq 'v221g=1 psn=' "$PKG/visgrid-v22.1g.lua"

    if command -v texlua >/dev/null 2>&1; then
        cat > "$PKG/parse-check.lua" <<EOF_PARSE
assert(loadfile([[${PKG}/visgrid-v22.1g.lua]]))
print('PASS: texlua parsed exact V22.1G')
EOF_PARSE
        texlua "$PKG/parse-check.lua"
    else
        echo "INFO: texlua absent on host; exact output SHA matches independently parse-validated reference."
    fi

    echo
    echo "===== 3/6 BACKUP + INSTALL SENSOR ONLY ====="
    REMOTE_BACKUP="$LIVE_LUA.v22-1f-before-v22-1g-$STAMP"
    ssh -o BatchMode=yes -o ConnectTimeout=8 "$DEV" "
        set -e
    ...
    REMOTE_TMP="$LIVE_LUA.v22-1g-new-$STAMP"
    scp -q "$PKG/visgrid-v22.1g.lua" "$DEV:$REMOTE_TMP"
    ssh -o BatchMode=yes -o ConnectTimeout=8 "$DEV" "
        set -e
    ...
    "
    echo "PASS: V22.1G sensor installed."

    echo
    echo "===== 4/6 VERIFY PROTECTED STATE UNCHANGED ====="
    LIVE_SHA_AFTER="$(ssh "$DEV" "sha256sum '$LIVE_LUA'" | awk 'NF {print $1; exit}' | tr -d '
')"
    GAME_SHA_AFTER="$(ssh "$DEV" "sha256sum '$GAME'" | awk 'NF {print $1; exit}' | tr -d '
')"
    TOPO_SHA_AFTER="$(ssh "$DEV" "sha256sum '$TOPOLOGY'" | awk 'NF {print $1; exit}' | tr -d '
')"
    DOOR_SHA_AFTER="$(ssh "$DEV" "sha256sum '$DOORGRAPH'" | awk 'NF {print $1; exit}' | tr -d '
')"
    DB_STAT_AFTER="$(ssh "$DEV" "stat -c '%s %Y' '$DB'" | awk 'NF>=2 {print $1, $2; exit}' | tr -d '
')"
    TOPO_COUNT_AFTER="$(ssh "$DEV" "find '$TOPOLOGY_CELLS' -maxdepth 1 -type f -name 'c_*.lua' | wc -l" | awk 'NF {print $1; exit}' | tr -d '
')"
    DOOR_COUNT_AFTER="$(ssh "$DEV" "find '$DOORGRAPH_CELLS' -maxdepth 1 -type f -name 'c_*.lua' | wc -l" | awk 'NF {print $1; exit}' | tr -d '
')"

    assert_eq "installed V22.1G SHA" "$LIVE_SHA_AFTER" "$EXPECTED_NEW_SHA"
    assert_eq "game binary unchanged" "$GAME_SHA_AFTER" "$GAME_SHA_BEFORE"
    ...
    assert_eq "navmesh DB size/mtime unchanged" "$DB_STAT_AFTER" "$DB_STAT_BEFORE"
...
COLLECTOR="$HOME/Downloads/pull-visgrid-v22-1g-structural-pvs.sh"
cat > "$COLLECTOR" <<'EOF_COLLECT'
#!/usr/bin/env bash
set -u
...
chmod +x "$COLLECTOR"
bash -n "$COLLECTOR"
echo "PASS: collector: $COLLECTOR"

if [[ -n "$REMOTE_BACKUP" ]]; then
    ROLLBACK="$HOME/Downloads/restore-visgrid-v22-1f-from-v22-1g.sh"
    cat > "$ROLLBACK" <<EOF_ROLL
#!/usr/bin/env bash
set -Eeuo pipefail
DEV="\${TSP_DEV:-root@192.168.1.25}"
LIVE_LUA="$LIVE_LUA"
...
EOF_ROLL
    chmod +x "$ROLLBACK"
    bash -n "$ROLLBACK"
    echo "PASS: rollback helper: $ROLLBACK"
fi

echo
echo "===== 6/6 COMPLETE ====="
echo "Installed sensor SHA: $EXPECTED_NEW_SHA"
echo "Restart OpenMW before testing."
echo "Primary test: Ald-ruhn Mages Guild — use the same stairs, balcony/railings,"
echo "arches and room views that previously showed blue missing structure."
echo "The structural pieces should already exist BEFORE rays have time to rescue them."
echo "Also recheck South Wall so the door-frame/revisit behavior remains intact."
echo "Then run:"
echo "  cd ~/Downloads"
echo "  ./pull-visgrid-v22-1g-structural-pvs.sh"
echo "New fields: psn=near structural sectors, psf=local floor labels,"
echo "psw=whole-current-floor sectors, psu=structural fail-open active,"
echo "pso=fail-open activations, psx=fail-open PVS updates."
echo "If psu=1 in a dense atrium/stair region, that is intentional: only topology"
echo "PVS is disabled there; VISGRID remains active so correctness wins safely."
