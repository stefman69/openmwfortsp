...
EXPECTED_BASE_SHA="bf5097ae5a5a668eb38802b8b9b67dc9ec3620e73d9b0a3fe0c2c5c19a524df4"
EXPECTED_NEW_SHA="8b2db68de598febff9559cb248e509a99d8c3abdda29c54b42447f6970b0f7fc"

STAMP="$(date +%Y%m%d-%H%M%S)"
OUT="$HOME/Downloads/openmw51-visgrid-v22-1f-$STAMP"
PKG="$OUT/pkg"
...
on_error() {
    ...
    echo
    echo "=================================================================="
    echo "V22.1F CONTROLLER STOPPED (rc=$rc)"
    echo "=================================================================="
    if [[ "$ROLLBACK_READY" == "1" && -n "$REMOTE_BACKUP" ]]; then
        ...
        "; then
            echo "PASS: exact V22.1E sensor restored."
        else
...
echo "=================================================================="
echo "OPENMW 0.51 — VISGRID V22.1F REVISIT + RAY-PVS CORRECTNESS"
echo "=================================================================="
echo "Exact base required: V22.1E $EXPECTED_BASE_SHA"
echo "Exact output:        V22.1F $EXPECTED_NEW_SHA"
echo
echo "Purpose:"
echo "  - keep spaces the player physically walked through warm for 20s"
echo "  - when the camera turns back toward that route, pre-open only the"
echo "    matching angular bins instead of waiting for rays to rediscover it"
echo "  - keep topology sectors actually occupied by that recent route structural"
echo "  - preserve V22.1E door-frame halo + intercell entry preload unchanged"
echo "  - positive collision rays now structurally latch every sector they prove"
echo "    visible, even when V22's 3/5 resident working-set is full"
echo "  - add trace counters that expose resident-cap bypasses in complex cells"
echo
...
echo
echo "===== 1/6 VERIFY EXACT LIVE V22.1E + SNAPSHOT PROTECTED STATE ====="
...
if [[ "$BASE_SHA" == "$EXPECTED_NEW_SHA" ]]; then
    echo "PASS: V22.1F is already installed."
    ROLLBACK_READY=0
else
    assert_eq "exact V22.1E sensor SHA" "$BASE_SHA" "$EXPECTED_BASE_SHA"
fi
...
if [[ "$BASE_SHA" == "$EXPECTED_NEW_SHA" ]]; then
    ...
    echo
    echo "===== 2/6 MATERIALIZE + VALIDATE EXACT V22.1F ====="
    cat > "$PKG/visgrid-v22.1f.lua" <<'EOF_V221F_SENSOR'
...
mapState.updateTopologyPvs = function(force)
    if not mapState.pvsBridge or mapState.topoCell == nil
        ...
        or (mapState.topoSector == nil and mapState.residentCount <= 0
            and (mapState.rayPvsCount or 0) <= 0) then
...
EOF_V221F_SENSOR
    ...
    NEW_SHA="$(sha256sum "$PKG/visgrid-v22.1f.lua" | awk '{print $1}')"
    assert_eq "materialized V22.1F SHA" "$NEW_SHA" "$EXPECTED_NEW_SHA"
    grep -Fq 'TSP_VISGRID_LUA_V22_1F_REVISIT_TRAIL' "$PKG/visgrid-v22.1f.lua"
    grep -Fq 'TSP_VISGRID_LUA_V22_1F_RAY_PVS_AUTHORITY' "$PKG/visgrid-v22.1f.lua"
    grep -Fq 'REVISIT-TRAIL' "$PKG/visgrid-v22.1f.lua"
    grep -Fq 'RAY-PVS active=' "$PKG/visgrid-v22.1f.lua"
    grep -Fq 'v221f=1 vt=' "$PKG/visgrid-v22.1f.lua"

    if command -v texlua >/dev/null 2>&1; then
        cat > "$PKG/parse-check.lua" <<EOF_PARSE
assert(loadfile([[${PKG}/visgrid-v22.1f.lua]]))
print('PASS: texlua parsed exact V22.1F')
EOF_PARSE
    ...
    REMOTE_BACKUP="$LIVE_LUA.v22-1e-before-v22-1f-$STAMP"
    ssh -o BatchMode=yes -o ConnectTimeout=8 "$DEV" "
        set -e
    ...
    REMOTE_TMP="$LIVE_LUA.v22-1f-new-$STAMP"
    scp -q "$PKG/visgrid-v22.1f.lua" "$DEV:$REMOTE_TMP"
    ssh -o BatchMode=yes -o ConnectTimeout=8 "$DEV" "
        set -e
    ...
    "
    echo "PASS: V22.1F sensor installed."
...
')"
    ...
    assert_eq "installed V22.1F SHA" "$LIVE_SHA_AFTER" "$EXPECTED_NEW_SHA"
...
COLLECTOR="$HOME/Downloads/pull-visgrid-v22-1f-revisit-trail.sh"
...
if [[ -n "$REMOTE_BACKUP" ]]; then
    ROLLBACK="$HOME/Downloads/restore-visgrid-v22-1e-from-v22-1f.sh"
...
echo "Test 1: South Wall basement — walk through it, turn away, then return."
echo "The recent route should already be open without waiting for ray cleanup."
echo "Test 2: Ald-ruhn Mages Guild — repeat the broken stairs/arches/large-room"
echo "views. A live ray-proven sector must stay structural instead of going blue."
echo "Then run:"
echo "  cd ~/Downloads"
echo "  ./pull-visgrid-v22-1f-revisit-trail.sh"
echo "Watch fields: vt/vts/vtb/vta/vtd/vtp = revisit trail; rv=active ray-PVS,"
echo "rvt=touches, rva=adds, rvx=expires, rvb=resident-cap bypasses, rvd=LRU drops,"
echo "rrj may still rise; if rvb rises too, the new ray-PVS correctness path"
echo "is specifically rescuing sectors the old resident cap rejected."
