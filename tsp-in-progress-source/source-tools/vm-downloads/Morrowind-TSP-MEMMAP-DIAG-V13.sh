...
echo "Launcher revision: openmw-tsp-pvr-texture-storage-readback-2026-09-22-v12"
...
tsp_apply_internal_render_scale() {
    ...
    # TSP_PVR_TEXTURE_FORK_V2
    # Original TSP/PowerVR gets the NEW isolated texture/storage diagnostic
    # sidecar. TSPS/Mali keeps the unchanged production libGL.
    TSP_PVR_TEXTURE_SIDE="$GAMEDIR/lib.tsp-pvr-texture-v2/libGL.so.1"
    if grep -q '^pvrsrvkm ' /proc/modules 2>/dev/null \
       && ! grep -qi '^mali' /proc/modules 2>/dev/null; then
        if [ ! -r "$TSP_PVR_TEXTURE_SIDE" ]; then
            echo "ERROR: TSP PowerVR texture-v2 sidecar missing:"
            echo "  $TSP_PVR_TEXTURE_SIDE"
            exit 74
        fi
        export TSP_GL4ES_OVERRIDE="$TSP_PVR_TEXTURE_SIDE"
...
# >>> TSP_MAP_DIAG_AUTO_V1 BEGIN
# Dedicated map/memory diagnostic launcher.
#
# Hardware gate:
#   Original TSP / PowerVR -> diagnostics ARM automatically.
#   TSPS / Mali            -> diagnostics stay OFF.
#
# This launcher never replaces or edits lib/libGL.so.1.  It only enables
# diagnostic hooks already compiled into the normal production libGL.
TSP_MAP_DIAG_ENABLED=0
TSP_MAP_DIAG_ROOT="$GAMEDIR/tsp_map_diag"
TSP_MAP_DIAG_LATEST="$TSP_MAP_DIAG_ROOT/latest"
TSP_MAP_DIAG_PREVIOUS="$TSP_MAP_DIAG_ROOT/previous"
TSP_MAP_DIAG_ARCHIVE="$GAMEDIR/tsp_map_diag_latest.tar"

# MASTER diagnostic switch for this TSP-only fork.
#   1 = native texture/storage accounting + PowerVR sampler + FBO trace +
#       limited map RTT readback
#   0 = use the TSP-only fork with ALL of those diagnostics disabled.
TSP_PVR_TEXTURE_DIAG="${TSP_PVR_TEXTURE_DIAG:-1}"
TSP_NATIVE_LIFE_DIAG="$TSP_PVR_TEXTURE_DIAG"
TSP_NATIVE_DRIVER_PID=""

tsp_native_driver_sampler() {
    _out="$1"
    _n=0
    {
        echo "# TSP_NATIVE_DRIVER_SAMPLER_V1"
        echo "# sample every 2 seconds"
    } > "$_out" 2>/dev/null || return 0
    while :; do
        {
            echo "===== sample=$_n time=$(date +%s 2>/dev/null) ====="
            if [ -r /sys/kernel/debug/pvr/driver_stats ]; then
                grep -E 'MemoryUsageAllocGPUMemUMA|MemoryUsageAllocGPUMemUMAPool|MemoryUsageMappedGPUMemUMA/LMA|MemoryUsage.*Max' \
                    /sys/kernel/debug/pvr/driver_stats 2>/dev/null || true
            else
                echo "driver_stats=unavailable"
            fi
            grep -E '^(MemAvailable|MemFree|Cached|SwapCached|Slab|SReclaimable|SUnreclaim|SwapTotal|SwapFree):' \
...
tsp_map_diag_is_powervr() {
    if grep -q '^pvrsrvkm ' /proc/modules 2>/dev/null; then
        return 0
    fi

    # Never classify a Mali/TSPS kernel as PowerVR.
    if grep -qi 'mali' /proc/modules 2>/dev/null; then
        return 1
    fi

    [ -d /sys/devices/platform/gpu/scenectrl ]
}

tsp_map_diag_collect() {
    [ "${TSP_MAP_DIAG_ENABLED:-0}" = 1 ] || return 0

    if [ -n "${TSP_NATIVE_DRIVER_PID:-}" ]; then
        kill "$TSP_NATIVE_DRIVER_PID" 2>/dev/null || true
        wait "$TSP_NATIVE_DRIVER_PID" 2>/dev/null || true
        TSP_NATIVE_DRIVER_PID=""
    fi

    _md="$TSP_MAP_DIAG_LATEST"
    _tmp="$GAMEDIR/.tsp_map_diag_latest.tar.$$"

    mkdir -p "$_md" 2>/dev/null || true

    {
        echo "TSP_MAP_DIAG_AUTO_V1"
        echo "collected=$(date 2>/dev/null)"
        echo "launcher=$0"
        ...
        echo
        echo "===== zram/zsmalloc modules ====="
        grep -E '^(zram|zsmalloc) ' /proc/modules 2>/dev/null || true
        echo
        echo "===== meminfo ====="
        cat /proc/meminfo 2>/dev/null || true
        echo
        echo "===== selected vmstat ====="
        grep -E '^(pgmajfault|pswpin|pswpout|pgscan_|pgsteal_|allocstall|compact_|oom_kill)' \
            /proc/vmstat 2>/dev/null || true
    } > "$_md/exit-state.txt" 2>&1

    cp -p "$GAMEDIR/openmw_log.txt" "$_md/openmw_log.txt" 2>/dev/null || true
    cp -p "$GAMEDIR/openal_soft.log" "$_md/openal_soft.log" 2>/dev/null || true
    dmesg > "$_md/dmesg.txt" 2>/dev/null || true

    if [ -d "$GAMEDIR/tsp_freeze_monitor/latest" ]; then
        rm -rf "$_md/freeze-monitor-latest" 2>/dev/null || true
        cp -pr "$GAMEDIR/tsp_freeze_monitor/latest" \
            "$_md/freeze-monitor-latest" 2>/dev/null || true
    fi

    if [ -d "$GAMEDIR/tsp_freeze_monitor/previous" ]; then
        rm -rf "$_md/freeze-monitor-previous" 2>/dev/null || true
        cp -pr "$GAMEDIR/tsp_freeze_monitor/previous" \
            "$_md/freeze-monitor-previous" 2>/dev/null || true
    fi

    if [ -f "$LIBGL_TSP_FBOPATH" ]; then
        grep -E \
            'ATTACH_TEX|ATTACH_SIZE|GLNAMEFIX|GLNAMEZERO|ATTACHVERIFY|DRAWFBO|PASS' \
            "$LIBGL_TSP_FBOPATH" \
            > "$_md/color-attachment-events.txt" 2>/dev/null || true
    fi

    if [ -f "$GAMEDIR/openmw_log.txt" ]; then
        grep -E \
            'TSP_GMAP_|TSP_RTT_|TSP_MAP_|TSP_LOAD_|TSP_NATIVE_|TSP_PVR_|PORTABLE ZRAM:|TSP_MAXCOLORATTACH' \
            "$GAMEDIR/openmw_log.txt" \
            > "$_md/map-memory-events.txt" 2>/dev/null || true
    fi

    rm -f "$_tmp" 2>/dev/null || true

    # BusyBox-safe raw tar.
    tar -cf "$_tmp" -C "$TSP_MAP_DIAG_ROOT" latest previous 2>/dev/null \
        || tar -cf "$_tmp" -C "$TSP_MAP_DIAG_ROOT" latest 2>/dev/null \
        || true

    if [ -s "$_tmp" ]; then
        mv -f "$_tmp" "$TSP_MAP_DIAG_ARCHIVE"
        echo "TSP_MAP_DIAG_AUTO_V1 archive=$TSP_MAP_DIAG_ARCHIVE"
    else
        rm -f "$_tmp" 2>/dev/null || true
        echo "TSP_MAP_DIAG_AUTO_V1 WARNING archive creation failed"
    fi
}

if tsp_map_diag_is_powervr; then
    TSP_MAP_DIAG_ENABLED="$TSP_PVR_TEXTURE_DIAG"

    mkdir -p "$TSP_MAP_DIAG_ROOT"
    rm -rf "$TSP_MAP_DIAG_PREVIOUS" 2>/dev/null || true
    if [ -d "$TSP_MAP_DIAG_LATEST" ]; then
        mv "$TSP_MAP_DIAG_LATEST" "$TSP_MAP_DIAG_PREVIOUS" 2>/dev/null || true
    fi
    mkdir -p "$TSP_MAP_DIAG_LATEST"

    # TSP_PVR_TEXTURE_DIAG_V2
    # One switch controls EVERY extra diagnostic in this launcher. The sidecar
    # itself remains selected on original TSP/PowerVR when this is 0.
    if [ "$TSP_PVR_TEXTURE_DIAG" = 1 ]; then
        export LIBGL_TSP_NATIVE_LIFE=1
        export LIBGL_TSP_NATIVE_LIFE_PATH="$TSP_MAP_DIAG_LATEST/native-life.log"

        export LIBGL_TSP_TEXTURE_DIAG=1
        export LIBGL_TSP_TEXTURE_DIAG_PATH="$TSP_MAP_DIAG_LATEST/texture-native.log"

        export LIBGL_TSP_MAP_READBACK_DIR="$TSP_MAP_DIAG_LATEST/map-readback"
        mkdir -p "$LIBGL_TSP_MAP_READBACK_DIR"

        rm -f             "$LIBGL_TSP_NATIVE_LIFE_PATH"             "$LIBGL_TSP_TEXTURE_DIAG_PATH"

        tsp_native_driver_sampler "$TSP_MAP_DIAG_LATEST/pvr-driver-stats.log" &
        TSP_NATIVE_DRIVER_PID=$!

        # Existing FBO trace is useful to correlate the new texture/readback
        # events, but is also behind the master diagnostic switch.
        export LIBGL_TSP_FBODUMP=1
        export LIBGL_TSP_FBOSNAP=1
        export LIBGL_TSP_FBOPATH="$TSP_MAP_DIAG_LATEST/fbo-trace.txt"
        rm -f "$LIBGL_TSP_FBOPATH"

        # Existing OpenMW map diagnostics. Repair mode 1 remains report-only.
        export TSP_GMAP_REPAIR=1
        export TSP_GMAP_DUMP="$TSP_MAP_DIAG_LATEST/tsp_globalmap_overlay.png"

        export OPENMW_DEBUG_LEVEL=info

        echo "TSP_PVR_TEXTURE_DIAG_V2 armed=1 pid=$TSP_NATIVE_DRIVER_PID"
    else
        unset LIBGL_TSP_NATIVE_LIFE
        unset LIBGL_TSP_NATIVE_LIFE_PATH
        unset LIBGL_TSP_TEXTURE_DIAG
        unset LIBGL_TSP_TEXTURE_DIAG_PATH
        unset LIBGL_TSP_MAP_READBACK_DIR
        unset LIBGL_TSP_FBODUMP
        unset LIBGL_TSP_FBOSNAP
        unset LIBGL_TSP_FBOPATH
        unset TSP_GMAP_REPAIR
        unset TSP_GMAP_DUMP
        echo "TSP_PVR_TEXTURE_DIAG_V2 armed=0"
    fi

    {
        ...
        echo "gpu=PowerVR"
        echo "pvr_texture_diag=$TSP_PVR_TEXTURE_DIAG"
        echo "native_life_path=${LIBGL_TSP_NATIVE_LIFE_PATH:-disabled}"
        echo "texture_diag_path=${LIBGL_TSP_TEXTURE_DIAG_PATH:-disabled}"
        echo "map_readback_dir=${LIBGL_TSP_MAP_READBACK_DIR:-disabled}"
        echo "libgl=$TSP_GL4ES_LIBRARY"
        echo "libgl_sha=$(sha256sum "$TSP_GL4ES_LIBRARY" 2>/dev/null | awk '{print $1}')"
        echo "openmw_sha=$(sha256sum "$OPENMW_BIN" 2>/dev/null | awk '{print $1}')"
        echo "kernel=$(uname -a 2>/dev/null)"
        echo "fbo_trace=${LIBGL_TSP_FBOPATH:-disabled}"
        echo "gmap_dump=${TSP_GMAP_DUMP:-disabled}"
    } > "$TSP_MAP_DIAG_LATEST/identity.txt" 2>&1

    echo "TSP_MAP_DIAG_AUTO_V1 ARMED gpu=PowerVR texture_diag=$TSP_PVR_TEXTURE_DIAG out=$TSP_MAP_DIAG_LATEST"
else
    unset LIBGL_TSP_NATIVE_LIFE
    unset LIBGL_TSP_NATIVE_LIFE_PATH
    unset LIBGL_TSP_TEXTURE_DIAG
    ...
    unset TSP_GMAP_REPAIR
    unset TSP_GMAP_DUMP
    echo "TSP_MAP_DIAG_AUTO_V1 SKIPPED gpu=non-PowerVR"
fi
# <<< TSP_MAP_DIAG_AUTO_V1 END
...
tsp_freeze_monitor_start() {
    ...
    if [ "${TSP_MAP_DIAG_ENABLED:-0}" != 1 ] && [ -f "$GAMEDIR/tsp_freeze_monitor_off" ]; then
...
# <<< TSP_V36_PERF_MONITOR_STOP END
    ...
    # TSP_MAP_DIAG_AUTO_V1: package automatically after OpenMW exits, while
    # game-session ZRAM state is still present.
    type tsp_map_diag_collect >/dev/null 2>&1 && tsp_map_diag_collect
