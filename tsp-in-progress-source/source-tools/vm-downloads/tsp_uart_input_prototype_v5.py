...
# The stock daemon's clamp_scale() was previously verified from this exact
# binary to use a ±900 input clamp before mapping to the uinput axis range.
# V5 keeps that scaling but removes the daemon's repeated calibration loop.
SCALE_LIMIT = 900
...
def ioctl_int(fd, request, value):
    # The stock daemon passes capability values as scalar ioctl arguments.
...
def scale_like_stock(delta):
    """Linear ±900 input -> ±32760 output, then saturate."""
    delta = max(-SCALE_LIMIT, min(SCALE_LIMIT, int(delta)))
    return int(round(delta * AXIS_MAX / SCALE_LIMIT))
...
def main():
    ...
    print("==========================================")
    print(" TSP UART INPUT PROTOTYPE V5")
    print("==========================================")
    ...
    print("Stock-style ±900 scaling; repeated calibration loop DISABLED")
    ...
    try:
        ...
        print("uinput device created: ABS_X/ABS_Y + ABS_Z/ABS_RZ", flush=True)
        ...
        motion_started = {path: None for path in uart_fds}
        motion_peak = {path: 0 for path in uart_fds}
        motion_marks = {path: set() for path in uart_fds}
        ...
        while RUNNING:
            ...
            for fd in ready:
                ...
                for packet in decode_packets(buffers[path]):
                    ...
                    # Match the stock daemon's left/right X orientation.
                    if path == "/dev/ttyS4":
                        delta_x = -delta_x
                    else:
                        delta_x = -delta_x

                    # Preserve the stock daemon's output scaling, but do it
                    # directly on the raw UART values after one startup center
                    # calibration rather than recalibrating every packet loop.
                    out_x = scale_like_stock(delta_x)
                    out_y = scale_like_stock(delta_y)
                    ...
                    # Timed Y-motion diagnostic for quick-vs-slow testing.
                    if abs(delta_y) >= 100:
                        if motion_started[path] is None:
                            motion_started[path] = time.monotonic()
                            motion_peak[path] = abs(out_y)
                            motion_marks[path] = set()
                            print(
                                "%s MOTION START raw_dY=%d outY=%d"
                                % (path, delta_y, out_y),
                                flush=True,
                            )

                        motion_peak[path] = max(motion_peak[path], abs(out_y))
                        elapsed = time.monotonic() - motion_started[path]

                        for mark in (0.10, 0.25, 0.50, 1.00, 2.00):
                            if elapsed >= mark and mark not in motion_marks[path]:
                                motion_marks[path].add(mark)
                                print(
                                    "%s +%.2fs peak_out=%d (%.1f%%) raw_dY=%d"
                                    % (
                                        path,
                                        mark,
                                        motion_peak[path],
                                        motion_peak[path] * 100.0 / AXIS_MAX,
                                        delta_y,
                                    ),
                                    flush=True,
                                )
