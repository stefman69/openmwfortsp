...
echo "Launcher revision: openmw-portable-zram-fog-nopbo-diag-2026-09-21-v9"
...
tsp_map_diag_collect() {
    ...
    if [ -f "$GAMEDIR/openmw_log.txt" ]; then
        grep -E \
            'TSP_GMAP_|TSP_RTT_|TSP_MAP_|TSP_LOAD_|TSP_FOG_|PORTABLE ZRAM:|TSP_MAXCOLORATTACH' \
...
if tsp_map_diag_is_powervr; then
    ...
    # TSP_FOG_NO_PBO_LAUNCH_V1
    # The modified OpenMW binary is an ADDED sidecar. The normal openmw-0.51
    # remains untouched and is still used on TSPS/Mali.
    TSP_FOG_NO_PBO_BIN="$GAMEDIR/bin/openmw-0.51.tsp-fog-nopbo-v1"
    if [ ! -x "$TSP_FOG_NO_PBO_BIN" ]; then
        echo "ERROR: TSP fog-no-PBO sidecar missing: $TSP_FOG_NO_PBO_BIN"
        exit 71
    fi
    OPENMW_BIN="$TSP_FOG_NO_PBO_BIN"
    export TSP_FOG_NO_PBO=1
    echo "TSP_FOG_NO_PBO_LAUNCH_V1 selected=1 binary=$OPENMW_BIN"
...

    {
        ...
        echo "gpu=PowerVR"
        echo "fog_no_pbo=${TSP_FOG_NO_PBO:-0}"
    ...
    echo "TSP_FOG_NO_PBO_LAUNCH_V1 selected=0 binary=$OPENMW_BIN"
