...
echo "Controller mode: native OpenMW 0.51 + system controller + hybrid helper"
...
# TSP_SYSTEM_CONTROLLER_V2 -- controller ownership belongs to the OS.
#
# On muOS the experimental system patch owns /usr/bin/trimui_inputd_smart_pro
# under the stock path. This launcher deliberately does NOT kill, start, stop,
# restore, or otherwise take ownership of that handler. OpenMW consumes the
# system-published TRIMUI Smart Pro Controller like every other application.
...
# TSP_SYSTEM_CONTROLLER_V2
# Stick calibration and input routing are system-owned. This launcher only
# consumes the controller SDL exposes; it carries no per-game UART route.
# Universal SDL mapping remains leftx:a0,lefty:a1,rightx:a2,righty:a3.
