...
# >>> TSP_FREEZE_DIAG_V2 BEGIN
# Long-run freeze diagnostics, deliberately separate from TSP_QUIET.
# This keeps normal quiet-mode behaviour (including the expensive 60s draw-thread
# telemetry remaining OFF) while allowing only the low-overhead perf sampler and
# OpenMW frame ring to run.  Disable with $GAMEDIR/tsp_freeze_diag_off.
TSP_FREEZE_DIAG=1
[ -f "$GAMEDIR/tsp_freeze_diag_off" ] && TSP_FREEZE_DIAG=0
export TSP_FREEZE_DIAG
# <<< TSP_FREEZE_DIAG_V2 END
...
echo "Launcher revision: openmw-muos-audio-watchdog-clean-2026-09-16-v1"
...
#
# TSP_FREEZE_DIAG_V2: quiet mode still suppresses all unrelated diagnostics, but the
# proven OpenMW frame ring is allowed for this long-run freeze capture.  The path is
# the portable game root instead of hardcoded /mnt/SDCARD so muOS/Knulli can use it.
if [ -f /mnt/SDCARD/tsp_ring_off ] || [ -f "$GAMEDIR/tsp_ring_off" ] || { [ "$TSP_QUIET" = 1 ] && [ "${TSP_FREEZE_DIAG:-0}" != 1 ]; }; then
    unset OPENMW_TSP_RING
    unset OPENMW_TSP_RING_TRIGGER_MS
    unset OPENMW_TSP_RING_MAX_DUMPS
    echo "TSP_RINGARM_V2 disabled (ring_off or diagnostics off)" >> "$TSP_PROG"
else
    if [ -f "$GAMEDIR/tsp_ring.conf" ]; then
        . "$GAMEDIR/tsp_ring.conf"
    elif [ -f /mnt/SDCARD/tsp_ring.conf ]; then
        . /mnt/SDCARD/tsp_ring.conf
    fi
    ...
    export OPENMW_TSP_RING="$GAMEDIR/tsp_ring"
    mkdir -p "$OPENMW_TSP_RING"
    export OPENMW_TSP_RING_TRIGGER_MS="$TSP_RING_TRIG"
    export OPENMW_TSP_RING_MAX_DUMPS="$TSP_RING_MAX"
    echo "TSP_RINGARM_V2 armed trigger=$TSP_RING_TRIG max=$TSP_RING_MAX out=$OPENMW_TSP_RING freeze_diag=${TSP_FREEZE_DIAG:-0}" >> "$TSP_PROG"
fi
...
# >>> TSP_CRASHWATCH_REARM_V1 BEGIN
# Re-enable the existing binary-side crashwatch during freeze diagnostics.
TSP_CRASH_OUT="$GAMEDIR/tsp_crashwatch_latest.txt"
export TSP_CRASH_OUT
if [ -s "$TSP_CRASH_OUT" ]; then
    cp -f "$TSP_CRASH_OUT" "$GAMEDIR/tsp_crashwatch_previous.txt" 2>/dev/null || true
fi
rm -f "$TSP_CRASH_OUT" 2>/dev/null || true

TSP_CRASHWATCH_HOOKS=""
for _cw in "$OPENMW_BIN" "$GAMEDIR/lib/libtsp_warm.so" "$GAMEDIR/lib/libtsp_fullscreen_scaler.so" "$TSP_GL4ES_LIBRARY"; do
    [ -r "$_cw" ] || continue
    if grep -aq 'TSP_CRASH_OUT' "$_cw" 2>/dev/null; then
        TSP_CRASHWATCH_HOOKS="${TSP_CRASHWATCH_HOOKS}${TSP_CRASHWATCH_HOOKS:+,}$_cw"
    fi
done
echo "TSP_CRASHWATCH_REARM_V1 armed out=$TSP_CRASH_OUT hooks=${TSP_CRASHWATCH_HOOKS:-NONE}"
# <<< TSP_CRASHWATCH_REARM_V1 END
...
tsp_perf_sampler() {
    ...
    # TSP_FREEZE_DIAG_V2: still one 2-second sampler and still shell-only reads
    # in the sampling loop.  Add the fields needed to separate:
    #   process leak vs GPU/kernel memory loss vs swap/reclaim/I/O collapse.
    {
        printf '# openmw perf sampler V2 pid=%s freeze_diag=%s\n' "$PERF_PID" "${TSP_FREEZE_DIAG:-0}"
        printf '# all memory values are kB; psi values are avg10; loop sectors are 512-byte sectors\n'
        printf '# t rss vsz data vmswap anon file shmem threads memavail memfree cached sreclaim cmafree swapfree swapcached psi_cpu psi_mem psi_io minflt majflt utime stime sys_pgmaj pswpin pswpout scan_direct steal_direct allocstall oom_kill io_read io_write loop_rsect loop_wsect loop_inflight loop_ioticks\n'
    } > "$TSP_PERF_LOG"

    LOOPSTAT=""
    while read -r _dev _type _size _used _prio; do
        case "$_dev" in
            /dev/loop*)
                _ln=${_dev##*/}
                [ -r "/sys/block/$_ln/stat" ] && LOOPSTAT="/sys/block/$_ln/stat"
                ;;
        esac
    done < /proc/swaps
    ...
    T=0
    while kill -0 "$PERF_PID" 2>/dev/null; do
        RSS=na; VSZ=na; DATA=na; VMSWAP=na; ANON=na; FILE=na; SHMEM=na; THR=na
        ...
        MA=na; MF=na; CACHED=na; SREC=na; CMA=na; SWFREE=na; SWCACHED=na
        while read -r k v _; do
            case "$k" in
                MemAvailable:) MA=$v ;;
                MemFree:)      MF=$v ;;
        ...
        MINFLT=na; MAJFLT=na; UT=na; ST=na
        if read -r _STATLINE < "/proc/$PERF_PID/stat"; then
            _REST=${_STATLINE##*) }
            set -- $_REST
            MINFLT=${8:-na}
            MAJFLT=${10:-na}
        ...
        IOR=na; IOW=na
        if [ -r "/proc/$PERF_PID/io" ]; then
            while read -r k v; do
                case "$k" in
                    read_bytes:)  IOR=$v ;;
                    write_bytes:) IOW=$v ;;
                esac
            done < "/proc/$PERF_PID/io"
        fi

        LRS=na; LWS=na; LINF=na; LIOT=na
        if [ -n "$LOOPSTAT" ] && [ -r "$LOOPSTAT" ]; then
            read -r _lr _lrm LRS _lrt _lw _lwm LWS _lwt LINF LIOT _lwi _ < "$LOOPSTAT"
        fi

        printf '%s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s\n' \
            "$T" "$RSS" "$VSZ" "$DATA" "$VMSWAP" "$ANON" "$FILE" "$SHMEM" "$THR" \
            "$MA" "$MF" "$CACHED" "$SREC" "$CMA" "$SWFREE" "$SWCACHED" \
            "$PC" "$PM" "$PI" "$MINFLT" "$MAJFLT" "$UT" "$ST" \
            "$PGMAJ" "$PSIN" "$PSOUT" "$SCAN" "$STEAL" "$ALSTALL" "$OOMK" \
            "$IOR" "$IOW" "$LRS" "$LWS" "$LINF" "$LIOT" >> "$TSP_PERF_LOG"
...
# =================== end TSP_AB_SWITCH_V1 ====================================
    ...
    # >>> TSP_FREEZE_DIAG_V2 PRELAUNCH BEGIN
    if [ "${TSP_FREEZE_DIAG:-0}" = 1 ]; then
        {
            echo "TSP_FREEZE_DIAG_V2"
            echo "date=$(date)"
            ...
            echo "cfw=${CFW_NAME:-unknown} device=${DEVICE_NAME:-unknown} cpu=${DEVICE_CPU:-unknown}"
            echo "--- /proc/swaps ---"
            ...
            for _p in cpu memory io; do
                echo "[$_p]"
                cat "/proc/pressure/$_p" 2>/dev/null
            done
            echo "--- power/gpu nodes ---"
            for _p in /sys/devices/platform/gpu/scenectrl/command /sys/devices/platform/gpu/scenectrl/status /sys/class/devfreq/*gpu*/cur_freq; do
                [ -e "$_p" ] || continue
                echo "$_p=$(cat "$_p" 2>/dev/null)"
            done
        } > "$GAMEDIR/tsp_freeze_diag_header.txt" 2>&1
        echo "TSP_FREEZE_DIAG_V2 armed perf=$TSP_PERF_LOG ring=${OPENMW_TSP_RING:-off} header=$GAMEDIR/tsp_freeze_diag_header.txt"
    fi
    # <<< TSP_FREEZE_DIAG_V2 PRELAUNCH END
...
if [ "$TSP_QUIET" = 1 ] && [ "${TSP_FREEZE_DIAG:-0}" != 1 ]; then
    TSP_PERF_MONITOR_PID=""
else   # TSP_FREEZE_DIAG_V2: low-overhead sampler only; expensive quiet-gated telemetry stays off
