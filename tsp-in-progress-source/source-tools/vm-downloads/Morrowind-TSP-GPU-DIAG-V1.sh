...
# TSP_MUOS_STICK_V2
# muOS-only stick changes are intentionally disabled here.
# The previous uinput proxy and the experimental joycal calibration both get
# removed from this launcher while we isolate the remaining input behavior.
# The universal SDL mapping above uses righty:a3 because SDL mapping indices
# are SDL joystick-axis indices, not raw Linux ABS codes: TSP exposes X,Y,Z,RZ
# as the four joystick axes, so RZ is SDL axis 3.
