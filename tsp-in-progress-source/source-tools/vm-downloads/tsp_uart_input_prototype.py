...
# Linux input/uinput constants used by the 4.9-era kernel on the TSP.
EV_SYN = 0x00
EV_ABS = 0x03
SYN_REPORT = 0x00
...
def configure_uart(path):
    ...
    # Raw 8N1 configuration, matching the daemon's effective setup.
    attrs[0] = 0
    attrs[1] = 0
    attrs[2] = (
...
def create_uinput():
    ...
    # Enable absolute events and the four stick axes.
    ioctl_int(fd, UI_SET_EVBIT, EV_ABS)
    for axis in (ABS_X, ABS_Y, ABS_Z, ABS_RZ):
        ioctl_int(fd, UI_SET_ABSBIT, axis)

    # uinput_user_dev is the legacy ABI supported by this kernel.
    # Layout:
    # name[80], input_id[8], ff_effects_max[4],
    # absmax[64], absmin[64], absfuzz[64], absflat[64].
    name = b"TSP UART Prototype Controller"
    name = name[:79] + b"\0"
    ...
    for axis in (ABS_X, ABS_Y, ABS_Z, ABS_RZ):
        absmax[axis] = AXIS_MAX
        absmin[axis] = AXIS_MIN
    ...
    # 80 + 8 + 4 + 64*4*4 = 1116 bytes.
    if len(user_dev) != 1116:
        raise RuntimeError("Unexpected uinput_user_dev size: %d" % len(user_dev))
...
def main():
    ...
    print("Clamp/deadzone from stock daemon: DISABLED")
