...
echo "Launcher revision: openmw-tsp-pvr-native-lifetime-2026-09-22-v11"
...
tsp_apply_internal_render_scale() {
    ...
    # TSP_PVR_LIFETIME_FORK_V1
    # Original TSP/PowerVR gets the isolated sidecar. TSPS/Mali keeps production.
    TSP_PVR_LIFETIME_SIDE="$GAMEDIR/lib.tsp-pvr-lifetime-v1/libGL.so.1"
    if grep -q '^pvrsrvkm ' /proc/modules 2>/dev/null \
       && ! grep -qi '^mali' /proc/modules 2>/dev/null; then
        if [ ! -r "$TSP_PVR_LIFETIME_SIDE" ]; then
            echo "ERROR: TSP PowerVR lifetime sidecar missing:"
            echo "  $TSP_PVR_LIFETIME_SIDE"
            exit 73
        fi
        export TSP_GL4ES_OVERRIDE="$TSP_PVR_LIFETIME_SIDE"
        echo "TSP_PVR_LIFETIME_FORK_V1 selected=1 lib=$TSP_PVR_LIFETIME_SIDE"
    else
        unset TSP_GL4ES_OVERRIDE
        echo "TSP_PVR_LIFETIME_FORK_V1 selected=0 using production libGL"
    fi
...
# Set this to 0 to keep the TSP-only fix while disabling ALL added native-life diagnostics.
TSP_NATIVE_LIFE_DIAG="${TSP_NATIVE_LIFE_DIAG:-1}"
...
if tsp_map_diag_is_powervr; then
    ...
    # Measurement-only native lifetime diagnostics. The resource-lifetime fix in
    # the TSP sidecar remains active even when this switch is 0.
    if [ "$TSP_NATIVE_LIFE_DIAG" = 1 ]; then
        export LIBGL_TSP_NATIVE_LIFE=1
        export LIBGL_TSP_NATIVE_LIFE_PATH="$TSP_MAP_DIAG_LATEST/native-life.log"
        rm -f "$LIBGL_TSP_NATIVE_LIFE_PATH"
        tsp_native_driver_sampler "$TSP_MAP_DIAG_LATEST/pvr-driver-stats.log" &
        TSP_NATIVE_DRIVER_PID=$!
        echo "TSP_NATIVE_LIFE_DIAG_V1 armed=1 pid=$TSP_NATIVE_DRIVER_PID"
    else
        unset LIBGL_TSP_NATIVE_LIFE
        unset LIBGL_TSP_NATIVE_LIFE_PATH
        echo "TSP_NATIVE_LIFE_DIAG_V1 armed=0"
    fi

    # Existing PowerVR attachment/draw diagnostics.
    ...
    # Existing OpenMW map diagnostics.  Repair mode 1 is report-only.
    export TSP_GMAP_REPAIR=1
    export TSP_GMAP_DUMP="$TSP_MAP_DIAG_LATEST/tsp_globalmap_overlay.png"

    # TSP_GMAP_MEM/TSP_LOAD instrumentation logs at Info level.
...

    {
        ...
        echo "gpu=PowerVR"
        echo "native_life_diag=$TSP_NATIVE_LIFE_DIAG"
        echo "native_life_path=${LIBGL_TSP_NATIVE_LIFE_PATH:-disabled}"
        echo "libgl=$TSP_GL4ES_LIBRARY"
        echo "libgl_sha=$(sha256sum "$TSP_GL4ES_LIBRARY" 2>/dev/null | awk '{print $1}')"
        echo "openmw_sha=$(sha256sum "$OPENMW_BIN" 2>/dev/null | awk '{print $1}')"
        echo "kernel=$(uname -a 2>/dev/null)"
        echo "fbo_trace=$LIBGL_TSP_FBOPATH"
        echo "gmap_dump=$TSP_GMAP_DUMP"
    } > "$TSP_MAP_DIAG_LATEST/identity.txt" 2>&1

    echo "TSP_MAP_DIAG_AUTO_V1 ARMED gpu=PowerVR out=$TSP_MAP_DIAG_LATEST"
else
    unset LIBGL_TSP_NATIVE_LIFE
    unset LIBGL_TSP_NATIVE_LIFE_PATH
