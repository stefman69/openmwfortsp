...
echo "Launcher revision: openmw-portable-zram-loadpurge-diag-2026-09-21-v10"
...
tsp_map_diag_collect() {
    ...
    if [ -f "$GAMEDIR/openmw_log.txt" ]; then
        grep -E \
            'TSP_GMAP_|TSP_RTT_|TSP_MAP_|TSP_LOAD_|TSP_LOADPURGE|TSP_MEMORY_CLEAR|TSP_FOG_|PORTABLE ZRAM:|TSP_MAXCOLORATTACH' \
...
if tsp_map_diag_is_powervr; then
    ...
    # TSP_LOADPURGE_AB_V1
    #
    # This is intentionally TSP / PowerVR ONLY.
    #
    # The currently installed sidecar was built from the same OpenMW source as
    # the fog-no-PBO test and therefore definitely contains TSP_LOADPURGE_V2.
    # With TSP_FOG_NO_PBO unset it follows the normal fog/PBO code path again,
    # so this run changes ONE thing: save-load resource purge is allowed to run.
    TSP_LOADPURGE_BIN="$GAMEDIR/bin/openmw-0.51.tsp-fog-nopbo-v1"
    if [ ! -x "$TSP_LOADPURGE_BIN" ]; then
        echo "ERROR: required TSP OpenMW sidecar is missing:"
        echo "  $TSP_LOADPURGE_BIN"
        echo "Run the previously installed fog-no-PBO sidecar installer first."
        exit 72
    fi

    OPENMW_BIN="$TSP_LOADPURGE_BIN"

    # Return fog uploads to their normal/PBO behavior. The no-PBO A/B did not
    # improve the map and is deliberately NOT part of this test.
    unset TSP_FOG_NO_PBO

    # Current normal launchers export TSP_NO_LOADPURGE=1.  The OpenMW binary
    # interprets that as "skip TSP_LOADPURGE_V2".  Remove it only here, after
    # the normal launch environment has been assembled, so StateManager::cleanup
    # executes:
    #   ResourceSystem::releaseGLObjects(nullptr)
    #   ResourceSystem::clearCache()
    unset TSP_NO_LOADPURGE

    export TSP_LOADPURGE_AB=1

    echo "TSP_LOADPURGE_AB_V1 enabled=1 gpu=PowerVR"
    echo "TSP_LOADPURGE_AB_V1 binary=$OPENMW_BIN"
    echo "TSP_LOADPURGE_AB_V1 TSP_NO_LOADPURGE=${TSP_NO_LOADPURGE-unset}"
    echo "TSP_LOADPURGE_AB_V1 TSP_FOG_NO_PBO=${TSP_FOG_NO_PBO-unset}"
...

    {
        ...
        echo "gpu=PowerVR"
        echo "loadpurge_ab=${TSP_LOADPURGE_AB:-0}"
        echo "no_loadpurge=${TSP_NO_LOADPURGE-unset}"
        echo "fog_no_pbo=${TSP_FOG_NO_PBO-unset}"
        echo "openmw_selected=$OPENMW_BIN"
        echo "libgl=$TSP_GL4ES_LIBRARY"
        echo "libgl_sha=$(sha256sum "$TSP_GL4ES_LIBRARY" 2>/dev/null | awk '{print $1}')"
        echo "openmw_sha=$(sha256sum "$OPENMW_BIN" 2>/dev/null | awk '{print $1}')"
        echo "kernel=$(uname -a 2>/dev/null)"
    ...
    unset TSP_LOADPURGE_AB
    echo "TSP_LOADPURGE_AB_V1 enabled=0 gpu=non-PowerVR"
