...
tsp_apply_internal_render_scale() {
    ...
    # TSP_PRODUCTION_GL_V1
    # The final TSP/TSPS build now ships one production OpenMW binary and one
    # production GL4ES library.  Do not select GPU-specific sidecars here.
    unset TSP_GL4ES_OVERRIDE
    unset TSP_RTTOCC_DIAG

    OPENMW_BIN="$GAMEDIR/bin/openmw-0.51"
    TSP_GL4ES_LIBRARY="$GAMEDIR/lib/libGL.so.1"

    if [ ! -x "$OPENMW_BIN" ]; then
        echo "ERROR: production OpenMW binary missing:"
        echo "  $OPENMW_BIN"
        exit 75
    fi

    if [ ! -r "$TSP_GL4ES_LIBRARY" ]; then
        echo "ERROR: production libGL missing:"
        echo "  $TSP_GL4ES_LIBRARY"
        exit 74
    fi

    echo "TSP_PRODUCTION_GL_V1 OpenMW=$OPENMW_BIN"
    echo "TSP_PRODUCTION_GL_V1 libGL=$TSP_GL4ES_LIBRARY"
...
TSP_PVR_TEXTURE_DIAG="${TSP_PVR_TEXTURE_DIAG:-0}"
