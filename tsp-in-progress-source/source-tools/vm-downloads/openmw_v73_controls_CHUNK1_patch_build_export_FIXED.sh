...
LIVE_HASH="$(ssh -n -o BatchMode=yes "$DEVICE" "test -s '$LIVE_BIN' && sha256sum '$LIVE_BIN' | cut -d ' ' -f1")" \
...
BUILD_HASH="$("${DOCKER[@]}" exec "$CONTAINER" bash -lc \
    "test -s '$BUILD/openmw' && sha256sum '$BUILD/openmw' | cut -d ' ' -f1")" \
...
# OpenMW's version target is intentionally regenerated/check-run during builds.
# Ninja dry-run therefore commonly shows:
#   GitVersion.cmake
#   version/version.cpp.o
#   components/libcomponents.a
#   final openmw relink
# That does NOT mean unrelated source is pending.
#
# Refuse only if an actual unexpected source translation unit would compile.

if printf '%s\n' "$PRE_DRY" | grep -Eq 'localmap\.cpp|mapwindow\.cpp'; then
    echo >&2
    echo "ERROR: pre-patch build would compile map source; refusing." >&2
    exit 10
fi

PRE_BAD_COMPILES="$(
    printf '%s\n' "$PRE_DRY" \
    | grep -E 'Building (C|CXX) object' \
    | grep -Ev 'version/version\.cpp\.o' \
    || true
)"

if [ -n "$PRE_BAD_COMPILES" ]; then
    echo >&2
    echo "ERROR: pre-patch build has unexpected source compilation pending:" >&2
    printf '%s\n' "$PRE_BAD_COMPILES" >&2
    echo >&2
    echo "No source has been modified." >&2
    exit 11
fi

echo "Pre-patch build state: PASS"
echo "Only normal version.cpp/relink work is pending, if anything."
...
# Any real source compile must be either:
#   - OpenMW's harmless generated version.cpp object, or
#   - one of the two intended controls translation units.
BAD_COMPILES="$(
    printf '%s\n' "$DRY" \
    | grep -E 'Building (C|CXX) object' \
    | grep -Ev 'version/version\.cpp\.o|mwinput/(inputmanagerimp|controllermanager)\.cpp\.o' \
    || true
)"

if [ -n "$BAD_COMPILES" ]; then
    echo "ERROR: controls build would compile unexpected source:" >&2
    printf '%s\n' "$BAD_COMPILES" >&2
    exit 31
fi
