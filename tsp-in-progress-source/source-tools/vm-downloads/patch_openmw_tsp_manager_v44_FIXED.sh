...
[ -f "$PORT/OpenMW_Manager.sh" ] || { echo "ERROR $PORT/OpenMW_Manager.sh missing"; exit 7; }'''
preflight_count=s.count(old)
if preflight_count==1:
    s=s.replace(old,new)
    print('PASS: patched original directory-only PORT preflight')
elif preflight_count==0:
    # The user's earlier path-only repair changed the loop body but did not add
    # the V2 comment marker. Accept that real repaired form as already fixed.
    aware=s.count('if [ -f "$candidate/OpenMW_Manager.sh" ]; then')
    if aware>=3 and '/mnt/mmc/ROMS/Ports' in s:
        print(f'PASS: PORT preflight already repaired ({aware} manager-aware selections); leaving it unchanged')
    else:
        raise SystemExit(f'PORT preflight: expected original block or an already-repaired manager-aware layout (found {aware})')
elif preflight_count>1:
    raise SystemExit(f'PORT preflight: expected 0 or 1 original blocks, found {preflight_count}')
