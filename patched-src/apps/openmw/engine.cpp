...
void OMW::Engine::createWindow()
{
    ...
    // TSP_SDL_WINDOW_SCALE_051_V27
    // V25 now performs the real low-resolution scene raster.
    // Create the SDL/EGL window at that same selected size and
    // allow CrossMix/display hardware to scale it to the 1280x720 LCD.
    const int width = tspInternalWidth;
    const int height = tspInternalHeight;
    Log(Debug::Info)
        << "TSP_SDL_WINDOW_SCALE_051_V27"
        << " window=" << width << "x" << height
        << " scene=" << tspInternalWidth << "x" << tspInternalHeight
        << " panel=1280x720"
        << " presentation=crossmix-display-scale";
