...
#   2. tsp_openmw_controls, a merged mouse/text helper based on
...
#   - MENU alone: toggle HYBRID mode.
#   - HYBRID left stick: mouse movement; R1 slows mouse.
#   - HYBRID A/B: left/right mouse click.
#   - HYBRID D-pad Up/Down: previous/next character.
#   - HYBRID D-pad Right: type selected character.
#   - HYBRID X: backspace/delete text.
#   - HYBRID Y: toggle selected character case.
#   - HYBRID Start: Enter/finish text.
#   - START+SELECT held ~2 seconds: emergency terminate openmw-0.51.
#
# The helper holds KEY_PAUSE while HYBRID mode is active. The OpenMW source
# patch reads that persistent state and shows/hides MyGUI's in-engine cursor in GUI mode.
...
OUTPUT_BINARY="${4:-/root/openmw-0.51-tsp-input-graphics-v4}"
...
SCRIPT_REVISION="TSP-051-INPUT-GRAPHICS-HYBRID-V4-2026-08-07"
...
marker = "TSP_MYGUI_CURSOR_051_HYBRID_V2"
cpp = re.sub(
    r"^[ \t]*//[ \t]*TSP_MYGUI_CURSOR[^\n]*\n",
    "",
    cpp,
...
new_update = r'''    // TSP_MYGUI_CURSOR_051_HYBRID_V2
    void InputManager::update(float dt, bool disableControls, bool disableEvents)
    {
        ...
        // tsp_openmw_controls holds Pause for the entire time HYBRID mode
        // is active. Reading the state every frame is more reliable than a
        // short edge-triggered toggle on the TSP/gl4es input path.
...
member_block = (
    ...
    + "\n\n"
    + indent + "// TSP_MYGUI_CURSOR_051_HYBRID_V2\n"
...
static void set_cursor_signal(bool visible)
{
    emit_event(EV_KEY, KEY_PAUSE, visible ? 1 : 0);
    sync_events();
    log_line(visible
        ? "CURSOR SIGNAL: KEY_PAUSE held (cursor ON)."
        : "CURSOR SIGNAL: KEY_PAUSE released (cursor OFF).");
}
...
static void set_mode(control_mode new_mode, bool notify_openmw)
{
    ...
    if (mode == MODE_GAME) {
        if (notify_openmw)
            set_cursor_signal(false);
        set_grab(false);
        log_line("MODE=GAME");
    } else {
        set_grab(true);
        if (notify_openmw)
            set_cursor_signal(true);
...
static void update_mouse(void)
{
    ...
    int dx = scaled_axis(lx, r1_down);
    int dy = scaled_axis(ly, r1_down);
...
static void handle_key_event(const struct input_event *event)
{
    ...
    if (event->code == KEY_TSP_MENU && pressed) {
        log_line("MENU pressed: toggling GAME/HYBRID mode.");
    ...
    // A/B are mouse buttons in HYBRID mode. Preserve press/release so
    // dragging works instead of emitting only a click tap.
    if (event->code == KEY_TSP_A) {
        set_mouse_button(BTN_LEFT, pressed);
        return;
    }
    if (event->code == KEY_TSP_B) {
...
static void handle_abs_event(const struct input_event *event)
{
    ...
    switch (event->code) {
        ...
        case ABS_TSP_RX:
        case ABS_TSP_RY:
            // Right stick intentionally unused in HYBRID mode.
            break;
        case ABS_TSP_DPAD_X: {
...
int main(int argc, char **argv)
{
    ...
    log_line("TSP OpenMW 0.51 hybrid control helper v2 started.");
    log_line("GAME mode: physical controller passes through to OpenMW native input.");
    log_line("MENU alone: toggle HYBRID mouse/text mode and MyGUI cursor state.");
    log_line("HYBRID left stick: mouse; R1: slow mouse; A/B: left/right click.");
    log_line("HYBRID D-pad Up/Down: character; Right: type selected character.");
    log_line("HYBRID X: backspace/delete; Y: toggle case.");
    log_line("HYBRID Start: Enter/finish text. Start/Select physical codes corrected.");
...
grep -n "TSP_MYGUI_CURSOR_051_HYBRID_V2" "$CPP" "$HPP"
echo
...
echo "alongside OpenMW for MENU-alone hybrid mode and the corrected controls."
echo "============================================================"
