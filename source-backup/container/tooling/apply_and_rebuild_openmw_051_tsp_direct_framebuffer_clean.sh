...
SCRIPT_REVISION="TSP-051-DIRECT-FRAMEBUFFER-CLEAN-2026-08-07"
...
echo "Artifact: direct-framebuffer-clean (new filename / cache break)"
echo "============================================================"
...
python3 - <<'PY_WRITE_PREFLIGHT'
from pathlib import Path
import tempfile

path = Path(tempfile.gettempdir()) / "openmw51_direct_fb_write_preflight.txt"
try:
    with path.open("w", encoding="utf-8", newline="\n") as handle:
        handle.write("ok\n")
    if path.read_text(encoding="utf-8") != "ok\n":
        raise RuntimeError("transactional write preflight returned unexpected content")
finally:
    try:
        path.unlink()
