...
static void update_mouse(void)
{
    ...
    if (dpad_x != 0) {
        dx = dpad_x * (r1_down ? 2 : 7);
    }

    if (dpad_y != 0) {
        dy = dpad_y * (r1_down ? 2 : 7);
    }
...
static void handle_abs_event(const struct input_event *event)
{
    ...
    switch (event->code) {
        ...
        case ABS_TSP_R2: {
            ...
            if (new_r2_down && !r2_down && mode != MODE_GAME) {
                if (mode == MODE_MOUSE) {
                    set_mode(MODE_TEXT);
                } else {
                    set_mode(MODE_MOUSE);
...
int main(int argc, char **argv)
{
    ...
    log_line("TSP OpenMW control helper v2 started.");
    log_line("Diagnostic logging enabled.");
    log_line("Mode values: 0=GAME 1=MOUSE 2=TEXT");
    log_line("Select release: GAME <-> MOUSE");
    log_line("R2 press in MOUSE/TEXT: MOUSE <-> TEXT");
    set_mode(MODE_GAME);
    ...
    log_line("TSP OpenMW control helper v2 stopped.");
