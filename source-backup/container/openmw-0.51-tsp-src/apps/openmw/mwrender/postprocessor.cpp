...
namespace
{
    ...
    constexpr bool TspLegacyDirectRender = true;
}
...
namespace MWRender
{
        ...
        , mDistortionCallback(new DistortionCallback)
    {
        ...
        // TSP_LEGACY_DIRECT_RENDER_CONSTRUCTOR_051_V2
        // OpenMW 0.48 supported leaving the Viewer on its original scene root
        // when no FBO-only depth/post effects were required. Recreate that
        // behavior for the TSP GL4ES/GLES2 compatibility build.
        if (TspLegacyDirectRender)
        {
            ...
            Log(Debug::Info)
                << "TSP GL4ES: legacy direct framebuffer path active; main scene FBO/ping-pong presentation bypassed.";
            return;
        }
    ...
    {
        // TSP_LEGACY_DIRECT_RENDER_RESIZE_GUARD_051_V2
        if (TspLegacyDirectRender)
        {
            osg::GraphicsContext* gc = mViewer->getCamera()->getGraphicsContext();
            mWidth = gc->getTraits()->width;
        ...
        // TSP_SCENE_RASTER_RESIZE_051_V25
    ...
    {
        // TSP_LEGACY_DIRECT_RENDER_ENABLE_GUARD_051_V2
        if (TspLegacyDirectRender)
        {
            mUsePostProcessing = false;
            return;
    ...
    {
        // TSP_LEGACY_DIRECT_RENDER_DISABLE_GUARD_051_V2
        if (TspLegacyDirectRender)
        {
            mUsePostProcessing = false;
            return;
    ...
    {
        // TSP_LEGACY_DIRECT_RENDER_TRAVERSE_GUARD_051_V2
        if (TspLegacyDirectRender)
        {
    ...
    {
        // TSP_LEGACY_DIRECT_RENDER_RELOAD_GUARD_051_V4
        if (TspLegacyDirectRender)
            return;
