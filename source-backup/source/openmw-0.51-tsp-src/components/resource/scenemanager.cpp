...
namespace Resource
{
    ...
    void SceneManager::setIncrementalCompileOperation(osgUtil::IncrementalCompileOperation* ico)
    {
        ...
        /* TSP_SHADER_PRECOMPILE: give the ShaderManager the same ICO so newly
           created programs are compiled at load instead of on first draw. */
        if (mShaderManager)
            mShaderManager->setIncrementalCompileOperation(ico);
    }
    ...
    void SceneManager::updateCache(double referenceTime)
    {
        ...
        if (mIncrementalCompileOperation)
        {
            ...
            for (osgUtil::IncrementalCompileOperation::CompileSets::iterator it = sets.begin(); it != sets.end();)
            {
                ...
                // ShaderManager queues a throwaway Geode per new program so the
                // link happens during the loading screen. Nothing else ever holds
                // that node, so the refcount test below always fires and the
                // precompile is cancelled before it runs. Skip those sets; they
                // remove themselves from the ICO once compiled.
                bool tspIsPrecompile = false;
                if ((*it)->_subgraphToCompile.valid())
                    (*it)->_subgraphToCompile->getUserValue("tspShaderPrecompile", tspIsPrecompile);
                if (tspIsPrecompile)
                {
                    static int tspKept = 0;
                    if (++tspKept <= 5 || (tspKept % 50) == 0)
                        Log(Debug::Info) << "TSP_SHADER_PRECOMPILE_KEEP_V47 kept=" << tspKept
                                         << " pending=" << sets.size();
                    ++it;
                    continue;
                }
