...
namespace MWWorld
{
    ...
    void Scene::tspUpdateRoomObjectLifecycle(float duration)
    {
        if (mCurrentCell != mTspRoomLifecycleCell)
        {
            ...
            MWRender::clearInteriorClutterResidency();
        ...
        // TSP_ROOM_OBJECT_SEPARATE_CLUTTER_051_V28
        const bool clutterEnabled = MWRender::isInteriorClutterResidencyEnabled();
        ...
        mCurrentCell->forEach([&](const Ptr& ptr) {
            ...
            // TSP_ROOM_OBJECT_SEPARATE_CLUTTER_051_V28
            const bool shouldLive = MWRender::isInteriorClutterObjectResident(origin);
        ...
        if (mTspRoomLogAccumulator >= 1.f)
        {
            mTspRoomLogAccumulator = 0.f;
            Log(Debug::Info)
                << "[TSP_ROOMOBJ_V28] clutter=" << (clutterEnabled ? 1 : 0)
                << " eligible=" << eligible
                << " resident=" << resident
                << " inactive=" << inactive
    ...
    void Scene::unloadCell(CellStore* cell, const DetourNavigator::UpdateGuard* navigatorUpdateGuard)
    {
        ...
        if (cell == mTspRoomLifecycleCell && !mTspRoomSuppressedRefs.empty())
        {
            ...
            mTspRoomLogAccumulator = 0.f;
            MWRender::clearInteriorClutterResidency();
            Log(Debug::Info) << "[TSP_ROOMOBJ_V28] unload-purge parked=" << tspParked;
        }
