...
namespace MWRender
{
    namespace
    {
        ...
        // TSP_ROOM_OBJECT_SEPARATE_CLUTTER_051_V28
        // Independent room mask for hard scene-side clutter residency.
        // Structural/fog PVS may fail open without resurrecting every clutter ref.
        std::atomic<bool> sClutterEnabled{ false };
        std::atomic<int> sClutterSectorCount{ 0 };
        std::atomic<int> sClutterActiveCount{ 0 };
        std::atomic<std::uint64_t> sClutterVisibleMask{ 0 };
        std::array<std::atomic<float>, sPvsMaxSectors * sPvsBoxStride> sClutterBoxes{};
    ...
    // TSP_ROOM_OBJECT_SEPARATE_CLUTTER_051_V28
    void setInteriorClutterResidency(
        std::span<const float> boxes, std::span<const int> activeSectorIds)
    {
        if (boxes.empty() || boxes.size() % sPvsBoxStride != 0)
        {
            clearInteriorClutterResidency();
            return;
        }
        ...
        const bool wasEnabled = sClutterEnabled.exchange(false, std::memory_order_acq_rel);
        for (std::size_t i = 0; i < boxes.size(); ++i)
            sClutterBoxes[i].store(boxes[i], std::memory_order_relaxed);
        sClutterSectorCount.store(sectorCount, std::memory_order_relaxed);
        sClutterActiveCount.store(activeCount, std::memory_order_relaxed);
        sClutterVisibleMask.store(visibleMask, std::memory_order_relaxed);
        sClutterEnabled.store(true, std::memory_order_release);

        if (!wasEnabled)
            Log(Debug::Info) << "[TSP_CLUTTER_V28] active sectors=" << sectorCount
                             << " resident=" << activeCount;
    }

    void clearInteriorClutterResidency()
    {
        const bool wasEnabled = sClutterEnabled.exchange(false, std::memory_order_acq_rel);
        sClutterSectorCount.store(0, std::memory_order_relaxed);
        sClutterActiveCount.store(0, std::memory_order_relaxed);
        sClutterVisibleMask.store(0, std::memory_order_relaxed);
        if (wasEnabled)
            Log(Debug::Info) << "[TSP_CLUTTER_V28] inactive";
    }

    bool isInteriorClutterResidencyEnabled()
    {
        return sClutterEnabled.load(std::memory_order_acquire);
    }

    bool isInteriorClutterObjectResident(const osg::Vec3f& origin)
    {
        // Only the dedicated clutter bridge controls this fail-open. Structural
        // PVS/grid/fog enable state is intentionally irrelevant here.
        if (!sClutterEnabled.load(std::memory_order_acquire))
            return true;

        const int sectorCount = sClutterSectorCount.load(std::memory_order_relaxed);
        const std::uint64_t visibleMask = sClutterVisibleMask.load(std::memory_order_relaxed);
        if (sectorCount <= 0 || sectorCount > sPvsMaxSectors || visibleMask == 0)
            return true;
        ...
        {
            const int base = sector * sPvsBoxStride;
            const float minX = sClutterBoxes[static_cast<std::size_t>(base + 0)].load(std::memory_order_relaxed);
            const float minY = sClutterBoxes[static_cast<std::size_t>(base + 1)].load(std::memory_order_relaxed);
            const float minZ = sClutterBoxes[static_cast<std::size_t>(base + 2)].load(std::memory_order_relaxed);
            const float maxX = sClutterBoxes[static_cast<std::size_t>(base + 3)].load(std::memory_order_relaxed);
            const float maxY = sClutterBoxes[static_cast<std::size_t>(base + 4)].load(std::memory_order_relaxed);
            const float maxZ = sClutterBoxes[static_cast<std::size_t>(base + 5)].load(std::memory_order_relaxed);
            if (!std::isfinite(minX) || !std::isfinite(minY) || !std::isfinite(minZ)
                || !std::isfinite(maxX) || !std::isfinite(maxY) || !std::isfinite(maxZ))
                continue;

            // Same V27 object-to-room ownership that restored Balmora clutter:
            // ZERO XY bleed, vertical slack for shelf/tabletop objects.
            constexpr float zSlack = 110.f;
            const bool overlaps = origin.x() >= minX && origin.x() <= maxX
                && origin.y() >= minY && origin.y() <= maxY
                && origin.z() >= minZ - zSlack && origin.z() <= maxZ + zSlack;
        ...
        // Genuinely unmapped content still fails OPEN. The crucial difference
        // from V27 is that a structural PVS clear no longer causes this path.
        return true;
    }
