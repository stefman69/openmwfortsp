...
namespace MWWorld
{
        ...
        SceneUtil::WorkQueue* workQueue, SceneUtil::UnrefQueue& unrefQueue)
    {
        ...
        // TSP_SAFENAV_FORCE_STUB_051_V10
        const char* tspEnableNavigator = std::getenv("OPENMW_TSP_ENABLE_NAVIGATOR");
        const bool tspNavigatorOptIn = tspEnableNavigator != nullptr
            && tspEnableNavigator[0] == '1' && tspEnableNavigator[1] == '\0';

        if (tspNavigatorOptIn && Settings::navigator().mEnable)
        {
        ...
        else
        {
            if (Settings::navigator().mEnable)
                Log(Debug::Info) << "TSP SafeNav v10: Navigator forced to stub; "
                                 << "set OPENMW_TSP_ENABLE_NAVIGATOR=1 to opt back in.";
    ...
    // TSP_WORLDCLEAR_MEM_V1
    static long long tspWorldClearInuseKb()
    {
    ...
    void World::clear()
    {
        ...
        // TSP_WORLDCLEAR_MEM_V1 - per-step accounting for the save-reload leak
        long long tspWc = tspWorldClearInuseKb();
        const long long tspWc0 = tspWc;
        auto tspStep = [&tspWc](const char* tspName) {
            const long long tspNow = tspWorldClearInuseKb();
            Log(Debug::Info) << "TSP_WORLDCLEAR_MEM_V1 step=" << tspName
                             << " freed_kb=" << (tspWc - tspNow) << " inuse_kb=" << tspNow;
            tspWc = tspNow;
        };
        ...
        Log(Debug::Info) << "TSP_WORLDCLEAR_MEM_V1 total_freed_kb=" << (tspWc0 - tspWc);
    }
