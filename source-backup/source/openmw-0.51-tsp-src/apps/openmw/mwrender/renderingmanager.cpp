...
namespace
{
    ...
    class TspActualRenderResolutionProbe final : public osg::Camera::DrawCallback
    {
    public:
        ...
        void operator()(osg::RenderInfo& renderInfo) const override
        {
            ...
            osg::Viewport* cameraViewport = camera ? camera->getViewport() : nullptr;
            osg::GraphicsContext* graphicsContext
                = camera ? camera->getGraphicsContext() : nullptr;
            const osg::GraphicsContext::Traits* traits
                = graphicsContext ? graphicsContext->getTraits() : nullptr;

            const int cameraWidth
                = cameraViewport ? static_cast<int>(cameraViewport->width()) : -1;
            const int cameraHeight
                = cameraViewport ? static_cast<int>(cameraViewport->height()) : -1;
            const int physicalWidth = traits ? traits->width : -1;
            const int physicalHeight = traits ? traits->height : -1;

            const char* libglFbo = std::getenv("LIBGL_FBO");
            const char* libglOutput = std::getenv("LIBGL_TSP_OUTPUT");

            Log(Debug::Info)
                << "TSP_ACTUAL_RENDER_RES_051_V25"
                << " requested=" << mRequestedWidth << "x" << mRequestedHeight
                << " camera_viewport=" << cameraWidth << "x" << cameraHeight
                << " gl_viewport=" << glViewport[2] << "x" << glViewport[3]
                << " gl_viewport_xy=" << glViewport[0] << "," << glViewport[1]
                << " gl_scissor=" << glScissor[2] << "x" << glScissor[3]
                << " physical_traits=" << physicalWidth << "x" << physicalHeight
                << " framebuffer_binding=" << framebufferBinding
                << " LIBGL_FBO=" << (libglFbo ? libglFbo : "<unset>")
...
namespace MWRender
{
        ...
        , mGroundCoverStore(groundcoverStore)
    {
        ...
        const bool tspUseIncrementalCompile = tspIncrementalCompileEnabled();
        Log(Debug::Info) << "TSP_WARM_ASYNC_GUARD_051_V14 incremental_compile="
                         << (tspUseIncrementalCompile ? 1 : 0);
        if (tspUseIncrementalCompile && std::getenv("OPENMW_DONT_PRECOMPILE") == nullptr)
        {
            mViewer->setIncrementalCompileOperation(new osgUtil::IncrementalCompileOperation);
            mViewer->getIncrementalCompileOperation()->setTargetFrameRate(Settings::cells().mTargetFramerate);
            // TSP_ICO_BUDGET_V1
            // This OSG build does not read OSG_MAXIMUM_NUM_OF_OBJECTS_TO_COMPILE_PER_FRAME,
            // so set the incremental-compile budget directly. Measured: 'Compiling 15'
            // draining into one frame alongside a 30-50 ms draw traversal.
            {
                osgUtil::IncrementalCompileOperation* tspIco = mViewer->getIncrementalCompileOperation();
                const char* tspIcoMax = std::getenv("TSP_ICO_MAXOBJ");
                if (tspIcoMax != nullptr && tspIcoMax[0] != '\0')
                    tspIco->setMaximumNumOfObjectsToCompilePerFrame(std::max(1, std::atoi(tspIcoMax)));
                const char* tspIcoMin = std::getenv("TSP_ICO_MINTIME");
                if (tspIcoMin != nullptr && tspIcoMin[0] != '\0')
                    tspIco->setMinimumTimeAvailableForGLCompileAndDeletePerFrame(std::atof(tspIcoMin));
                Log(Debug::Info) << "TSP_ICO_BUDGET_V1 maxobj="
                                 << tspIco->getMaximumNumOfObjectsToCompilePerFrame()
                                 << " mintime="
                                 << tspIco->getMinimumTimeAvailableForGLCompileAndDeletePerFrame();
            }
        }
        ...
        /* TSP_SHADER_WARMDRAW: attach the shader warm-up group so the throwaway
        ...
        // TSP_GL4ES_SCENEMANAGER_SCENE_DEPTH_051
        // Supply the primary scene depth textures to the global depth slot on GL4ES.
        // TSP_LEGACY_DIRECT_RENDER_NO_OPAQUE_DEPTH_051_V2
        // Direct framebuffer mode has no scene-depth texture for shader sampling.
        resourceSystem->getSceneManager()->setOpaqueDepthTex(nullptr, nullptr);
        // TSP_GL4ES_DISABLE_NORMALS_RT_051
        // Indexed color masks (glColorMaski) are unavailable on this GL4ES/GLES2 path.
        // TSP_LEGACY_DIRECT_RENDER_NO_NORMALS_RT_051_V2
        resourceSystem->getSceneManager()->setSupportsNormalsRT(false);
        // TSP_LEGACY_DIRECT_RENDER_NO_WEATHER_DEPTH_051_V2
        resourceSystem->getSceneManager()->setWeatherParticleOcclusion(false);
        ...
        // TSP_LEGACY_DIRECT_RENDER_NO_SOFT_PARTICLES_051_V2
        ...
        const osg::GraphicsContext::Traits* tspTraits
        ...
        Log(Debug::Info)
            << "TSP_SCENE_RASTER_VIEWPORT_051_V25"
            << " scene=" << tspInternalWidth << "x" << tspInternalHeight
            << " physical="
            << (tspTraits ? tspTraits->width : -1)
            << "x"
            << (tspTraits ? tspTraits->height : -1);
    ...
    void RenderingManager::update(float dt, bool paused)
    {
        ...
        // TSP_DEPTH_PARTITION_051_V14_NEARFAR
        stateUpdater->setNearFar(tspWorldNearClip(mNearClip, mViewDistance), mViewDistance);
    ...
    // TSP_PLAYERANIM_MEM_V1
    static long long tspPlayerAnimInuseKb()
    {
    ...
    void RenderingManager::renderPlayer(const MWWorld::Ptr& player)
    {
        ...
        const long long tspM0 = tspPlayerAnimInuseKb();
        const bool tspHadPrev = mPlayerAnimation.valid();
        Log(Debug::Info) << "TSP_PLAYERANIM_MEM_V1 phase=entry had_prev="
                         << (tspHadPrev ? 1 : 0) << " inuse_kb=" << tspM0;
        ...
        const long long tspM1 = tspPlayerAnimInuseKb();
        Log(Debug::Info) << "TSP_PLAYERANIM_MEM_V1 phase=constructed alloc_kb="
                         << (tspM1 - tspM0) << " inuse_kb=" << tspM1;

        mCamera->setAnimation(tspNewAnimation.get());
        mCamera->attachTo(player);
        mPlayerAnimation = tspNewAnimation;

        const long long tspM2 = tspPlayerAnimInuseKb();
        tspPreviousAnimation = nullptr;
        const long long tspM3 = tspPlayerAnimInuseKb();
        Log(Debug::Info) << "TSP_PLAYERANIM_MEM_V1 phase=old-released freed_kb="
                         << (tspM2 - tspM3) << " inuse_kb=" << tspM3
                         << " net_kb=" << (tspM3 - tspM0);
    }
    ...
    void RenderingManager::updateProjectionMatrix()
    {
        ...
        const bool tspPartition = tspDepthPartitionEnabled() && !SceneUtil::AutoDepth::isReversed();
        const float tspFarPassNear = tspWorldNearClip(mNearClip, mViewDistance);
        const float tspNearPassFar = tspNearPassFarClip(tspFarPassNear, mViewDistance);
        
        osg::Matrix unreversedProjectionMatrix
            = osg::Matrix::perspective(fov, aspect, tspFarPassNear, mViewDistance);
        
        Log(Debug::Info) << "TSP_DEPTH_PARTITION_051_V14"
                         << " enabled=" << (tspPartition ? 1 : 0)
                         << " configured_near=" << mNearClip
                         << " far_pass_near=" << tspFarPassNear
                         << " near_pass_far=" << tspNearPassFar
                         << " far=" << mViewDistance
                         << " far_ratio="
                         << (tspFarPassNear > 0.f ? mViewDistance / tspFarPassNear : 0.f);
        ...
        if (tspPartition)
        {
            osg::Camera* masterCamera = mViewer->getCamera();
        
            if (!sTspNearDepthCamera)
            {
                sTspNearDepthCamera = new osg::Camera;
                sTspNearDepthCamera->setName("TSP V14 near depth partition");
                sTspNearDepthCamera->setGraphicsContext(masterCamera->getGraphicsContext());
                sTspNearDepthCamera->setViewport(masterCamera->getViewport());
                sTspNearDepthCamera->setRenderTargetImplementation(masterCamera->getRenderTargetImplementation());
                sTspNearDepthCamera->setDrawBuffer(masterCamera->getDrawBuffer());
                sTspNearDepthCamera->setReadBuffer(masterCamera->getReadBuffer());
                sTspNearDepthCamera->setClearMask(GL_DEPTH_BUFFER_BIT);
                sTspNearDepthCamera->setClearDepth(1.0);
                sTspNearDepthCamera->setComputeNearFarMode(
                    osg::CullSettings::DO_NOT_COMPUTE_NEAR_FAR);
                sTspNearDepthCamera->setCullMask(masterCamera->getCullMask());
                sTspNearDepthCamera->setAllowEventFocus(false);
                sTspNearDepthCamera->setRenderOrder(osg::Camera::POST_RENDER, 10);
        
                mViewer->addSlave(
                    sTspNearDepthCamera.get(), osg::Matrixd(), osg::Matrixd(), true);
        
                Log(Debug::Info)
                    << "TSP_DEPTH_PARTITION_051_V14 near_camera_created=1";
            }
            ...
            const osg::Matrixd tspNearProjection
                = osg::Matrixd::perspective(fov, aspect, mNearClip, tspNearPassFar);
            osg::Matrixd tspInverseFar;
            if (!tspInverseFar.invert(osg::Matrixd(unreversedProjectionMatrix)))
                Log(Debug::Warning)
                    << "TSP_DEPTH_PARTITION_051_V14 inverse_far_projection_failed=1";
            else if (osgViewer::View::Slave* tspSlave
                = mViewer->findSlaveForCamera(sTspNearDepthCamera.get()))
            {
                tspSlave->_projectionOffset = tspInverseFar * tspNearProjection;
                tspSlave->_viewOffset.makeIdentity();
                tspSlave->_useMastersSceneData = true;
            }
        }
        else if (sTspNearDepthCamera)
        {
            bool tspRemovedNearSlave = false;
            for (unsigned int tspSlaveIndex = 0;
                 tspSlaveIndex < mViewer->getNumSlaves();
                 ++tspSlaveIndex)
            {
                if (mViewer->getSlave(tspSlaveIndex)._camera.get()
                    == sTspNearDepthCamera.get())
                {
                    mViewer->removeSlave(tspSlaveIndex);
                    tspRemovedNearSlave = true;
                    break;
                }
            }
        
            Log(Debug::Info)
                << "TSP_DEPTH_PARTITION_051_V14 near_camera_removed="
                << (tspRemovedNearSlave ? 1 : 0);
        
            sTspNearDepthCamera = nullptr;
        }
