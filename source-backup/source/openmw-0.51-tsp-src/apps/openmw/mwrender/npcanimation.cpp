...
namespace MWRender
{
    ...
    public:
            ...
            osgUtil::RenderBin* bin, osg::RenderInfo& renderInfo, osgUtil::RenderLeaf*& previous) override
        {
            ...
            // TSP_DEPTHCLEAR_NULLGUARD_051_V44
            // getCurrentCamera()->getUserData() is only non-null on the camera that
            // PostProcessor registered itself on (postprocessor.cpp:246), and it
            // clears that user data again on teardown (postprocessor.cpp:152). The
            // TSP resolution split puts a second camera in the graph, so this bin
            // can be drawn under a camera that never carried the pointer. Upstream
            // dereferences it unconditionally, which is a SIGSEGV at a small
            // offset off null - measured as fault addr 0x170, 13.2 s in, on the
            // first cell that renders a first-person weapon.
            //
            // Degrade instead of dying: with no post-processor there is no
            // FirstPerson FBO to bind, so clear depth on whatever framebuffer is
            // already current and draw the bin once, normally. The weapon loses
            // its separate depth-accumulation pass; nothing else changes.
            osg::ref_ptr<osg::FrameBufferObject> tspFirstPersonFbo;
            osg::ref_ptr<osg::FrameBufferObject> tspPrimaryFbo;
            osg::ref_ptr<osg::FrameBufferObject> tspOpaqueDepthFbo;
            if (postProcessor != nullptr)
            {
                tspFirstPersonFbo = postProcessor->getFbo(PostProcessor::FBO_FirstPerson, frameId);
                tspPrimaryFbo = postProcessor->getPrimaryFbo(frameId);
                tspOpaqueDepthFbo = postProcessor->getFbo(PostProcessor::FBO_OpaqueDepth, frameId);
            }

            if (!tspFirstPersonFbo || !tspPrimaryFbo)
            {
                static bool tspWarned = false;
                if (!tspWarned)
                {
                    tspWarned = true;
                    Log(Debug::Warning)
                        << "TSP_DEPTHCLEAR_NULLGUARD_051_V44 postProcessor="
                        << (postProcessor != nullptr ? 1 : 0)
                        << " firstPersonFbo=" << (tspFirstPersonFbo ? 1 : 0)
                        << " primaryFbo=" << (tspPrimaryFbo ? 1 : 0)
                        << " - first-person depth pass skipped for this camera";
                }
            ...
            tspFirstPersonFbo->apply(*state);
            glClear(GL_DEPTH_BUFFER_BIT | GL_STENCIL_BUFFER_BIT);
            // color accumulation pass
            bin->drawImplementation(renderInfo, previous);

            auto primaryFBO = tspPrimaryFbo;
            primaryFBO->apply(*state);

            if (tspOpaqueDepthFbo)
                tspOpaqueDepthFbo->apply(*state);
