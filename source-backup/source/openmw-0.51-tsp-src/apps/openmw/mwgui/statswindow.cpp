...
namespace MWGui
{
        ...
        , mMinFullWidth(mMainWidget->getSize().width)
    {
        ...
        // TSP_STATS_READABLE_ROWS_051_V35_3
        // Keep Stats compact enough to fit but never put 28px glyphs
        // inside an 18px-high widget.
        constexpr int tspStatsRowHeight = 27;

        MyGUI::IntCoord coord{ 0, 0, 204, tspStatsRowHeight };
        ...
        for (const ESM::Attribute& attribute : store.get<ESM::Attribute>())
        {
            ...
            auto* name = box->createWidget<MyGUI::TextBox>(
                "SandText",
                { 0, 0, 160, tspStatsRowHeight },
                alignment);
            name->setFontHeight(Settings::gui().mTspStatsFontSize);
            name->setNeedMouseFocus(false);
            name->setCaption(attribute.mName);

            auto* value = box->createWidget<MyGUI::TextBox>(
                "SandTextRight",
                { 160, 0, 44, tspStatsRowHeight },
                MyGUI::Align::Right | MyGUI::Align::Top);
            value->setFontHeight(Settings::gui().mTspStatsFontSize);
    ...
    void StatsWindow::addGroup(std::string_view label, MyGUI::IntCoord& coord1, MyGUI::IntCoord& coord2)
    {
        MyGUI::TextBox* groupWidget = mSkillView->createWidget<MyGUI::TextBox>("SandBrightText",
            ...
            MyGUI::Align::Left | MyGUI::Align::Top | MyGUI::Align::HStretch);
        groupWidget->setFontHeight(Settings::gui().mTspSkillListFontSize);
        ...
        const std::string& value, const std::string& state, MyGUI::IntCoord& coord1, MyGUI::IntCoord& coord2)
    {
        ...
        skillNameWidget->setFontHeight(Settings::gui().mTspSkillListFontSize);
        ...
        skillValueWidget->setFontHeight(Settings::gui().mTspSkillListFontSize);
    ...
    void StatsWindow::updateSkillArea()
    {
        ...
        // TSP_STATS_READABLE_ROWS_051_V35_3
        constexpr int tspStatsSkillRowHeight = 27;

        MyGUI::IntCoord coord1(
            10,
            0,
            mSkillView->getWidth() - (10 + valueSize) - 24,
            tspStatsSkillRowHeight);

        MyGUI::IntCoord coord2(
            coord1.left + coord1.width,
            coord1.top,
            valueSize,
            coord1.height);
