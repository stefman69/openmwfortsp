...
namespace MWGui
{
    namespace
    {
        // TSP_DIALOGUE_FONT_SETTING_051_20260813
        //
        // settings.cfg:
        //
        // [GUI]
        // dialogue font size = 22
        //
        // Only the large Dialogue text regions use this.
        int getTspDialogueFontSize()
        {
            int size = Settings::Manager::getOrDefault<int>(
                "dialogue font size", "GUI", 22);
        ...
        std::unordered_map<std::string, std::unique_ptr<Link>>& topicLinks) const
    {
        ...
        if (!mTitle.empty())
        {
            BookTypesetter::Style* title = typesetter->createStyle({}, colors.header, true);
        ...
        BookTypesetter::Style* textStyle = typesetter->createStyle({}, colors.normal, true);
        ...
        std::unordered_map<std::string, std::unique_ptr<Link>>&) const
    {
        ...
        BookTypesetter::Style* title = typesetter->createStyle({}, textColour, true);
        ...
        , mGreetingCallback(std::make_unique<ResponseCallback>(this, false))
    {
        ...
        // History view
        // TSP_DIALOGUE_READABLE_HISTORY_051_20260813
    ...
    void DialogueWindow::updateTopicsPane()
    {
        ...
        if (mPtr.getType() == ESM::NPC::sRecordId)
            mTopicsList->addItem(gmst.find("sPersuasion")->mValue.getString(), 0, getTspDialogueFontSize());

        if (services & ESM::NPC::AllItems)
            mTopicsList->addItem(gmst.find("sBarter")->mValue.getString(), 0, getTspDialogueFontSize());

        if (services & ESM::NPC::Spells)
            mTopicsList->addItem(gmst.find("sSpells")->mValue.getString(), 0, getTspDialogueFontSize());

        if (travel)
            mTopicsList->addItem(gmst.find("sTravel")->mValue.getString(), 0, getTspDialogueFontSize());

        if (services & ESM::NPC::Spellmaking)
            mTopicsList->addItem(gmst.find("sSpellmakingMenuTitle")->mValue.getString(), 0, getTspDialogueFontSize());

        if (services & ESM::NPC::Enchanting)
            mTopicsList->addItem(gmst.find("sEnchanting")->mValue.getString(), 0, getTspDialogueFontSize());

        if (services & ESM::NPC::Training)
            mTopicsList->addItem(gmst.find("sServiceTrainingTitle")->mValue.getString(), 0, getTspDialogueFontSize());

        if (services & ESM::NPC::Repair)
            mTopicsList->addItem(gmst.find("sRepair")->mValue.getString(), 0, getTspDialogueFontSize());

        if (isCompanion())
            mTopicsList->addItem(gmst.find("sCompanionShare")->mValue.getString(), 0, getTspDialogueFontSize());
        ...
        for (const auto& keyword : mKeywords)
        {
            ...
            mTopicsList->addItem(keyword, sVerticalPadding, getTspDialogueFontSize());
    ...
    void DialogueWindow::updateHistory(bool scrollbar)
    {
        ...
        std::shared_ptr<BookTypesetter> typesetter
            = BookTypesetter::create(mHistory->getWidth(), std::numeric_limits<int>::max(), getTspDialogueFontSize());

        for (const auto& text : mHistoryContents)
            text->write(typesetter, mKeywordSearch, mTopicLinks);

        BookTypesetter::Style* body = typesetter->createStyle({}, MyGUI::Colour::White, true);
    ...
    void DialogueWindow::setControllerFocus(size_t index, bool focused)
    {
        ...
        if (focused)
        {
            // TSP_DIALOGUE_TOPIC_SCROLL_051_V62 -- was a hand-rolled offset sum that started
            // at i = 6 (assuming six entries always fit), added
            // sVerticalPadding to service topics that were added with 0, and
            // never clamped. Any topic wrapping to two lines pushed the
            // highlight out of view. MWList::ensureItemVisible already does
            // this from real widget geometry, clamped to the canvas.
            if (index < mTopicsList->getItemCount())
            {
                const std::string& tspFocusKeyword = mTopicsList->getItemNameAt(index);
                if (!tspFocusKeyword.empty())
                    mTopicsList->ensureItemVisible(
                        mTopicsList->getItemWidget(tspFocusKeyword));
            }
        }
    }
