...
namespace
{
    ...
    // TSP_LEVELUP_LAYOUT_051_V63 -- every 20 below was chosen for the stock 16px
    // font. Derive them instead so rows stop overlapping when the font grows.
    // sColumnOffsets stays as-is: moving column 2 needs the dialog width,
    // which lives in the layout, so the font key keeps the columns fitting.
    int tspLevelupFontSize()
    {
        int size = Settings::Manager::getOrDefault<int>("levelup font size", "GUI", 18);
        if (size < 8)
            size = 8;
        else if (size > 32)
            size = 32;
    ...
    int tspLevelupRowStep()
    {
        return tspLevelupFontSize() + 6;
    }
    int tspLevelupMultiplierGap()
    {
        return tspLevelupFontSize() + 6;
    }
}
namespace MWGui
{
        ...
        , mCoinCount(sMaxCoins)
    {
...

        {
            ...
            for (const ESM::Attribute& attribute : store)
            {
                ...
                Widgets widgets;
                // TSP_LEVELUP_LAYOUT_051_V63
                const int tspRowStep = tspLevelupRowStep();
                const int tspGap = tspLevelupMultiplierGap();
                widgets.mMultiplier = mAssignWidget->createWidget<MyGUI::TextBox>(
                    "SandTextVCenter", { offset, tspRowStep * row, 100, tspRowStep },
                    MyGUI::Align::Default);
                widgets.mMultiplier->setFontHeight(tspLevelupFontSize());
                auto* hbox = mAssignWidget->createWidget<Gui::HBox>(
                    {}, { offset + tspGap, tspRowStep * row, 200, tspRowStep },
                    MyGUI::Align::Default);
                ...
                widgets.mButton->setFontHeight(tspLevelupFontSize());
                widgets.mValue = hbox->createWidget<Gui::AutoSizedTextBox>("SandText", {}, MyGUI::Align::Default);
                widgets.mValue->setFontHeight(tspLevelupFontSize());
            ...
            mAssignWidget->setCanvasSize(MyGUI::IntSize(
                mAssignWidget->getWidth(), std::max(mAssignWidget->getHeight(), static_cast<int>(tspLevelupRowStep() * mPerCol))));
    ...
    void LevelupDialog::assignCoins()
    {
        ...
        for (size_t i = 0; i < mSpentAttributes.size(); ++i)
        {
            ...
            const int xdiff
                = widgets.mMultiplier->getCaption().empty() ? 0 : tspLevelupMultiplierGap();
