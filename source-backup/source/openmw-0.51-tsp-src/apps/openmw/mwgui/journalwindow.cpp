...
namespace
{
    ...
    struct JournalWindowImpl : MWGui::JournalBooks, MWGui::JournalWindow
    {
        ...
        struct AddNamesToList
        {
            ...
            {
                // TSP_JOURNAL_LIST_FONT_V3
                // Main journal pages keep their large BookTypesetter font.
                // Only quest/topic menu rows use 16px.
                mList->addItem(name, 0, 16);
            }
        };
        ...
        void moveSelectedIndex(int offset)
        {
            ...
            // TSP_JOURNAL_INDEX_NAV_V3
            // Navigate columns using their real lengths. Latin's final
            // column has only 8 entries (S-Z), so index 26 must never
            // become selectable.
            const size_t numChars = mEncoding == ToUTF8::WINDOWS_1251 ? 30 : 26;
            const size_t rows = mIndexRowCount;
            const size_t columns = (numChars + rows - 1) / rows;

            const size_t col = mSelectedIndex / rows;
            const size_t row = mSelectedIndex % rows;
            const size_t colStart = col * rows;
            const size_t colCount = std::min(rows, numChars - colStart);

            if (offset == -1) // Up
            {
                if (row == 0)
                    mSelectedIndex = colStart + colCount - 1;
                else
                    --mSelectedIndex;
            }
            ...
            else // Left / Right
            {
                size_t newCol;
                if (offset < 0)
                    newCol = (col + columns - 1) % columns;
                else
                    newCol = (col + 1) % columns;

                const size_t newStart = newCol * rows;
                const size_t newCount = std::min(rows, numChars - newStart);
                mSelectedIndex = newStart + std::min(row, newCount - 1);
            }
        ...
        void setControllerFocusedQuest(size_t index)
        {
            ...
            if (mSelectedQuest < listSize)
            {
                ...
                // TSP_JOURNAL_SCROLL_V3
                // Scroll using actual rendered row positions/heights.
                Gui::MWList* list = getWidget<Gui::MWList>(mQuestMode ? QuestsList : TopicsList);
                list->ensureItemVisible(mButtons[mSelectedQuest]);
            }
        }
    };
}
