...
namespace MWGui
{
    void SettingsWindow::configureWidgets(MyGUI::Widget* widget, bool init)
    {
        ...
        while (widgets.next())
        {
            ...
            // TSP_DROPDOWN_FONT_051_V56 -- a ComboBox ignores FontHeight from the layout
            // and from its MW_ComboBox ResourceLayout template, so these drew at the
            // global font size. Runtime setFontHeight is what works. getOrDefault means
            // no gui.hpp and no defaults.bin entry.
            if (MyGUI::ComboBox* tspCombo = current->castType<MyGUI::ComboBox>(false))
            {
                int tspComboFont = Settings::Manager::getOrDefault<int>("dropdown font size", "GUI", 18);
                if (tspComboFont < 8)
                    tspComboFont = 8;
                else if (tspComboFont > 40)
                    tspComboFont = 40;
                tspCombo->setFontHeight(tspComboFont);
            }
        ...
        , mCfgMgr(cfgMgr)
    {
        ...
        // TSP_INTERNAL_RESOLUTION_MENU_051_V21
        // Internal GL4ES render targets only. SDL/EGL remains 1280x720.
        ...
        Log(Debug::Info)
            << "TSP_INTERNAL_RESOLUTION_MENU_051_V21"
            << " targets=1280x720,1152x648,1024x576,960x540,800x450,640x360";
    ...
    void SettingsWindow::onResolutionAccept()
    {
        ...
        if (resolution)
        {
            ...
            // TSP_INTERNAL_RESOLUTION_AUTORESTART_051_V20R3
            // Store selected INTERNAL resolution. Do not call apply(), because
            // SDL/KMS must remain at the physical 1280x720 panel mode.
            Settings::video().mResolutionX.set(resolution->first);
            Settings::video().mResolutionY.set(resolution->second);

            const char* restartMarker = std::getenv("OPENMW_TSP_RESTART_MARKER");

            if (restartMarker != nullptr && *restartMarker != '\0')
            {
                std::ofstream marker(
                    restartMarker,
                ...
                {
                    Log(Debug::Warning)
                        << "Could not create TSP resolution restart marker: "
                        << restartMarker;
                }
            }
    ...
    void SettingsWindow::updateControlsBox()
    {
        ...
        for (const int& action : actions)
        {
            ...
            // TSP_SETTINGS_COMPACT_CONTROLS_051_V35_3
            leftText->setFontHeight(Settings::gui().mTspControlsFontSize);
            leftText->setCaptionWithReplacing(desc);

            Gui::SharedStateButton* rightText = mControlsBox->createWidget<Gui::SharedStateButton>(
                "SandTextButton", MyGUI::IntCoord(), MyGUI::Align::Default);
            rightText->setFontHeight(Settings::gui().mTspControlsFontSize);
    ...
    void SettingsWindow::updateWindowModeSettings()
    {
        ...
        {
            // TSP_INTERNAL_RESOLUTION_MODE_GUARD_051_V21
            // These entries are internal render targets, not SDL display modes.
            // Never replace the selected target with the panel's native mode.
            mWindowBorderButton->setEnabled(false);
        }
