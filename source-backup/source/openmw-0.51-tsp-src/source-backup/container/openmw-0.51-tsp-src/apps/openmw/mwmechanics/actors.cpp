...
namespace MWMechanics
{
    ...
    {
        TSP_SCOPE(TspProf::TSP_SLOT_ACTORS); // Actors::update
        if (!paused)
        {
            ...
            // TSP_ACTOR_SCAN_BUDGET_V1
            // Two O(N^2) scans live inside this per-actor loop: the engageCombat
            // inner loop over every other actor, and updateHeadTracking, which
            // walks mActors itself. In a busy exterior that is thousands of pair
            // tests per frame - measured at 907 ms in the `actors` slot. Cap the
            // per-frame work; whatever is cut runs on a following frame, so
            // nothing is dropped permanently, it is just noticed a beat later.
            //
            // Reimplemented 2026-08-22 - the original V1 source did not survive
            // the tree loss, so the budgets are deliberately env-tunable:
            //   TSP_ACTOR_COMBAT_PAIR_BUDGET   pair tests/frame   (default 1500)
            //   TSP_ACTOR_HEADTRACK_BUDGET     actors/frame       (default 12)
            // Set either very high to restore uncapped behaviour without a rebuild.
            static int sTspPairBudget = -1;
            static int sTspHeadBudget = -1;
            if (sTspPairBudget < 0)
            {
                const char* ep = std::getenv("TSP_ACTOR_COMBAT_PAIR_BUDGET");
                sTspPairBudget = (ep != nullptr && std::atoi(ep) > 0) ? std::atoi(ep) : 1500;
                const char* eh = std::getenv("TSP_ACTOR_HEADTRACK_BUDGET");
                sTspHeadBudget = (eh != nullptr && std::atoi(eh) > 0) ? std::atoi(eh) : 12;
            }
            int tspPairsLeft = sTspPairBudget;
            int tspHeadLeft = sTspHeadBudget;
            int tspPairsCut = 0;
            int tspHeadCut = 0;
