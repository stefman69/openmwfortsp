...
/* TSP_LISTWRAP_V2 - time OSG display-list submission separately.
   The shim counts ~178 glDrawElements/frame while OSG reports ~656 visible
   drawables. nifloader.cpp never calls setUseDisplayList(false), so NIF statics
   keep OSG's default display-list path and are submitted via glCallList, which
   this shim never wrapped - that cost has been hiding inside other_us (61 ms of
   an 83 ms slow frame, unattributed). glNewList/glEndList are timed separately:
   if news is non-zero, lists are being RECOMPILED during gameplay, which would
   be a finding on its own.
   V2 over V1: the L line carries d= (this frame's draw count) so the reader can
   drop menu/loading frames exactly like the S readers do (agreement 25). */
REAL(glCallList, void, (unsigned int))
REAL(glCallLists, void, (int, unsigned int, const void *))
REAL(glNewList, void, (unsigned int, unsigned int))
REAL(glEndList, void, (void))
static double us_list = 0.0;
static double us_newl = 0.0;
static unsigned long n_list = 0;
static unsigned long n_newl = 0;
void glCallList(unsigned int list)
{
    double t0 = 0;
    st_init();
    resolve_glCallList();
    if (g_on) { n_list++; t0 = now_ms(); }
    if (real_glCallList) real_glCallList(list);
    if (g_on) us_list += (now_ms() - t0) * 1000.0;
}
void glCallLists(int n, unsigned int type, const void *lists)
{
    ...
    st_init();
    resolve_glCallLists();
    if (g_on) { if (n > 0) n_list += (unsigned long)n; t0 = now_ms(); }
    if (real_glCallLists) real_glCallLists(n, type, lists);
    if (g_on) us_list += (now_ms() - t0) * 1000.0;
}
void glNewList(unsigned int list, unsigned int mode)
{
    ...
    st_init();
    resolve_glNewList();
    if (g_on) { n_newl++; t0 = now_ms(); }
    if (real_glNewList) real_glNewList(list, mode);
    if (g_on) us_newl += (now_ms() - t0) * 1000.0;
}
void glEndList(void)
{
    double t0 = 0;
    ...
    resolve_glEndList();
    if (g_on) { t0 = now_ms(); }
    if (real_glEndList) real_glEndList();
    if (g_on) us_newl += (now_ms() - t0) * 1000.0;
}
...
void SDL_GL_SwapWindow(void* w)
{
    ...
    /* TSP_LISTWRAP_V2 - move list time out of other_us and emit an L line.
       No ms= key on the L line, so the existing S-line readers skip it. */
    sum += us_list + us_newl;
    other = frame_us - sum;
    if (g_out && g_frame < g_max)
        fprintf(g_out, "L f=%lu d=%lu lms=%.2f list_us=%.0f lists=%lu new_us=%.0f news=%lu oth_us=%.0f\n",
                g_frame, n_draw, frame_us / 1000.0, us_list, n_list, us_newl, n_newl, other);
    us_list = 0.0;
    us_newl = 0.0;
    n_list = 0;
    n_newl = 0;
