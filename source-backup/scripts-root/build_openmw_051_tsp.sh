...
for candidate in \
    ...
    /usr
do
    if find "$candidate" -type f \( -name 'libosg.so.162' -o -name 'libosg.so' \) \
...
if [ -z "$OSG_PREFIX" ]; then
    echo "ERROR: Could not locate the patched OSG installation used by OpenMW 0.48."
    echo "Do not install a random replacement; the patched OSG stack is required."
    exit 1
fi

echo "Patched OSG prefix candidate: $OSG_PREFIX"

export CMAKE_PREFIX_PATH="$OSG_PREFIX:/usr/local:/usr:${CMAKE_PREFIX_PATH:-}"
export PKG_CONFIG_PATH="$OSG_PREFIX/lib/pkgconfig:$OSG_PREFIX/lib/aarch64-linux-gnu/pkgconfig:/usr/local/lib/pkgconfig:/usr/local/lib/aarch64-linux-gnu/pkgconfig:${PKG_CONFIG_PATH:-}"
export LD_LIBRARY_PATH="$OSG_PREFIX/lib:$OSG_PREFIX/lib/aarch64-linux-gnu:/usr/local/lib:/usr/local/lib/aarch64-linux-gnu:${LD_LIBRARY_PATH:-}"
