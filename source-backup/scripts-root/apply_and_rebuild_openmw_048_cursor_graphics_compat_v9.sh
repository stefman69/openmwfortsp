...
OUTPUT_BINARY="${3:-/root/openmw_cursor_graphics_compat_v9}"
...
echo "============================================================"
echo "OpenMW 0.48 TSP cursor + GL4ES compatibility patch V9"
...
CPP_BACKUP="$CPP.before-tsp-compat-v9.$STAMP"
HPP_BACKUP="$HPP.before-tsp-compat-v9.$STAMP"
WATER_BACKUP="$WATER_CPP.before-tsp-compat-v9.$STAMP"
GROUNDCOVER_BACKUP="$GROUNDCOVER_CPP.before-tsp-compat-v9.$STAMP"
OBJECTPAGING_BACKUP="$OBJECTPAGING_CPP.before-tsp-compat-v9.$STAMP"
...
new_update = r'''    // TSP_MYGUI_CURSOR_V9
...
if depth_callback_line in water:
    water = water.replace(
        depth_callback_line,
        "    // TSP_GL4ES_WATER_V9: depth clamp disabled for gl4es/GLES2.",
        1,
    )
elif "TSP_GL4ES_WATER_V9: depth clamp disabled" not in water:
...
if "TSP_GL4ES_GROUNDCOVER_V9: use normal transforms" not in groundcover:
    groundcover, constructor_count = constructor_pattern.subn(
        "         // TSP_GL4ES_GROUNDCOVER_V9: use normal transforms, not instancing.",
        groundcover,
        count=1,
    )
    if constructor_count != 1:
        raise RuntimeError(
            ...
            "Expected the two VertexAttribDivisor assignments and shader "
            "attribute bindings in Groundcover::Groundcover."
        )

new_groundcover_chunk = r'''    osg::ref_ptr<osg::Node> Groundcover::createChunk(
        InstanceMap& instances, const osg::Vec2f& center)
    {
        // TSP_GL4ES_GROUNDCOVER_V9
...
grep -n "TSP_MYGUI_CURSOR_V9" "$CPP" "$HPP"
grep -n "SDL_SCANCODE_PAUSE" "$CPP"
grep -n "mTspCursorVisible" "$CPP" "$HPP"
grep -n "TSP_GL4ES_WATER_V9" "$WATER_CPP"
grep -n "TSP_GL4ES_GROUNDCOVER_V9" "$GROUNDCOVER_CPP"
