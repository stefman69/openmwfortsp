...
OUTPUT_BINARY="${4:-/root/openmw-0.51-tsp-gl4es-white-screen-linkfix}"
...
SCRIPT_REVISION="TSP-051-GL4ES-WHITE-SCREEN-LINKFIX-2026-08-07"
...
echo
echo "Preparing COLLADA linker compatibility..."
# Ubuntu 20.04 provides COLLADA DOM 2.4, while this OpenMW/OSG CMake cache can
# regenerate the final OpenMW link command with the unavailable name
# -lcollada-dom2.5-dp.  The previous workaround only exported LIBRARY_PATH.
# That is not reliable for an already-configured CMake/Ninja tree because the
# generated linker command does not contain an explicit -L directory.
#
# This revision makes the repair deterministic in THREE ways:
#   1. build-only alias in /root,
#   2. explicit -L directory persisted in CMAKE_EXE_LINKER_FLAGS,
#   3. temporary alias in /usr/lib, a normal GNU linker search directory.
# The alias points at the real 2.4 library.  The linked ELF should therefore
# retain the real library SONAME rather than a fictitious 2.5 runtime need.
LINK_COMPAT_DIR="/root/openmw-0.51-tsp-link-compat"
LINK_COMPAT_NAME="libcollada-dom2.5-dp.so"
SYSTEM_COMPAT_LINK="/usr/lib/$LINK_COMPAT_NAME"
mkdir -p "$LINK_COMPAT_DIR"

COLLADA_REAL=""
if command -v ldconfig >/dev/null 2>&1; then
    COLLADA_REAL="$(ldconfig -p 2>/dev/null \
        | awk '/libcollada-dom2\.4-dp\.so/{print $NF; exit}')"
fi

if [ -z "$COLLADA_REAL" ] || [ ! -e "$COLLADA_REAL" ]; then
    COLLADA_REAL="$(find \
        /usr/lib \
        /usr/local/lib \
        -maxdepth 4 \
        \( -type f -o -type l \) \
        -name 'libcollada-dom2.4-dp.so*' \
        -print 2>/dev/null \
        | head -1 || true)"
fi

if [ -z "$COLLADA_REAL" ] || [ ! -e "$COLLADA_REAL" ]; then
    echo "ERROR: Could not locate the installed COLLADA DOM 2.4 library."
    echo "The OpenMW linker workaround cannot be installed safely."
    echo "Searched ldconfig plus /usr/lib and /usr/local/lib."
    exit 1
fi

COLLADA_REAL="$(readlink -f "$COLLADA_REAL")"
if [ ! -f "$COLLADA_REAL" ]; then
    echo "ERROR: Resolved COLLADA library is not a regular file:"
    echo "  $COLLADA_REAL"
    exit 1
fi

ln -sfn "$COLLADA_REAL" "$LINK_COMPAT_DIR/$LINK_COMPAT_NAME"
ln -sfn "$COLLADA_REAL" "$SYSTEM_COMPAT_LINK"

for alias_path in \
    "$LINK_COMPAT_DIR/$LINK_COMPAT_NAME" \
    "$SYSTEM_COMPAT_LINK"
do
    if [ "$(readlink -f "$alias_path")" != "$COLLADA_REAL" ]; then
        echo "ERROR: COLLADA compatibility alias verification failed:"
        echo "  $alias_path"
        exit 1
    fi
done

# Persist an explicit -L path in the existing CMake cache.  This is the key
# difference from the previous failed revision: Ninja's actual g++ command
# will now contain a linker search directory before -lcollada-dom2.5-dp.
CURRENT_EXE_LINKER_FLAGS="$(sed -n \
    's/^CMAKE_EXE_LINKER_FLAGS:STRING=//p' \
    "$BUILD_DIR/CMakeCache.txt" 2>/dev/null \
    | head -1)"

case " $CURRENT_EXE_LINKER_FLAGS " in
    *" -L$LINK_COMPAT_DIR "*)
        FIXED_EXE_LINKER_FLAGS="$CURRENT_EXE_LINKER_FLAGS"
        ;;
    *)
        FIXED_EXE_LINKER_FLAGS="${CURRENT_EXE_LINKER_FLAGS:+$CURRENT_EXE_LINKER_FLAGS }-L$LINK_COMPAT_DIR"
        ;;
esac
...
cmake \
    -S "$SOURCE_DIR" \
    -B "$BUILD_DIR" \
    "-DCMAKE_EXE_LINKER_FLAGS:STRING=$FIXED_EXE_LINKER_FLAGS"

CACHED_EXE_LINKER_FLAGS="$(sed -n \
    's/^CMAKE_EXE_LINKER_FLAGS:STRING=//p' \
    "$BUILD_DIR/CMakeCache.txt" \
    | head -1)"

case " $CACHED_EXE_LINKER_FLAGS " in
    *" -L$LINK_COMPAT_DIR "*) ;;
    *)
        echo "ERROR: CMake did not retain the COLLADA linker search directory."
        echo "CMAKE_EXE_LINKER_FLAGS=$CACHED_EXE_LINKER_FLAGS"
        exit 1
        ;;
esac

# Ask Ninja for the real final command and refuse to build unless the explicit
# -L fix is present.  This catches a bad cache regeneration BEFORE compilation.
OPENMW_LINK_COMMAND="$(ninja -C "$BUILD_DIR" -t commands openmw 2>/dev/null \
    | grep -F ' -o openmw ' \
    | tail -1 || true)"

if [ -z "$OPENMW_LINK_COMMAND" ]; then
    echo "ERROR: Could not locate the generated OpenMW linker command."
    exit 1
fi

if printf '%s\n' "$OPENMW_LINK_COMMAND" | grep -Fq -- "-lcollada-dom2.5-dp"; then
    if ! printf '%s\n' "$OPENMW_LINK_COMMAND" | grep -Fq -- "-L$LINK_COMPAT_DIR"; then
        echo "ERROR: OpenMW still requests COLLADA 2.5 but the explicit -L fix"
        echo "is missing from the real Ninja linker command. Refusing to build."
        printf '%s\n' "$OPENMW_LINK_COMMAND"
        exit 1
    fi
fi

export LIBRARY_PATH="$LINK_COMPAT_DIR${LIBRARY_PATH:+:$LIBRARY_PATH}"
export LD_LIBRARY_PATH="$(dirname "$COLLADA_REAL")${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}"

echo "COLLADA real library:        $COLLADA_REAL"
echo "Build-only alias:            $LINK_COMPAT_DIR/$LINK_COMPAT_NAME"
echo "Default-path alias:          $SYSTEM_COMPAT_LINK"
echo "CMAKE_EXE_LINKER_FLAGS:      $CACHED_EXE_LINKER_FLAGS"
echo "Generated linker command contains explicit -L fix: YES"
...
echo
echo "Verifying COLLADA runtime dependency..."
COLLADA_NEEDED="$(readelf -d "$BUILT_BINARY" 2>/dev/null     | grep -i 'collada' || true)"
if [ -n "$COLLADA_NEEDED" ]; then
    printf '%s\n' "$COLLADA_NEEDED"
fi
if printf '%s\n' "$COLLADA_NEEDED" | grep -q 'libcollada-dom2.5-dp'; then
    echo "ERROR: The executable retained the build-only COLLADA 2.5 alias as a runtime dependency."
    echo "Refusing to package it."
    exit 1
fi

echo "COLLADA runtime dependency verification: OK"
...
echo
echo "============================================================"
echo "OpenMW 0.51 TSP GL4ES white-screen/link-fix rebuild completed"
echo "============================================================"
