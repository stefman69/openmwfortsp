...
static void load_config(void)
{
    ...
    if (g_scale_enabled && (g_source_w <= 0 || g_source_h <= 0))
    {
        fprintf(
            stderr,
            "TSP_SWAPSCALER_051_V33 scale-disabled reason=invalid-source source=%s\n",
...
#define LOAD_GLES(name)                                                        \
    do {                                                                       \
        *(void**)(&p_##name) = dlsym(g_gles, #name);                           \
        if (!p_##name) {                                                       \
            fprintf(stderr, "TSP_SWAPSCALER_051_V33 missing-symbol=%s\n", #name); \
            fflush(stderr);                                                    \
            g_disabled = 1;                                                    \
            return 0;                                                          \
        }                                                                      \
...
static int load_gles(void)
{
    ...
    if (!g_gles)
    {
        ...
        fprintf(
            stderr,
            "TSP_SWAPSCALER_051_V33 disabled reason=libGLESv2-dlopen-failed error=%s\n",
            err ? err : "<unknown>");
        fflush(stderr);
        g_disabled = 1;
        return 0;
...
static GLuint compile_shader(GLenum type, const char* source)
{
    ...
    if (!ok)
    {
        ...
        fprintf(
            stderr,
            "TSP_SWAPSCALER_051_V33 shader-compile-failed type=0x%x log=%s\n",
            type,
            log);
        fflush(stderr);
...
static int ensure_resources(void)
{
    ...
    if (!g_program)
    {
        ...
        if (!linked)
        {
            ...
            fprintf(
                stderr,
                "TSP_SWAPSCALER_051_V33 program-link-failed log=%s\n",
                log);
            fflush(stderr);
            return 0;
        }
...
static void present_scaled(SDL_Window* window, const char* hook_name)
{
    ...
    if (!g_first_frame_logged)
    {
        fprintf(
            stderr,
            "TSP_SWAPSCALER_051_V33 frame=1 "
            "hook=%s scale=%d fps_overlay=%d "
