...
OUTPUT_BINARY="${4:-/root/openmw-0.51-tsp-input-graphics-v1}"
...
SCRIPT_REVISION="TSP-051-INPUT-GRAPHICS-HYBRID-V1-2026-08-07"
...
if (
    ...
    if len(shader_matches) == 1:
        ...
        # The comment wording can change. Accept the block only when its
        # three defining calls occur in one unambiguous order.
        ...
        if not (0 <= force_true < recreate < restore):
            raise RuntimeError(
                "Could not locate the OpenMW 0.51 forced "
                "simple-water shader block."
            )
...
[ ! -e "$PACKAGE_BINARY" ] || cp -f "$PACKAGE_BINARY" "$PACKAGE_BINARY.before-input-graphics-v1-$STAMP"
