# OpenMW 0.51 TrimUI Smart Pro Manager V2.1

This is the corrected functional revision built from the original launcher mockup. It is installed as a separate Ports entry named `OpenMW_51_Manager_v2`; the proven `Morrowind_51.sh` remains unchanged and is executed only after the player selects **Play Morrowind**.
...
- SDL2 fullscreen presentation through the TrimUI firmware's active display stack, with the existing low-overhead evdev controller input. V2's direct `/dev/fb0` writer was removed after it produced an input-responsive black screen on the target firmware.
...
- Persistent mod data-root discovery, enable/disable and manual asset-priority ordering. Discovery covers both `openmw51/mods` and valid non-base mod roots already referenced by `openmw.cfg`; the root containing `Morrowind.esm` and internal `resources/vfs*` roots are never exposed as toggleable mods.
...
The corrected package expands to one `openmw51-tsp-manager-v2.1` directory containing:
...
```bash
cd ~/Downloads
tar -xzf ./openmw51-tsp-manager-v2.1-package.tar.gz
cd ./openmw51-tsp-manager-v2.1
...
```bash
cd ~/Downloads/openmw51-tsp-manager-v2.1
./build_install_openmw51_tsp_manager_v2.sh collect
```

This collects manager status, selected mod order, default-navmesh candidates, swap state, UDisk state, navmesh profile, generator tail, current display/SDL facts, manager tail and current `openmw.cfg`.
