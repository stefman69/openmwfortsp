...
def discover_mods():
    ...
    for path in configured:
        if not is_data_root(path):
            continue
        names = {p.name.casefold() for p in plugin_files(path)}
...
def default_navmesh_candidates():
    ...
    if override.is_file():
        try:
            ...
            pass
    base = Path("/mnt/SDCARD/data/ports")
    candidates = []
    if not base.is_dir():
        return candidates
    excluded = {"backups", "backup", "savegame-0.51", "navmesh-tool-runtime", ".tsp-051-source-backups"}
    for current, dirs, files in os.walk(base):
        ...
        for name in files:
            ...
            if not (low == "navmesh.db" or ("navmesh" in low and low.endswith(".db"))):
                continue
            path = p / name
            if path == NAVDB or not sqlite_navmesh(path):
                continue
            words = str(path).casefold()
            score = 10 if low == "navmesh.db" else 20
            for token in ("default", "prebuilt", "vanilla", "base", "stock", "assets", "install"):
                if token in words:
                    score += 10
