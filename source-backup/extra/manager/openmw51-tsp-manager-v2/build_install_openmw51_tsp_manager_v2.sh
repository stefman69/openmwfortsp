...
source_selftest() {
    ...
    if command -v g++ >/dev/null 2>&1; then
        g++ -std=c++17 -O2 -Wall -Wextra -Wpedantic "$CPP" -o "$TMP/manager-host-test" || fail 15 "host C++ compile failed"
        "$TMP/manager-host-test" --selftest | grep -Fq OPENMW51_MANAGER_V2_SELFTEST_PASS || fail 15 "C++ behavior selftest failed"
        echo "PASS host C++ compile + behavior selftest"
    else
...
"$CXX" -std=c++17 -O2 -Wall -Wextra -Wpedantic -static-libstdc++ -static-libgcc \
    /tmp/openmw51_launcher_manager_v2.cpp -o /tmp/openmw51-manager-v2 || exit 21
