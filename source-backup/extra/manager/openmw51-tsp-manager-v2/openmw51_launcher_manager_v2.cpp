...
{
public:
    explicit Framebuffer(const std::string& path)
    {
        mFd = open(path.c_str(), O_RDWR);
        if (mFd < 0) throw std::runtime_error("open framebuffer failed: " + path);
        if (ioctl(mFd, FBIOGET_FSCREENINFO, &mFix) < 0) throw std::runtime_error("FBIOGET_FSCREENINFO failed");
        if (ioctl(mFd, FBIOGET_VSCREENINFO, &mVar) < 0) throw std::runtime_error("FBIOGET_VSCREENINFO failed");
        mBytes = (mVar.bits_per_pixel + 7u) / 8u;
        if (mBytes < 2 || mBytes > 4) throw std::runtime_error("unsupported framebuffer bpp");
        mLength = mFix.smem_len;
        mMap = mmap(nullptr, mLength, PROT_READ | PROT_WRITE, MAP_SHARED, mFd, 0);
        if (mMap == MAP_FAILED) throw std::runtime_error("framebuffer mmap failed");
        mBackup.resize(mLength);
        std::memcpy(mBackup.data(), mMap, mLength);
    }
    ~Framebuffer()
    {
        if (mMap != MAP_FAILED && !mBackup.empty())
        {
            std::memcpy(mMap, mBackup.data(), std::min(mLength, mBackup.size()));
            msync(mMap, mLength, MS_SYNC);
        }
        if (mMap != MAP_FAILED) munmap(mMap, mLength);
        if (mFd >= 0) close(mFd);
    }
    int width() const { return int(mVar.xres); }
    int height() const { return int(mVar.yres); }
    uint32_t pack(Color c) const
    {
        auto cv = [](uint8_t x, uint32_t n) { if (!n) return 0u; uint32_t m=(1u<<n)-1u; return (uint32_t(x)*m+127u)/255u; };
        uint32_t p=0;
        p |= cv(c.r,mVar.red.length)<<mVar.red.offset;
        p |= cv(c.g,mVar.green.length)<<mVar.green.offset;
        p |= cv(c.b,mVar.blue.length)<<mVar.blue.offset;
        if (mVar.transp.length) p |= ((1u<<mVar.transp.length)-1u)<<mVar.transp.offset;
        return p;
    }
    void pixel(int x, int y, Color c)
    {
        if (x<0 || y<0 || x>=width() || y>=height()) return;
        size_t off=size_t(y+mVar.yoffset)*mFix.line_length+size_t(x+mVar.xoffset)*mBytes;
        if (off+mBytes>mLength) return;
        uint32_t p=pack(c); auto* d=static_cast<uint8_t*>(mMap)+off;
        for (size_t i=0;i<mBytes;++i) d[i]=uint8_t((p>>(8u*i))&0xffu);
    }
...
private:
    int mFd=-1; void* mMap=MAP_FAILED; size_t mLength=0,mBytes=0;
    fb_fix_screeninfo mFix{}; fb_var_screeninfo mVar{}; std::vector<uint8_t> mBackup;
};
...
static void header(Framebuffer&f){f.rect(0,0,f.width(),68,{9,10,12});label(f,"OPENMW 0.51",28,18,3,ACC);label(f,"TRIMUI SMART PRO MANAGER",270,25,2,FG);label(f,"INTEGRATED V2",1030,25,2,DIM);f.rect(0,68,f.width(),2,ACC);}
