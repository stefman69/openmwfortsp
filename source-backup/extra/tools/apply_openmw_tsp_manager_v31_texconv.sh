...
#  15. The game launcher runs on stock TrimUI OS as well as CrossMix. Stock
#      SDL2 is built without sensor support and OpenMW 0.49+ aborts on
#      SDL_INIT_SENSOR; the launcher now probes the SDL2 the loader will
#      actually pick and preloads lib/libtsp_sdl_sensor_shim.so only when that
#      library lacks sensor support. TSP_SDL_SENSOR_SHIM=1/0 overrides.
#  16. The navmesh generator lives in <root>/tools. The manager, the backend
#      and the Ports wrapper all prefer it there; the PORTS copy is now only a
#      fallback and is refreshed only if it already exists.
#  14. .omwscripts files are content files. The manager could not see them, so a
#      switched-off script mod kept its content= line and OpenMW aborted with
#      "Failed loading TSPPerformance.omwscripts: the content file does not
#      exist". They are now managed, ordered after every ESM/ESP, and any
#      content= naming a file that is installed nowhere is removed on apply and
#      blocks PLAY with an offer to repair.
#
...
WORK="$HOST_DIR/openmw-tsp-manager-v2.9"
...
GAME_SH="$WORK/Morrowind.sh"
SHIM_SO="$WORK/libtsp_sdl_sensor_shim.so"
SHIM_EXPECTED_SHA="98e8aaf5bdeaf6dcf73cdb1732cf7f08f1aeefaf91d9b0729f91017ba234a4d0"
...
BUILD_LOG="$WORK/openmw-manager-v29-build-$STAMP.log"
STATE="$HOST_DIR/openmw-manager-v29-install.state"
...
// V2.8: PLAY refuses to start a config that names a content file OpenMW cannot
// load, and offers to repair it.
...
static void refresh(App&a){a.status=readKv(a.statusFile);
    ...
    a.mods=readMods(a.modsFile);if(a.modSel>=int(a.mods.size()))a.modSel=std::max(0,int(a.mods.size())-1);if(a.modSel<a.scroll)a.scroll=a.modSel;if(a.modSel>=a.scroll+8)a.scroll=a.modSel-7;std::ifstream in(a.resultFile);std::getline(in,a.lastResult);}
...
static void header(Framebuffer&f){f.rect(0,0,f.width(),68,{9,10,12});label(f,"OPENMW 0.51",28,18,3,ACC);label(f,"TRIMUI SMART PRO MANAGER",270,25,2,FG);f.rect(0,68,f.width(),2,ACC);}
...
{
    title(f,"DIAGNOSTICS","EXACT PATHS AND READINESS   INTEGRATED V3.0");f.rect(350,178,902,410,P1);
...
static void handle(App&a,Action q)
{
    ...
    if(a.page==Page::Home)
    {
        if(q==Action::Up)a.home=(a.home+5)%6;else if(q==Action::Down)a.home=(a.home+1)%6;else if(q==Action::Back)request(a,"exit");else if(q==Action::Refresh)dispatch(a,"status");else if(q==Action::Select){if(a.home==0){
            // OpenMW aborts on a content= entry whose file is not installed, so
            // offer the repair instead of letting the launch fail.
            if(val(a,"content_status","VALID")!="VALID")
                confirm(a,Page::Home,"OPENMW.CFG NAMES A CONTENT FILE THAT IS NOT INSTALLED AND THE GAME WILL NOT START. REPAIR THE CONFIG NOW?","mods-apply");
            else if(val(a,"navmesh_profile")=="STALE")confirm(a,Page::Home,"ACTIVE MODS DO NOT MATCH THIS NAVMESH. PLAY ANYWAY?","play");
            else request(a,"play");}else if(a.home==1){a.page=Page::Setup;a.opt=0;}else if(a.home==2)a.page=Page::Mods;else if(a.home==3){a.page=Page::Navmesh;a.opt=0;}else if(a.home==4)a.page=Page::Diagnostics;else request(a,"exit");}return;
    }
...
static int selftest()
{
    ...
    if(parseCommand("mods-apply").args.size()!=1||!parseCommand("mods-apply").acknowledge)
    { std::cerr<<"selftest: repair action mapping failed\n"; return 1; }
...
V2.8 changes:
  - .omwscripts files are content files. They were invisible to the manager, so
    a disabled mod's content= line survived every apply and OpenMW aborted with
    "Failed loading TSPPerformance.omwscripts: the content file does not exist";
  - any content= naming a file that exists in no data root is removed on apply,
    and reported by the status scan, so a config can no longer point at content
...
# V2.9: the generator lives inside the game build. The Ports copies are only a
# fallback for an installation that still has the standalone entry.
GENERATOR_NAME = "OpenMW_Generate_Full_Navmesh_3Worker.sh"
GENERATOR_CANDIDATES = (
    ROOT / "tools" / GENERATOR_NAME,
    Path("/mnt/SDCARD/Roms/PORTS") / GENERATOR_NAME,
    Path("/mnt/sdcard/mmcblk1p1/Roms/PORTS") / GENERATOR_NAME,
)
BASE_PLUGINS = {"morrowind.esm", "tribunal.esm", "bloodmoon.esm", "builtin.omwscripts"}
# Everything OpenMW accepts as a content= entry.
PLUGIN_EXTS = {".esm", ".esp", ".omwaddon", ".omwgame", ".omwscripts"}
# The subset that is a real TES3 plugin: has masters, can move collision, and is
# what "this mod changes the world" means for the navmesh profile.
CONTENT_PLUGIN_EXTS = {".esm", ".esp", ".omwaddon", ".omwgame"}
SCRIPT_EXTS = {".omwscripts"}
...
def collision_sensitive(path: Path) -> bool:
    # A script list does not move geometry, so it must not mark the navmesh
    # stale on its own.
    if any(p.suffix.casefold() in CONTENT_PLUGIN_EXTS for p in plugin_files(path)):
        return True
    try:
...
def read_masters(path: Path):
    # An .omwscripts file is a plain list of Lua scripts, not a TES3 plugin.
    if path.suffix.casefold() in SCRIPT_EXTS:
        return []
...
def sort_plugins(paths, external_content):
    ...
    def priority(key):
        ...
        # Masters, then plugins, then script lists. An .omwscripts entry has to
        # follow the content it attaches to.
        if suffix in {".esm", ".omwgame"}:
            kind = 0
    ...
    for item in active:
        ...
        for current, dirs, files in os.walk(root):
            ...
            if rel_current.parts and rel_current.parts[0].casefold() not in {"meshes", "groundcover", "navmesh", "navmeshes"}:
                dirs[:] = []
                files = [x for x in files if Path(x).suffix.casefold() in CONTENT_PLUGIN_EXTS]
            dirs.sort(key=natural)
            for name in sorted(files, key=natural):
                path = Path(current) / name
                if path.suffix.casefold() not in CONTENT_PLUGIN_EXTS and (not rel_current.parts or rel_current.parts[0].casefold() not in {"meshes", "groundcover", "navmesh", "navmeshes"}):
                    continue
                try:
                    st = path.stat()
                except OSError:
...
def known_content_names(plan):
    """Every content file name that actually exists somewhere OpenMW will look."""
    names = set(BASE_PLUGINS)
    roots = [Path(item["path"]) for item in plan]
    data, _ = all_cfg_entries()
    roots.extend(cfg_path(x) for x in data)
    roots.extend([ROOT / "data" / "Data Files", ROOT / "data"])
    for root in roots:
...
def orphan_content(plan, content):
    """content= entries naming a file that is not installed anywhere. OpenMW
    treats one of these as fatal: 'the content file does not exist'."""
    known = known_content_names(plan)
    seen, orphans = set(), []
    for name in content:
        key = name.casefold()
...
def plugin_health(plan):
    try:
        ...
        orphans = orphan_content(plan, existing)
        if orphans:
            return "MISSING CONTENT: " + ", ".join(orphans[:3])
...
def apply_mods():
    ...
    # A content= entry whose file is not installed anywhere is fatal to OpenMW.
    # Drop it here rather than leaving the config unbootable.
    orphans = {x.casefold() for x in orphan_content(plan, all_content)}
    external_content = [
        x for x in all_content
        if x.casefold() not in all_mod_plugins and x.casefold() not in orphans
    ]
    ...
    def strip(lines):
        ...
        for line in lines:
            ...
            if s.startswith("content="):
                name = unquote(s[8:]).casefold()
                if name in all_mod_plugins or name in orphans:
                    continue
            out.append(line)
        return out
    ...
    removed = f", removed {len(orphans)} missing content entries" if orphans else ""
    result(
        f"Applied {sum(1 for x in plan if x['enabled'])} mod roots and "
        f"{len(sorted_plugins)} content files to {target.name}{removed}"
    )
...
def generator_path():
    # Invoked through /bin/bash, so an exFAT card without execute bits is fine.
...
def write_status():
    ...
    _cfg_data, cfg_content = all_cfg_entries()
    orphans = orphan_content(plan, cfg_content)
    gen = generator_path()
    ...
    values = {
        ...
        "content_orphans": len(orphans),
        "content_status": ("VALID" if not orphans else "MISSING: " + ", ".join(orphans[:2])),
...
def selftest():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # V2.8: the exact device crash. A script mod that is switched off must
        # lose its content= line, and one that is switched on must gain it,
        # ordered after every ESM/ESP.
        scripts_on = MODROOT / "90 ScriptsOn"
        scripts_off = MODROOT / "91 ScriptsOff"
        (scripts_on / "scripts").mkdir(parents=True)
        (scripts_off / "scripts").mkdir(parents=True)
        (scripts_on / "TSPInteriorVisGrid.omwscripts").write_text("PLAYER: scripts/x.lua\n")
        (scripts_off / "TSPPerformance.omwscripts").write_text("PLAYER: scripts/y.lua\n")
        user_cfg.write_text(
            "fallback-archive=Morrowind.bsa\ncontent=Morrowind.esm\n"
            "content=TSPPerformance.omwscripts\n",
            encoding="utf-8",
        )
        PLAN_JSON.unlink(missing_ok=True)
        PLAN_TSV.unlink(missing_ok=True)
        script_plan = discover_mods()
        by_name = {x["name"]: x for x in script_plan}
        assert "90 ScriptsOn" in by_name and "91 ScriptsOff" in by_name
        # A script list is not collision content: it must not mark the navmesh.
        assert not by_name["90 ScriptsOn"]["needs_navmesh"]
        wanted = {"10 Core", "20 Patch", "90 ScriptsOn"}
        for item in script_plan:
            item["enabled"] = item["name"] in wanted
        save_plan(script_plan)
        assert orphan_content(script_plan, ["TSPPerformance.omwscripts"]) == []
        apply_mods()
        applied_text = user_cfg.read_text(encoding="utf-8")
        assert "content=TSPInteriorVisGrid.omwscripts" in applied_text, applied_text
        assert "content=TSPPerformance.omwscripts" not in applied_text, applied_text
        assert applied_text.index("content=A.esm") < applied_text.index("content=TSPInteriorVisGrid.omwscripts")

        # A content= line whose file exists nowhere is fatal to OpenMW: it must
        # be reported and removed rather than left in a config that cannot boot.
        broken = user_cfg.read_text(encoding="utf-8") + "content=DeletedMod.esp\n"
        user_cfg.write_text(broken, encoding="utf-8")
        broken_plan = discover_mods()
        assert orphan_content(broken_plan, ["DeletedMod.esp"]) == ["DeletedMod.esp"]
        assert plugin_health(broken_plan).startswith("MISSING CONTENT: DeletedMod.esp")
        apply_mods()
        assert "DeletedMod.esp" not in user_cfg.read_text(encoding="utf-8")
        assert plugin_health(discover_mods()) == "VALID"
        shutil.rmtree(scripts_on)
        shutil.rmtree(scripts_off)
        user_cfg.write_text("fallback-archive=Morrowind.bsa\ncontent=Morrowind.esm\n", encoding="utf-8")
        PLAN_JSON.unlink(missing_ok=True)
        PLAN_TSV.unlink(missing_ok=True)
        discover_mods()
...
OPENMW_V25_BACKEND_PY_EOF
    ...
    cat > "$GAME_SH" <<'OPENMW_V29_GAME_SH_EOF'
#!/bin/bash
...
OPENMW_V29_GAME_SH_EOF

    base64 -d > "$SHIM_SO" <<'OPENMW_V29_SHIM_B64_EOF'

...

...

OPENMW_V29_SHIM_B64_EOF

    chmod +x "$ACTION_SH" "$WRAPPER_SH" "$GENERATOR_SH" "$PY_SRC" "$GAME_SH"
    for file in "$CPP" "$ACTION_SH" "$WRAPPER_SH" "$GENERATOR_SH" "$PY_SRC" "$GAME_SH" "$SHIM_SO"; do
...
host_selftest() {
    ...
    bash -n "$GAME_SH" || fail 13 "game launcher syntax failed"
    need python3
    ...
    # --- the game launcher keeps everything it already did ------------------
    for token in \
        'TSP_CLEAN_LAYOUT_CONFIG_MIGRATION_V1' \
        'openmw-clean-path-migrate.py' \
        'TSP_VISGRID_V24_ROOM_RESIDENCY' \
        'libtsp_warm.so' \
        'libtsp_fullscreen_scaler.so' \
        '/mnt/UDISK/openmw-nav/navmesh.db' \
        '/mnt/SDCARD/data/ports/openmw'
    do
        grep -Fq -e "$token" "$GAME_SH" || fail 16 "game launcher lost: $token"
    done
    grep -Fq 'TSP_SDL_SENSOR_FALLBACK_051_V29' "$GAME_SH" || fail 16 "SDL sensor fallback missing"
    grep -Fq 'TSP_LD_PRELOAD="$TSP_SDL_SHIM:$TSP_LD_PRELOAD"' "$GAME_SH" || fail 16 "shim is not preloaded first"
    [ "$(sha_of "$SHIM_SO")" = "$SHIM_EXPECTED_SHA" ] || fail 16 "the SDL sensor shim is not the supplied binary"
    case "$(file "$SHIM_SO")" in
        *aarch64*|*ARM64*|*ARM\ aarch64*) ;;
        *) fail 16 "the SDL sensor shim is not an ARM64 object: $(file "$SHIM_SO")";;
    esac
    echo "PASS game launcher keeps its behaviour and carries the ARM64 sensor shim"

    # --- executable proof: the sensor probe picks the right library ---------
    awk '/^tsp_sdl2_lacks_sensor\(\) \{/,/^\}/' "$GAME_SH" >  "$TMP/sdlprobe.sh"
    awk '/^tsp_find_sdl2\(\) \{/,/^\}/'        "$GAME_SH" >> "$TMP/sdlprobe.sh"
    [ -s "$TMP/sdlprobe.sh" ] || fail 16 "SDL probe functions not found in the launcher"
    mkdir -p "$TMP/sdl/stock" "$TMP/sdl/crossmix"
    printf 'ELF stock sdl2\0SDL not built with sensor support\0' > "$TMP/sdl/stock/libSDL2-2.0.so.0"
    printf 'ELF crossmix sdl2\0sensors are fine here\0'          > "$TMP/sdl/crossmix/libSDL2-2.0.so.0"
    (
        set -u
        . "$TMP/sdlprobe.sh"
        LD_LIBRARY_PATH="$TMP/sdl/stock"
        picked="$(tsp_find_sdl2)"
        [ "$picked" = "$TMP/sdl/stock/libSDL2-2.0.so.0" ] || { echo "FAIL probe picked $picked"; exit 1; }
        tsp_sdl2_lacks_sensor "$picked" || { echo "FAIL stock SDL2 was not detected as sensorless"; exit 1; }
        LD_LIBRARY_PATH="$TMP/sdl/crossmix:$TMP/sdl/stock"
        picked="$(tsp_find_sdl2)"
        [ "$picked" = "$TMP/sdl/crossmix/libSDL2-2.0.so.0" ] || { echo "FAIL probe ignored path order"; exit 1; }
        tsp_sdl2_lacks_sensor "$picked" && { echo "FAIL CrossMix SDL2 was flagged sensorless"; exit 1; }
        tsp_sdl2_lacks_sensor "$TMP/sdl/nothing-here" && { echo "FAIL a missing file counted as sensorless"; exit 1; }
        exit 0
    ) || fail 16 "SDL sensor probe behaviour failed"
    echo "PASS SDL sensor probe: stock needs the shim, CrossMix does not"

    # --- the generator is self-contained in <root>/tools ---------------------
    grep -Fq '"$ROOT/tools/OpenMW_Generate_Full_Navmesh_3Worker.sh"' "$ACTION_SH" \
        || fail 16 "the manager does not look in <root>/tools for the generator"
    grep -Fq '"$ROOT/tools/OpenMW_Generate_Full_Navmesh_3Worker.sh"' "$WRAPPER_SH" \
        || fail 16 "the Ports wrapper does not look in <root>/tools for the generator"
    grep -Fq 'ROOT / "tools" / GENERATOR_NAME' "$PY_SRC" \
        || fail 16 "the backend does not look in <root>/tools for the generator"
    echo "PASS the navmesh generator is found inside the game build"
    ...
    grep -Fq 'INTEGRATED V3.0' "$CPP" || fail 16 "V3.0 UI marker missing"
...
BROKEN_NAVMESHTOOL_EOF
    ...
    # --- .omwscripts are content, and orphaned content is repaired ----------
    grep -Fq '".omwscripts"' "$PY_SRC" || fail 16 "omwscripts is not a managed content type"
    grep -Fq 'CONTENT_PLUGIN_EXTS' "$PY_SRC" || fail 16 "script/plugin split missing"
    grep -Fq 'def orphan_content' "$PY_SRC" || fail 16 "orphaned content detection missing"
    grep -Fq 'content_status' "$PY_SRC" || fail 16 "content health is not published"
    grep -Fq 'content_status' "$CPP" || fail 16 "the UI never shows content health"
    grep -Fq 'REPAIR THE CONFIG NOW?' "$CPP" || fail 16 "PLAY does not offer the config repair"
    python3 - "$PY_SRC" <<'CONTENT_PROBE_EOF' || fail 14 "omwscripts ordering/orphan behaviour failed"
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    core = root / "core"
    core.mkdir()
    backend.fake_plugin(core / "Base.esm", ["Morrowind.esm"])
    (core / "Late.omwscripts").write_text("PLAYER: scripts/a.lua\n")
    ordered = backend.sort_plugins(backend.plugin_files(core), ["Morrowind.esm"])
    names = [p.name for p in ordered]
    assert names == ["Base.esm", "Late.omwscripts"], names
    assert backend.read_masters(core / "Late.omwscripts") == []
    plan = [{"path": str(core), "enabled": True, "needs_navmesh": False}]
    assert backend.orphan_content(plan, ["Late.omwscripts"]) == []
    assert backend.orphan_content(plan, ["Gone.omwscripts"]) == ["Gone.omwscripts"]
print("CONTENT_PROBE_PASS")
CONTENT_PROBE_EOF
    echo "PASS omwscripts ordering and orphaned-content detection"
...
collect_action() {
    ensure_ssh
    local out="$HOST_DIR/openmw-manager-v29-diagnostics-$STAMP.tar.gz"
...
strings "$ROOT/bin/openmw-manager-v2" 2>/dev/null | grep -F 'INTEGRATED V' || true
...
exec > >(tee "$WORK/openmw-manager-v29-install-$STAMP.log") 2>&1

cat <<'BANNER'
============================================================
OPENMW 0.51 TSP — MANAGER V2.9
STOCK-OS LAUNCHER FALLBACK + SELF-CONTAINED GENERATOR
============================================================
Installs seven device files:
  <root>/bin/openmw-manager-v2
  <root>/launcher/openmw-manager-action-v2.sh
  <root>/launcher/openmw-launcher-backend-v2.py
  <root>/tools/OpenMW_Generate_Full_Navmesh_3Worker.sh   (new home)
  <root>/lib/libtsp_sdl_sensor_shim.so                   (stock OS)
  <PORTS>/OpenMW_Manager.sh
  <PORTS>/Morrowind.sh
The standalone generator Ports entry is refreshed only if present.

Never touched: openmw.cfg, mods, saves, navmesh.db, swapfile,
the clean-path migrator.
============================================================
BANNER
...
for candidate in "$ROOT/tools/OpenMW_Generate_Full_Navmesh_3Worker.sh" "$PORT/OpenMW_Generate_Full_Navmesh_3Worker.sh"; do
    if [ -f "$candidate" ]; then ls -l "$candidate"; sha256sum "$candidate"; fi
done
echo "The generator's canonical home after this install is $ROOT/tools."
[ -f "$ROOT/lib/libtsp_sdl_sensor_shim.so" ] && { echo 'Existing SDL sensor shim:'; sha256sum "$ROOT/lib/libtsp_sdl_sensor_shim.so"; }
echo 'SDL2 the game will load:'
for d in /mnt/SDCARD/System/lib /usr/trimui/lib /usr/lib /lib; do
    [ -e "$d/libSDL2-2.0.so.0" ] || continue
    if grep -q 'SDL not built with sensor support' "$d/libSDL2-2.0.so.0" 2>/dev/null; then
        echo "  NO-SENSOR  $d/libSDL2-2.0.so.0"
    else
        echo "  SENSOR-OK  $d/libSDL2-2.0.so.0"
    fi
done
...
GAME_SHA="$(sha_of "$GAME_SH")"
SHIM_SHA="$(sha_of "$SHIM_SO")"
for value in "$BIN_SHA" "$ACTION_SHA" "$WRAPPER_SHA" "$GENERATOR_SHA" "$PY_SHA" "$GAME_SHA" "$SHIM_SHA"; do valid_sha "$value" || fail 29 "invalid artifact SHA"; done
...
echo "PASS game launcher:        $GAME_SHA"
echo "PASS SDL sensor shim:      $SHIM_SHA"

echo
echo "===== 4/7 ALREADY-INSTALLED CHECK ====="
if ssh "$DEV" 'bash -s' -- "$ROOT" "$BIN_SHA" "$ACTION_SHA" "$WRAPPER_SHA" "$GENERATOR_SHA" "$PY_SHA" "$GAME_SHA" "$SHIM_SHA" <<'REMOTE_IDENTITY' >/dev/null 2>&1
set -u
ROOT="$1"; BIN_SHA="$2"; ACTION_SHA="$3"; WRAPPER_SHA="$4"; GENERATOR_SHA="$5"; PY_SHA="$6"; GAME_SHA="$7"; SHIM_SHA="$8"
PORT=""
for candidate in /mnt/SDCARD/Roms/PORTS /mnt/sdcard/mmcblk1p1/Roms/PORTS; do [ -d "$candidate" ] && PORT="$candidate" && break; done
...
check "$PORT/Morrowind.sh" "$GAME_SHA" || exit 1
check "$ROOT/lib/libtsp_sdl_sensor_shim.so" "$SHIM_SHA" || exit 1
check "$ROOT/tools/OpenMW_Generate_Full_Navmesh_3Worker.sh" "$GENERATOR_SHA" || exit 1
REMOTE_IDENTITY
then
...
DEVICE_BACKUP="$ROOT/launcher/install-backups/manager-v29-before-$STAMP"
...
backup "$PORT/Morrowind.sh" game-launcher
backup "$ROOT/lib/libtsp_sdl_sensor_shim.so" sdl-sensor-shim
backup "$ROOT/tools/OpenMW_Generate_Full_Navmesh_3Worker.sh" tools-generator
sync
...
scp -q "$GAME_SH" "$DEV:$ROOT/launcher/.v24-game-launcher.incoming" || fail 31 "game launcher upload failed"
scp -q "$SHIM_SO" "$DEV:$ROOT/launcher/.v24-sdl-shim.incoming" || fail 31 "SDL sensor shim upload failed"

if ! ssh "$DEV" 'bash -s' -- "$ROOT" "$DEVICE_BACKUP" "$BIN_SHA" "$ACTION_SHA" "$WRAPPER_SHA" "$GENERATOR_SHA" "$PY_SHA" "$GAME_SHA" "$SHIM_SHA" <<'REMOTE_INSTALL'
set -u
ROOT="$1"; B="$2"; BIN_SHA="$3"; ACTION_SHA="$4"; WRAPPER_SHA="$5"; GENERATOR_SHA="$6"; PY_SHA="$7"; GAME_SHA="$8"; SHIM_SHA="$9"
...
check "$stage/.v24-game-launcher.incoming" "$GAME_SHA" || exit 28
check "$stage/.v24-sdl-shim.incoming" "$SHIM_SHA" || exit 29
bash -n "$stage/.v24-game-launcher.incoming" || exit 30
...
# V2.9: the generator's home is inside the game build. The Ports entry is
# refreshed only when it already exists, so it can simply be deleted.
mkdir -p "$ROOT/tools" "$ROOT/lib" || exit 31
install -m 755 "$stage/.v24-navmesh-generator.incoming" "$ROOT/tools/OpenMW_Generate_Full_Navmesh_3Worker.sh" || exit 21
if [ -f "$PORT/OpenMW_Generate_Full_Navmesh_3Worker.sh" ]; then
    install -m 755 "$stage/.v24-navmesh-generator.incoming" "$PORT/OpenMW_Generate_Full_Navmesh_3Worker.sh" || exit 32
    echo "Refreshed the optional standalone generator Ports entry."
else
    echo "No standalone generator Ports entry; the manager uses $ROOT/tools."
fi
install -m 755 "$stage/.v24-manager-python.incoming" "$ROOT/launcher/openmw-launcher-backend-v2.py" || exit 26
install -m 755 "$stage/.v24-game-launcher.incoming" "$PORT/Morrowind.sh" || exit 33
install -m 644 "$stage/.v24-sdl-shim.incoming" "$ROOT/lib/libtsp_sdl_sensor_shim.so" || exit 34
...
check "$ROOT/tools/OpenMW_Generate_Full_Navmesh_3Worker.sh" "$GENERATOR_SHA" || exit 22
check "$ROOT/launcher/openmw-launcher-backend-v2.py" "$PY_SHA" || exit 27
check "$PORT/Morrowind.sh" "$GAME_SHA" || exit 35
check "$ROOT/lib/libtsp_sdl_sensor_shim.so" "$SHIM_SHA" || exit 36
...
echo "Launcher SHA:  $GAME_SHA"
echo "Shim SHA:      $SHIM_SHA"
echo
...
echo "space guard changed there. Its canonical copy now lives in <root>/tools,"
echo "so the Ports entry can be deleted without breaking the manager."
echo
echo "The game launcher now runs on stock TrimUI OS as well as CrossMix. It"
echo "reports its decision at startup under SDL2 SENSOR SUPPORT; force it with"
echo "TSP_SDL_SENSOR_SHIM=1 or off with TSP_SDL_SENSOR_SHIM=0."
echo
