...
REMOTE_TMP_BIN="/tmp/openmw-0.51.v30-r5-r1-lightkeep"

V30_R4_EXPECT_SHA="9203cc3dbdd4c0352c2e2db8c09a3ae10241972b5a1bb93c87b3e9f28ff0904d"

DL="$HOME/Downloads"
STAMP="$(date +%Y%m%d-%H%M%S)"
LOG="$DL/openmw51-roomwake-v30-r5-r1-lightkeep-$STAMP.log"
BUILD_LOG="$DL/openmw51-roomwake-v30-r5-r1-lightkeep-build-$STAMP.log"
STATE="$DL/openmw51-roomwake-v30-r5-r1-lightkeep.state"
HOST_BIN="$DL/openmw-0.51-v30-r5-r1-lightkeep"
TMP="$(mktemp -d "$DL/.roomwake-v30-r5-r1.XXXXXX")"
...
echo "============================================================"
echo "OPENMW 0.51 — ROOMWAKE V30 R5-R1 LIGHT KEEPALIVE"
echo "CONTROLLER REVISION: V30-R5-R1 STRUCTURAL PATCHER / LIGHT KEEPALIVE 3X"
echo "============================================================"
...
# -------------------------------------------------------------------------
# V30 R5-R1: light-only 3x keepalive.
...
# STRUCTURAL PATCHER: no regex and no exact multiline formatting dependency.
# -------------------------------------------------------------------------

def normalized_cpp(text):
...
# Patch the hard-object lifecycle call by locating the actual C++ statement.
call_prefix = 'const bool shouldLive = MWRender::isInteriorTopologyObjectResident('
call_positions = find_all(sc, call_prefix)
if len(call_positions) != 1:
    ...
    raise RuntimeError(
        'V30 R5-R1 shouldLive prefix matches=%d expected 1; candidates=%r'
        % (len(call_positions), candidates[-30:]))

call_token = call_positions[0]
call_line_start = sc.rfind('\n', 0, call_token) + 1
call_end = sc.find(';', call_token)
if call_end < 0:
    raise RuntimeError('V30 R5-R1 shouldLive statement semicolon not found')
call_end += 1
call_stmt = sc[call_line_start:call_end]
call_norm = normalized_cpp(call_stmt)
expected_call_norm = (
    'const bool shouldLive = MWRender::isInteriorTopologyObjectResident( '
    'origin, 0.f, false);'
)
if call_norm != expected_call_norm:
    raise RuntimeError(
        'V30 R5-R1 unexpected shouldLive statement after V30 reapply: %r'
        % call_norm)

indent = sc[call_line_start:call_token]
if indent.strip():
    raise RuntimeError('V30 R5-R1 shouldLive indentation parse failed: %r' % indent)

call_repl = (
    indent + '// TSP_ROOM_LIGHT_KEEPALIVE_051_V30_R5\n'
    + indent + '// Lights get 3x R4 minimum inter-room distance; other objects remain radius 0.\n'
    + indent + 'const bool tspRoomExtendedLight = ptr.getType() == ESM::REC_LIGH;\n'
    + indent + 'const float tspRoomObjectRadius = tspRoomExtendedLight ? 1560.f : 0.f;\n'
    + indent + 'const bool shouldLive = MWRender::isInteriorTopologyObjectResident(\n'
    + indent + '    origin, tspRoomObjectRadius, false);'
)
sc = sc[:call_line_start] + call_repl + sc[call_end:]
print('PASS V30 R5-R1 structural shouldLive:', call_norm)

# Patch the exact-XY nonstructural ownership block structurally as well.
z_token = 'constexpr float zSlack = 110.f;'
z_positions = find_all(vc, z_token)
blocks = []
for zpos in z_positions:
    line_start = vc.rfind('\n', 0, zpos) + 1
    overlap_pos = vc.find('overlaps = origin.x()', zpos, zpos + 500)
    if overlap_pos < 0:
        continue
    overlap_end = vc.find(';', overlap_pos)
    if overlap_end < 0:
        continue
    blocks.append((line_start, overlap_end + 1, zpos))

if len(blocks) != 1:
    nearby = [line for line in vc.splitlines()
              if 'zSlack' in line or 'overlaps = origin.x()' in line]
    raise RuntimeError(
        'V30 R5-R1 ownership blocks=%d expected 1; candidates=%r'
        % (len(blocks), nearby[-30:]))

block_start, block_end, zpos = blocks[0]
old_block = vc[block_start:block_end]
old_norm = normalized_cpp(old_block)
expected_old_norm = (
...
if old_norm != expected_old_norm:
    raise RuntimeError(
        'V30 R5-R1 unexpected ownership block: %r' % old_norm)

vis_indent = vc[block_start:zpos]
if vis_indent.strip():
    raise RuntimeError('V30 R5-R1 ownership indentation parse failed: %r' % vis_indent)

new_overlap = (
    vis_indent + '// TSP_ROOM_LIGHT_KEEPALIVE_051_V30_R5\n'
    + vis_indent + '// radius=0 preserves exact V27/V30 clutter ownership. Positive radius\n'
    + vis_indent + '// is used only by ESM Light refs and expands XY around active sectors.\n'
    + vis_indent + '// Z remains tight to avoid waking lights on another floor.\n'
    + vis_indent + 'const float xyPad = radius > 0.f ? radius : 0.f;\n'
    + vis_indent + 'const float zSlack = radius > 0.f ? 100.f : 110.f;\n'
    + vis_indent + 'overlaps = origin.x() >= minX - xyPad && origin.x() <= maxX + xyPad\n'
    + vis_indent + '    && origin.y() >= minY - xyPad && origin.y() <= maxY + xyPad\n'
    + vis_indent + '    && origin.z() >= minZ - zSlack && origin.z() <= maxZ + zSlack;'
)
vc = vc[:block_start] + new_overlap + vc[block_end:]
print('PASS V30 R5-R1 structural ownership:', old_norm)
