...
WORK="$HOST_DIR/openmw-tsp-manager-v2.5"
...
BUILD_LOG="$WORK/openmw-manager-v25-build-$STAMP.log"
STATE="$HOST_DIR/openmw-manager-v25-install.state"
...
def atlas_descriptor():
    return {
        ...
        "content_plugins": [],
        "validated_assets": {"nif_count": TSP_ATLAS_NIF_COUNT, "nif_bytes": TSP_ATLAS_NIF_BYTES},
...
def validate_default_mod_profile():
    ...
    nifs = sorted((path for path in atlas_root.rglob("*") if path.is_file() and path.suffix.casefold() == ".nif"), key=lambda path: natural(str(path)))
    nif_bytes = sum(path.stat().st_size for path in nifs)
    if len(nifs) != TSP_ATLAS_NIF_COUNT or nif_bytes != TSP_ATLAS_NIF_BYTES:
        raise RuntimeError(
            f"TSPAtlasGlobalV02 asset mismatch: expected {TSP_ATLAS_NIF_COUNT} NIFs/{TSP_ATLAS_NIF_BYTES} bytes, "
            f"found {len(nifs)} NIFs/{nif_bytes} bytes"
        )
    texture_roots = (
...
def install_atlas_descriptor(atlas_root: Path) -> None:
    target = atlas_root / TSP_ATLAS_DESCRIPTOR_NAME
    text = json.dumps(atlas_descriptor(), indent=2, sort_keys=True) + "\n"
    try:
    ...
    if json.loads(target.read_text(encoding="utf-8")) != atlas_descriptor():
        raise RuntimeError("TSP Atlas data-mod manifest verification failed")


def apply_default_mod_profile():
    roots = validate_default_mod_profile()
    install_atlas_descriptor(dict((ident, path) for ident, _name, path in roots)["tsp-atlas-global-v02"])
    plan = discover_mods()
    ...
    for _ident, _name, path in roots:
        quoted = f'data="{path}"'
        plain = f"data={path}"
        position = max(text.rfind(quoted), text.rfind(plain))
        if position < 0:
...
def default_mod_profile_health():
    # First-launch setup installs storage only, so this deep content audit is no
    # longer on any critical path. It rglobs a large mod root, so it only runs
    # when launcher/check-default-mods exists.
    if not (LAUNCHER / "check-default-mods").exists():
        return "NOT CHECKED"
    try:
        validate_default_mod_profile()
        return "READY"
    except (OSError, RuntimeError) as exc:
        message = " ".join(str(exc).split())
        return "MISSING: " + message[:110]
...
def selftest():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert default_mod_profile_health() == "READY"
        ...
        managed = CFG.read_text(encoding="utf-8")
        expected_roots = [str((MODROOT / relative).resolve()) for _ident, _name, relative in DEFAULT_MOD_PROFILE]
...
host_selftest() {
    ...
    echo "PASS navmesh generator keeps the supplied behaviour plus the V2.5 fixes"
    ...
    grep -Fq 'INTEGRATED V2.5' "$CPP" || fail 16 "V2.5 UI marker missing"
...
echo "[00:00:02 I] 139620/139620 (100%) navmesh tiles are generated"
echo "[00:00:02 I] Done"
exit 0
FAKE_NAVMESHTOOL_EOF
    ...
    echo "PASS Python backend behaviour selftest"
...
collect_action() {
    ensure_ssh
    local out="$HOST_DIR/openmw-manager-v25-diagnostics-$STAMP.tar.gz"
...
exec > >(tee "$WORK/openmw-manager-v25-install-$STAMP.log") 2>&1

cat <<'BANNER'
============================================================
OPENMW 0.51 TSP — MANAGER V2.5
VISIBLE NAVMESH BUILD + BOUNDED IDENTITY PROBE + FAST SCAN
============================================================
...
DEVICE_BACKUP="$ROOT/launcher/install-backups/manager-v25-before-$STAMP"
...
sync
echo "PASS Manager V2.5 installed and self-tested on device"
