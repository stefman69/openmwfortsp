...
if len(handler_matches) != 1:
    candidates = [
        ...
        if 'onFrame' in line and ('guarded' in line or 'engineHandlers' in line)
    ]
    raise RuntimeError(
        'V26 onFrame handler structure matches=%d, expected 1; candidates=%r'
        % (len(handler_matches), candidates[-20:])
    )
base_callback = handler_matches[0].group('base')
print('PASS: captured proven V26 frame callback: ' + base_callback)
...
# Door distance is reconsidered while the player walks WITHIN the same room.
# Do not rewrite engineHandlers.  Wrap the exact mapState callback it already uses.
frame_wrapper = (
    "\nmapState.v29BaseOnFrame = %s\n"
    "%s = function(dt)\n"
...
startup_pat = re.compile(
    r"(?m)^[ \t]*print\(['\"]\[TSP_VISGRID_V26\] CURRENT-SECTOR-ONLY[^\n]*\)[ \t]*$"
)
startup_matches = list(startup_pat.finditer(s))
if len(startup_matches) != 1:
    candidates = [line for line in s.splitlines() if '[TSP_VISGRID_V26] CURRENT-SECTOR-ONLY' in line]
    raise RuntimeError(
        'V26 startup marker matches=%d, expected 1; candidates=%r'
        % (len(startup_matches), candidates[-10:])
    )
sm = startup_matches[0]
s = s[:sm.end()] + "\nprint('[TSP_VISGRID_V29] DOOR-PREWAKE-HARD v26-core=1 portal-3d=1 rays=0')" + s[sm.end():]

for token in (
    'TSP_VISGRID_LUA_V29_DOOR_PREWAKE_HARD',
    "pcall(camera.setInteriorTopologyPvs,",
    'mapState.pvsBoxes, ids, 0.0, 0.0',
    'mapState.v29BaseOnFrame = ' + base_callback,
...
profile_pat = re.compile(
    r"\$TSP_VISGRID_DIR/v(?:26|27|28|29)_profiles/visgrid-[^'\"\n]+\.lua"
)
...
if len(profile_matches) != 1:
    ...
    raise RuntimeError(
        'launcher selected-profile structure matches=%d, expected 1; candidates=%r'
        % (len(profile_matches), candidates[-20:])
    )
pm = profile_matches[0]
l = l[:pm.start()] + new_profile + l[pm.end():]

l, n_profile_label = re.subn(
    r"(?m)^([ \t]*echo[ \t]+['\"]Visgrid Profile=)[^'\"]*(['\"][ \t]*)$",
    r"\1v29-door-prewake-hard\2",
    l,
    count=1,
)
if n_profile_label != 1:
    raise RuntimeError('launcher Visgrid Profile label matches=%d, expected 1' % n_profile_label)

l, n_policy = re.subn(
    r"(?m)^[ \t]*echo[ \t]+['\"]Visgrid room policy=[^'\"]*['\"][ \t]*$",
    'echo "Visgrid room policy=V26 hard delete; current room + physically-near portal destinations"',
    l,
    count=1,
)
if n_policy != 1:
...
bad_handler = "guarded('onFrame', " + "onFrameBody, dt)"
if bad_handler in c:
    raise SystemExit('hard-coded onFrameBody callback assumption survived controller')
if re.search(r'\.write_text\s*\([^)]*newline\s*=', c, re.S):
    raise SystemExit('Python 3.8 newline-argument misuse survived controller')
