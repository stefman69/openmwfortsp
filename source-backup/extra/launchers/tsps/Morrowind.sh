...
if [ "$OSG_THREADING" = DrawThreadPerContext ] && ! grep -qs '^core=off' "$GAMEDIR/tsp_drawthread_policy.txt"; then
    ...
    echo "TSP_DRAWTHREAD_V1 core tier=[$_tier] @ ${_best} kHz main=cpu${TSP_MAIN_GUESS:-?} draw=cpu${TSP_DRAW_CPU:-none} onlined=${TSP_DRAW_ONLINED:-none} online_now=$(cat "$TSP_SYS_CPU/online" 2>/dev/null)" | tee -a /mnt/SDCARD/tsp_prog.txt
fi
...
if grep -qs '^clock=off' "$GAMEDIR/tsp_drawthread_policy.txt"; then
    echo "TSP_CPUCLOCK_V1 off (policy)" | tee -a /mnt/SDCARD/tsp_prog.txt
else
    : > "$TSP_CLOCK_SAVE"; _cl=""
    for _d in "$TSP_SYS_CPU"/cpu[0-9]*; do
        _n=${_d##*/cpu}; case "$_n" in ''|*[!0-9]*) continue ;; esac
    ...
    echo "TSP_CPUCLOCK_V1 armed$_cl avail=[$(cat "$TSP_SYS_CPU/cpu0/cpufreq/scaling_available_frequencies" 2>/dev/null | tr -s ' ' ',')] cooling=[$_cool] temp=$(for _z in /sys/class/thermal/thermal_zone*/temp; do cat "$_z" 2>/dev/null; done | sort -n | tail -1)" | tee -a /mnt/SDCARD/tsp_prog.txt
fi
# <<< TSP_CPUCLOCK_V1 END
    tsp_lever_prelaunch
# ===================== TSP_AB_SWITCH_V1 ======================================
...
echo "TSP_DRAWTHREAD_V1 armed model=$OSG_THREADING main=cpu${TSP_CPU_MAIN:-?}/${TSP_CPU_MAIN_MASK:-none} draw=cpu${TSP_DRAW_CPU:-none}/${TSP_DRAW_MASK:-none} rest=${TSP_REST_MASK:-${TSP_CPU_BG_MASK:-none}} idle_park=$TSP_IDLE_PARK pid=$OPENMW_PID $(date '+%F %T')" | tee -a /mnt/SDCARD/tsp_prog.txt
