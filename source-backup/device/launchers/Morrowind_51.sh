#!/bin/bash
export OPENMW_TSP_NAVMESHDB=/mnt/UDISK/openmw51-nav/navmesh.db
...
export TSP_MPW_OUT=/mnt/SDCARD/tsp_mpwatch.txt
export TSP_MPW_SECS=30
export OPENMW_TSP_FIX_NULL_VIEWPORT=1
export OPENMW_TSP_RING=/mnt/SDCARD/tsp_ring.txt
export OPENMW_TSP_RING_TRIGGER_MS=45     # TSP_INTERIOR_CAPTURE plateau not spike
export OPENMW_TSP_RING_MAX_DUMPS=30      # TSP_INTERIOR_CAPTURE
export TSP_STATE=1                       # TSP_STATECOST
export TSP_STATE_OUT=/mnt/SDCARD/tsp_state.txt   # TSP_STATECOST
export TSP_STATE_MAX=2000000             # TSP_STATECOST
    export TSP_NO_CELL_GLRELEASE=1
export TSP_CRASH_OUT=/mnt/SDCARD/tsp_crash.txt
    TSP_LD_PRELOAD="$GAMEDIR/lib/libtsp_state.so:$GAMEDIR/lib/libtsp_mpwatch.so:$GAMEDIR/lib/libtsp_crash.so:$GAMEDIR/lib/libtsp_diag.so:$GAMEDIR/lib/libtsp_warm.so:$GAMEDIR/lib/libtsp_fullscreen_scaler.so:$TSP_GL4ES_LIBRARY"
...
export LIBGL_FBCONVERT=1
    ...
    LD_PRELOAD="$TSP_LD_PRELOAD" "$OPENMW_BIN" \
        ...
        --config "$CONFIG_DIR" 2>>/mnt/SDCARD/tsp_prog.txt &
